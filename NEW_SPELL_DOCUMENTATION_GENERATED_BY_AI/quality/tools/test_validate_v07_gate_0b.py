...
MODULE_PATH = Path(__file__).with_name("validate_v07_gate_0b.py")
SPEC = importlib.util.spec_from_file_location("validate_v07_gate_0b", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
gate = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(gate)


class GateV07ZeroBValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_all_work_package_ids_suffixes_order_and_status_are_exact(self) -> None:
        ...
        wrong_id["qualified_work_packages"][0]["work_package_id"] = "V07-OBS-010"
    ...
    def test_pending_state_rejects_candidate_evidence_or_claim_smuggling(self) -> None:
        mutations = (
            ...
            ("claims", "v0_7_candidate_qualified", True),
    ...
    def test_activation_documents_render_and_validate_bound_values(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            gate_path = root / "SPELL_v0.7_Gate_0B.md"
            release_path = root / "SPELL_v0.7_Release.md"
            ...
            self.assertIn(
                b"| Annotated tag object | `refs/tags/v0.7.0` | Pending |\n\n"
    ...
    def test_atomic_activation_applies_exact_proposal_and_removes_lock(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            scope_path = root / gate.ACTIVATION_LOCK_RELATIVE.parent / "v0.7-gate-0b.json"
    ...
    def test_reviewed_driver_ir_api_migration_dependency_and_compatibility_deltas_are_exact(self) -> None:
        mutations = (
            ("internal_ir", "new_internal_version", "9.9"),
            ("internal_ir", "legacy_serialized_bytes_must_remain_unchanged", False),
            ("driver_contract", "legacy_unary_rpc_count", 10),
            ("observation_api", "route_families", ["UNBOUNDED_API"]),
            ("database_schema", "new_tables_by_migration", {}),
            ("dependencies", "new_python_runtime_dependencies", ["unknown"]),
    ...
    def test_candidate_manifest_contract_requires_all_45_passing_unwaived_ids(self) -> None:
        ...
        skipped["work_packages"]["V07-OBS-002"]["test_ids"]["V07-OBS-002-ATOMIC"]["skipped_count"] = 1
        ...
        mapped_skip["historical_platform_skips"]["mapped_test_ids_skipped"] = ["V07-OBS-009-LOAD-SECURITY"]
        mutations.append(mapped_skip)
        missing = copy.deepcopy(manifest)
        missing["work_packages"]["V07-OBS-008"]["test_ids"].pop("V07-OBS-008-BACKPRESSURE")
    ...
    def test_projection_migration_ast_has_exact_activation_tuple(self) -> None:
        raw = (
            ROOT / "backend/migrations/versions/v0005_observation_projection.py"
        ).read_bytes()
        expected = gate.MIGRATION_TABLES["0005_observation_projection"]
        self.assertEqual(
            gate._migration_new_tables(raw, "0005_observation_projection"),
            expected,
        )
        changed = raw.replace(
            b"    observation_outbox,\n)",
            b"    observation_outbox,\n    observation_streams,\n)",
        )
        self.assertNotEqual(
            gate._migration_new_tables(changed, "0005_observation_projection"),
            expected,
        )

    def test_condition_migration_ast_has_exact_bound_inventory(self) -> None:
        raw = (
            ROOT / "backend/migrations/versions/v0006_observation_conditions.py"
        ).read_bytes()
        self.assertEqual(
            gate._condition_migration_models(raw),
            gate.CONDITION_MIGRATION_MODELS,
        )
        mutations = (
            (b" if table.name != Execution.__tablename__", b""),
            (b"table.name != Execution.__tablename__", b"table.name == Execution.__tablename__"),
            (b"metadata.sorted_tables", b"metadata.tables"),
            (b"Execution.__table__.to_metadata(metadata)", b"VerifyOperation.__table__.to_metadata(metadata)"),
            (b"    WaitForOperation.__table__,\n", b""),
        )
        for before, after in mutations:
            with self.subTest(before=before):
                ...
                with self.assertRaises(gate.GateValidationError):
                    gate._condition_migration_models(changed)

    def test_current_observation_api_and_driver_services_match_reviewed_surface(self) -> None:
        app = (ROOT / "backend/app.py").read_text(encoding="utf-8")
        for marker in gate.API_ROUTE_MARKERS:
            self.assertIn(marker, app)
        for path in self.scope["reviewed_surface_delta"]["observation_api"]["service_boundary_files"]:
            self.assertTrue((ROOT / path).is_file())
        proto = (ROOT / "contracts/spell/driver/v1/driver.proto").read_text(encoding="utf-8")
        self.assertEqual(
            gate._rpc_names(gate._service_body(proto, "DriverInfrastructureService")),
            (
                "Handshake", "Health", "OpenContext", "CloseContext",
                "AttachExecution", "DetachExecution", "CancelLifecycleOperation",
                "DrainHost", "GetOperation",
            ),
        )
        self.assertEqual(
            gate._rpc_names(gate._service_body(proto, "DriverObservationService")),
            ("GetTime", "GetTM"),
        )
