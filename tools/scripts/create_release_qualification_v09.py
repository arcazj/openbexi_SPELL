#!/usr/bin/env python3
"""Create the canonical SPELL v0.9.0 release-qualification manifest."""
...
from scripts import validate_release_evidence_v09 as release  # noqa: E402
from scripts.build_reproducible_v09 import product_package_sha256_v09  # noqa: E402
from scripts.source_fingerprint_v09 import source_fingerprint_v09  # noqa: E402


MANIFEST_RELATIVE = Path("artifacts/v0.9") / release.MANIFEST_NAME
WORK_PACKAGE_EVIDENCE_PATHS = frozenset(
    path
    for path in release.CANONICAL_EVIDENCE_PATHS
    if path.startswith("artifacts/v0.9/work-package/")
)
...
@dataclass(frozen=True)
class ReleaseQualificationProductionV09:
    path: str
    sha256: str
...
def _assert_generation_git_state(
    ...
    *,
    ...
    tracked_evidence = sorted(
        release._git_name_list(
            root,
            ["ls-tree", "-r", "--name-only", "-z", "HEAD", "--", "artifacts/v0.9"],
...
def _sbom(root: Path, source_fingerprint: str) -> dict[str, Any]:
    directory_relative = "artifacts/v0.9/sbom"
...
def _supply_chain(root: Path, source_fingerprint: str) -> dict[str, Any]:
    relative = "artifacts/v0.9/supply-chain.json"
    ...
    return {
        ...
        "accepted_v08_artifacts_unchanged": capture.get(
            "accepted_v08_artifacts_unchanged"
        ),
        "accepted_v08_release": capture.get("accepted_v08_release"),
    }


def _evidence(root: Path) -> dict[str, Any]:
    files = release.canonical_evidence_files(root, root / "artifacts/v0.9")
    declarations = {
...
def _validate_manifest_components(root: Path, manifest: Mapping[str, Any]) -> None:
    ...
    release.validate_v08_unchanged(root, _mapping(manifest["inherited_v08"], "inherited_v08"))
...
def build_release_qualification_v09(root: Path) -> dict[str, Any]:
    ...
    _require(
        source_fingerprint_v09(source_root) == source_fingerprint,
        "source changed while release qualification was assembled",
    )
    _require(
        product_package_sha256_v09(source_root) == product_package,
...
def create_release_qualification_v09(
    root: Path,
) -> ReleaseQualificationProductionV09:
    source_root = root.resolve()
    output = source_root / MANIFEST_RELATIVE
    manifest = build_release_qualification_v09(source_root)
    raw = _manifest_bytes(manifest)
    output.parent.mkdir(parents=True, exist_ok=True)
    _require(
        not release.path_has_link_or_reparse_v09(source_root, output.parent),
        "release qualification output directory must not be a link or reparse point",
    )
    _require(
        not release.path_has_link_or_reparse_v09(source_root, output),
    ...
    return ReleaseQualificationProductionV09(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = create_release_qualification_v09(args.root)
    print(
        json.dumps(
            {
                "schema_version": release.SCHEMA_VERSION,
