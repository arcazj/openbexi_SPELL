...
MODULE_PATH = Path(__file__).with_name("validate_v08_gate_0b.py")
SPEC = importlib.util.spec_from_file_location("validate_v08_gate_0b", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
gate = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(gate)


class GateV08ZeroBValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_all_work_package_ids_suffixes_order_and_status_are_exact(self) -> None:
        ...
        wrong_id["qualified_work_packages"][0]["work_package_id"] = "V08-DATA-010"
    ...
    def test_pending_state_rejects_candidate_evidence_or_claim_smuggling(self) -> None:
        mutations = (
            ...
            ("claims", "v0_8_candidate_qualified", True),
    ...
    def test_activation_documents_render_and_validate_bound_values(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            gate_path = root / "SPELL_v0.8_Gate_0B.md"
            release_path = root / "SPELL_v0.8_Release.md"
            ...
            self.assertIn(
                b"| Annotated tag object | `refs/tags/v0.8.0` | Pending |\n\n"
    ...
    def test_atomic_activation_applies_exact_proposal_and_removes_lock(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            scope_path = root / gate.ACTIVATION_LOCK_RELATIVE.parent / "v0.8-gate-0b.json"
    ...
    def test_reviewed_driver_ir_api_migration_dependency_and_compatibility_deltas_are_exact(self) -> None:
        mutations = (
            ...
            ("data_api", "route_families", ["UNBOUNDED_API"]),
    ...
    def test_candidate_manifest_contract_requires_all_45_passing_unwaived_ids(self) -> None:
        ...
        skipped["work_packages"]["V08-DATA-002"]["test_ids"]["V08-DATA-002-GRAPH"]["skipped_count"] = 1
        ...
        mapped_skip["historical_platform_skips"]["mapped_test_ids_skipped"] = ["V08-DATA-009-SECURITY"]
        mutations.append(mapped_skip)
        missing = copy.deepcopy(manifest)
        missing["work_packages"]["V08-DATA-008"]["test_ids"].pop("V08-DATA-008-POSTGRES")
    ...
    def test_data_model_ast_has_exact_fifteen_table_inventory(self) -> None:
        raw = (ROOT / "backend/data_models.py").read_bytes()
        expected = gate.MIGRATION_TABLES["0007_data_local_service"]
        self.assertEqual(gate._data_model_table_names(raw), expected)
        changed = raw.replace(
            b'__tablename__ = "data_audit_outbox"',
            b'__tablename__ = "data_audit_outbox_changed"',
            1,
        )
        with self.assertRaises(gate.GateValidationError):
            gate._data_model_table_names(changed)

    def test_data_migration_and_api_match_reviewed_surface(self) -> None:
        migration = (
            ROOT / "backend/migrations/versions/v0007_data_local_service.py"
        ...
        app = (ROOT / "backend/data_api.py").read_text(encoding="utf-8")
        for marker in gate.API_ROUTE_MARKERS:
            self.assertIn(marker, app)
        for path in self.scope["reviewed_surface_delta"]["data_api"]["service_boundary_files"]:
