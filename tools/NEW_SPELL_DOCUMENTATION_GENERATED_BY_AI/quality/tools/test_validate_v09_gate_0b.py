...
MODULE_PATH = Path(__file__).with_name("validate_v09_gate_0b.py")
SPEC = importlib.util.spec_from_file_location("validate_v09_gate_0b", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
gate = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(gate)


class GateV09ZeroBValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_all_work_package_ids_suffixes_order_and_status_are_exact(self) -> None:
        ...
        wrong_id["qualified_work_packages"][0]["work_package_id"] = "V09-DEV-010"
    ...
    def test_pending_state_rejects_candidate_evidence_or_claim_smuggling(self) -> None:
        mutations = (
            ...
            ("claims", "v0_9_candidate_qualified", True),
    ...
    def test_activation_documents_render_and_validate_bound_values(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            gate_path = root / "SPELL_v0.9_Gate_0B.md"
            release_path = root / "SPELL_v0.9_Release.md"
            ...
            self.assertIn(
                b"| Annotated tag object | `refs/tags/v0.9.0` | Pending |\n\n"
    ...
    def test_atomic_activation_applies_exact_proposal_and_removes_lock(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            scope_path = root / gate.ACTIVATION_LOCK_RELATIVE.parent / "v0.9-gate-0b.json"
    ...
    def test_reviewed_development_migration_dependency_and_compatibility_deltas_are_exact(self) -> None:
        mutations = (
            ("development_surface", "route_prefix", "/unbounded"),
            ("development_surface", "procedure_language_services_are_non_executing", False),
            ("accepted_boundaries", "accepted_v08_artifacts_immutable", False),
    ...
    def test_candidate_manifest_contract_requires_all_45_passing_unwaived_ids(self) -> None:
        ...
        skipped["work_packages"]["V09-DEV-002"]["test_ids"]["V09-DEV-002-COMPLETION"]["skipped_count"] = 1
        ...
        mapped_skip["historical_platform_skips"]["mapped_test_ids_skipped"] = ["V09-DEV-009-FAULT-RECOVERY"]
        mutations.append(mapped_skip)
        missing = copy.deepcopy(manifest)
        missing["work_packages"]["V09-DEV-008"]["test_ids"].pop("V09-DEV-008-TRANSACTION-AUDIT")
    ...
    def test_development_model_ast_has_exact_nineteen_table_inventory(self) -> None:
        raw = (ROOT / "backend/development_models.py").read_bytes()
        expected = gate.MIGRATION_TABLES["0008_development_environment"]
        self.assertEqual(gate._development_model_table_names(raw), expected)
        changed = raw.replace(
            b'__tablename__ = "development_runtime_pins"',
            b'__tablename__ = "development_runtime_pins_changed"',
            1,
        )
        with self.assertRaises(gate.GateValidationError):
            gate._development_model_table_names(changed)

    def test_development_migration_and_surface_files_match_reviewed_scope(self) -> None:
        migration = (
            ROOT / "backend/migrations/versions/v0008_development_environment.py"
        ...
        surface = self.scope["reviewed_surface_delta"]["development_surface"]
        self.assertEqual(surface["frontend_entrypoint"], "/development.html")
        self.assertIn("backend/development_api.py", surface["service_boundary_files"])
        ...
        self.assertIn(
            '_git_blob(root, commit, "backend/development_api.py")',
            validator_source,
        )
        self.assertNotIn(
            '_git_blob(root, commit, "backend/app.py")\n    _require(\n'
            '        surface["route_prefix"]',
            validator_source,
        )
