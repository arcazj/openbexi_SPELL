#!/usr/bin/env python3
"""Create the source-bound SPELL v0.11 qualification manifest."""
...
def _gate_paths(values: list[str]) -> dict[str, Path]:
    ...
    for value in values:
        ...
        if not separator or gate_id not in EXPECTED_GATES or gate_id in result:
            raise ReleaseV11Error(f"invalid or duplicate --junit value: {value}")
        result[gate_id] = Path(raw_path)
    if set(result) != set(EXPECTED_GATES):
        raise ReleaseV11Error("every required v0.11 JUnit gate must be supplied")
    return result
...
def create_manifest(arguments: argparse.Namespace, root: Path = ROOT) -> dict:
    ...
    if not arguments.frontend_build_passed or not arguments.image_probe_passed:
        raise ReleaseV11Error("frontend build and image probe must both pass")
    if not re.fullmatch(r"sha256:[0-9a-f]{64}", arguments.image_id):
        raise ReleaseV11Error("image identity must be a full sha256 digest")

    commit = git_commit(root)
    manifest = {
        "schema_version": "spell.v11.release-qualification/1",
...
def main() -> int:
    ...
    try:
        ...
        print(f"v0.11-release-qualification=FAIL reason={exc}")
        return 1
    print(
        "v0.11-release-qualification=PASS "
