#!/usr/bin/env python3
"""Validate the bounded SPELL v0.8.0 final release evidence and tag."""
...
DEFAULT_EVIDENCE_ROOT = ROOT / "artifacts" / "v0.8"
MANIFEST_NAME = "release-qualification.json"
SCHEMA_VERSION = "spell.v08.release-qualification/1"
PRODUCT_VERSION = "0.8.0"
SCOPE_PROFILE = "LOCAL_SYNTHETIC_NON_CUI_DATA_SERVICE"

# The Gate 0A and accepted v0.7 identities are immutable governance inputs.
...
V07_COMMIT = V07_RELEASE_COMMIT
V07_ARTIFACT_TREE = "b6b4a9239e36eaea61da8e7d87cc5bffecfd064f"
V07_EXTERNAL_RELEASE_BINDING = {
...
RELEASE_TAG = "v0.8.0"
RELEASE_TAG_REF = "refs/tags/v0.8.0"
FORBIDDEN_ALIAS_REF = "refs/tags/v0.8"
RELEASE_TAG_TITLE = "SPELL v0.8.0"
RELEASE_PACKAGE_PATH = "artifacts/v0.8/openbexi-spell-v0.8.0.tar.gz"
...
WORK_PACKAGE_PATH = "artifacts/v0.8/work-package/qualification.json"
WORK_PACKAGE_VALIDATOR = "scripts/validate_candidate_evidence_v08.py"
GATE_0B_SCOPE = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/"
    "scopes/v0.8-gate-0b.json"
)
GATE_0B_DOCUMENT = "SPELL_v0.8_Gate_0B.md"
GATE_0B_VALIDATOR = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/"
    "validate_v08_gate_0b.py"
)
GATE_0B_MARKER = (
...
FINAL_QUALIFICATION_PATH = "artifacts/v0.8/final/qualification.json"
CANONICAL_EVIDENCE_PATHS = frozenset(
    {
        ...
        "artifacts/v0.8/work-package/tests/backend-docker-host.xml",
        "artifacts/v0.8/work-package/tests/backend-v08-soak.json",
        "artifacts/v0.8/work-package/tests/driver-host.xml",
        "artifacts/v0.8/work-package/tests/tooling.xml",
        "artifacts/v0.8/work-package/tests/frontend-vitest.xml",
        "artifacts/v0.8/work-package/tests/frontend-build.json",
        "artifacts/v0.8/work-package/tests/frontend-playwright-mocked.xml",
        "artifacts/v0.8/work-package/tests/frontend-playwright-live.xml",
        "artifacts/v0.8/work-package/browser/desktop-as-run-report.png",
        "artifacts/v0.8/work-package/browser/desktop-v03-validation.png",
        "artifacts/v0.8/work-package/browser/mobile-durable-prompt.png",
        "artifacts/v0.8/work-package/browser/telemetry-observation-desktop.png",
        "artifacts/v0.8/work-package/browser/telemetry-observation-mobile.png",
        "artifacts/v0.8/work-package/browser/telemetry-observation-desktop.json",
        "artifacts/v0.8/work-package/browser/telemetry-observation-mobile.json",
        FINAL_QUALIFICATION_PATH,
        "artifacts/v0.8/final/tests/backend-sqlite.xml",
        "artifacts/v0.8/final/tests/backend-postgresql.xml",
        "artifacts/v0.8/final/tests/backend-docker-host.xml",
        "artifacts/v0.8/final/tests/driver-host.xml",
        "artifacts/v0.8/final/tests/tooling.xml",
        "artifacts/v0.8/final/tests/frontend-unit.json",
        "artifacts/v0.8/final/tests/frontend-build.json",
        "artifacts/v0.8/final/tests/browser-mocked.xml",
        "artifacts/v0.8/final/tests/browser-real.xml",
        "artifacts/v0.8/sbom/backend.cdx.json",
        "artifacts/v0.8/sbom/driver.cdx.json",
        "artifacts/v0.8/sbom/frontend.cdx.json",
        "artifacts/v0.8/sbom/proxy.cdx.json",
        "artifacts/v0.8/sbom/SHA256SUMS",
        "artifacts/v0.8/supply-chain.json",
    }
)
JUNIT_SUITE_IDS = (
    "backend_sqlite",
...
SBOM_SUBJECTS = {
    "backend.cdx.json": "openbexi_spell-backend:0.8",
    "driver.cdx.json": "openbexi_spell-driver:0.8",
    "frontend.cdx.json": "openbexi_spell-frontend:0.8",
    "proxy.cdx.json": "openbexi_spell-proxy:0.8",
}
...
class ReleaseEvidenceError(ValueError):
    """Raised when final v0.8 evidence cannot be accepted."""
...
@dataclass(frozen=True)
class ReleaseEvidenceValidationV08:
...
def read_strict_json(
    ...
    *,
    ...
    _require(
        path.is_file() and not is_link_or_reparse_v08(path),
...
def validate_frontend_build(
    ...
    _require(relative == "artifacts/v0.8/final/tests/frontend-build.json", "frontend build capture path differs")
    ...
    _require(capture.get("schema_version") == "spell.v08.frontend-build/1", "frontend build schema differs")
...
def validate_version_metadata(root: Path) -> None:
    ...
    _require(
        pyproject_path.is_file() and not is_link_or_reparse_v08(pyproject_path),
    ...
    _require(supply.get("release_artifact_root") == "artifacts/v0.8", "v0.8 artifact root differs")
    ...
    for relative in ("driver_host/Dockerfile", "driver_host/pki.Dockerfile"):
        ...
        _require("ARG SPELL_PACKAGE_VERSION=0.8.0" not in lines, f"{relative} incorrectly advances driver identity")


def _allowed_candidate_path(path: str) -> bool:
    return path in ALLOWED_CANDIDATE_DELTA or path.startswith("artifacts/v0.8/work-package/")
...
def validate_release_closeout_paths(
    ...
    *,
    ...
    for path in paths:
        _require(
            path.startswith("artifacts/v0.8/"),
            f"release closeout changes source outside canonical v0.8 evidence: {path}",
        )
        _require(
...
def validate_committed_release_package(root: Path, release_head: str) -> str:
    package_raw = _git_blob_bytes(
        root, release_head, RELEASE_PACKAGE_PATH, "v0.8.0 release package"
    )
    sidecar_raw = _git_blob_bytes(
        root, release_head, RELEASE_PACKAGE_SIDECAR_PATH, "v0.8.0 package sidecar"
    )
    package_path = _regular_relative(
        root, RELEASE_PACKAGE_PATH, "v0.8.0 release package"
    )
    sidecar_path = _regular_relative(
        root, RELEASE_PACKAGE_SIDECAR_PATH, "v0.8.0 package sidecar"
    )
    _require(
        package_path.read_bytes() == package_raw,
        "v0.8.0 release package differs from the release commit",
    )
    _require(
        sidecar_path.read_bytes() == sidecar_raw,
        "v0.8.0 package sidecar differs from the release commit",
    )
    archive_sha = sha256_bytes(package_raw)
    try:
        sidecar = sidecar_raw.decode("ascii")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.8.0 package sidecar is not ASCII") from exc
    _require(
        sidecar == f"{archive_sha}  {package_path.name}\n",
        "v0.8.0 package sidecar differs",
    )
    return archive_sha
...
def validate_v07_unchanged(root: Path, inherited: Mapping[str, Any]) -> None:
    _exact_keys(
        inherited,
        (
            "classification", "supports", "direct_v08_proof", "accepted_tag_ref",
    ...
    _require(inherited.get("classification") == "ACCEPTED_RELEASE_BASELINE_NOT_DIRECT_V08_PROOF", "v0.7 evidence classification differs")
    _require(inherited.get("supports") == ["release-lineage", "compatibility", "toolchain"], "v0.7 support scope differs")
    _require(inherited.get("direct_v08_proof") is False, "v0.7 evidence is misclassified as direct v0.8 proof")
    expected = {
        "accepted_tag_ref": V07_TAG_REF,
        "accepted_tag_object": V07_TAG_OBJECT,
        "accepted_commit": V07_COMMIT,
        "accepted_artifact_tree": V07_ARTIFACT_TREE,
        "accepted_archive_path": V07_ARCHIVE_RELATIVE,
        "accepted_archive_sha256": V07_ARCHIVE_SHA256,
        "accepted_sidecar_path": V07_SIDECAR_RELATIVE,
        "accepted_sidecar_sha256": V07_SIDECAR_SHA256,
        "accepted_tag_archive_claim": V07_TAG_ARCHIVE_CLAIM,
    }
    for key, value in expected.items():
        _require(inherited.get(key) == value, f"inherited_v07.{key} differs")
    try:
        validate_accepted_v07_release(root)
    except ValueError as exc:
        raise ReleaseEvidenceError(str(exc)) from exc
    _require(_git_line(root, ["show-ref", "--verify", "--hash", V07_TAG_REF], V07_TAG_REF) == V07_TAG_OBJECT, "v0.7.0 tag moved")
    _require(_git_line(root, ["rev-parse", "HEAD:artifacts/v0.7"], "HEAD v0.7 artifact tree") == V07_ARTIFACT_TREE, "accepted v0.7 artifacts changed in release HEAD")
    _require(_git_line(root, ["rev-parse", f"{V07_COMMIT}:artifacts/v0.7"], "tagged v0.7 artifact tree") == V07_ARTIFACT_TREE, "accepted v0.7 tag artifact tree differs")
    diff = _run_git(root, ["diff", "--quiet", V07_COMMIT, "--", "artifacts/v0.7"], accepted=(0, 1))
    _require(diff.returncode == 0, "accepted v0.7 artifacts have a working-tree change")
    status = _run_git(
        root,
        ["status", "--porcelain", "-z", "--untracked-files=all", "--", "artifacts/v0.7"],
    ).stdout
    _require(status == b"", "accepted v0.7 artifacts have an untracked change")


def canonical_evidence_files(root: Path, evidence_root: Path) -> dict[str, Path]:
    _require(
        evidence_root.is_dir()
        and not path_has_link_or_reparse_v08(root, evidence_root),
        "v0.8 evidence root is missing or unsafe",
    )
    files: dict[str, Path] = {}
    for path in evidence_root.rglob("*"):
        ...
        if path_has_link_or_reparse_v08(root, path):
            raise ReleaseEvidenceError(
                f"v0.8 evidence contains a link or reparse point: {relative_root}"
            )
        ...
        if path.name.startswith("."):
            raise ReleaseEvidenceError(f"v0.8 canonical evidence contains a temporary file: {relative_root}")
        if path.name.endswith((".tar.gz", ".tar.gz.sha256")):
            _require(
                relative_root in RELEASE_PACKAGE_PATHS,
                f"v0.8 evidence contains a noncanonical package output: {relative_root}",
            )
            continue
        _require(path.stat().st_size <= MAX_EVIDENCE_FILE_BYTES, f"v0.8 evidence file is oversized: {relative_root}")
        files[relative_root] = path
    _require(
        set(files) == CANONICAL_EVIDENCE_PATHS,
        "canonical v0.8 evidence file inventory differs",
    )
    return dict(sorted(files.items()))


def evidence_fingerprint_v08(files: Mapping[str, Path]) -> str:
...
def _scan_secret_bytes(payload: bytes, label: str) -> None:
    ...
    from scripts.build_reproducible_v08 import SECRET_PATTERNS
...
def validate_evidence_inventory(
    ...
    _require(set(declared) == set(files), "canonical v0.8 evidence file inventory differs")
    ...
    fingerprint = evidence_fingerprint_v08(files)
...
def validate_work_package(
    ...
    from scripts.validate_candidate_evidence_v08 import validate_candidate_evidence

    result = validate_candidate_evidence(root, root / "artifacts/v0.8/work-package")
...
def _validate_suite_manifest(
    ...
    expected_path = f"artifacts/v0.8/final/tests/{suite_id.replace('_', '-')}.{extension}"
    ...
    *,
    ...
    _exact_keys(
        ...
        (
            ...
            "toolchain", "suites", "teardown", "accepted_v07_release",
            "accepted_exceptions", "overall_pass",
        ),
        "final qualification summary",
    )
    _require(
        summary.get("schema_version") == "spell.v08.final-qualification/1",
    ...
    _require(
        _mapping(summary.get("accepted_v07_release"), "final qualification accepted_v07_release")
        == V07_EXTERNAL_RELEASE_BINDING,
        "final qualification accepted v0.7 release binding differs",
    )
    _require(
...
def validate_sbom(
    ...
    _require(sbom.get("directory") == "artifacts/v0.8/sbom", "SBOM directory differs")
    _require(sbom.get("checksum_manifest") == "artifacts/v0.8/sbom/SHA256SUMS", "SBOM checksum path differs")
    _require(sbom.get("source_fingerprint_sha256") == source_fingerprint, "SBOM declared source fingerprint differs")
    directory = root / "artifacts/v0.8/sbom"
    _require(
        directory.is_dir() and not path_has_link_or_reparse_v08(root, directory),
        "SBOM directory is missing or unsafe",
    )
    discovered = {
        path.name
        for path in directory.iterdir()
        if path.is_file() and not is_link_or_reparse_v08(path)
    }
...
def validate_supply_chain(
    ...
    _exact_keys(
        ...
        (
            ...
            "accepted_v07_artifacts_unchanged", "accepted_v07_release",
        ),
        "supply_chain",
    )
    expected_path = "artifacts/v0.8/supply-chain.json"
    _require(supply.get("capture") == expected_path, "supply-chain capture path differs")
    _require(supply.get("test_id") == "V08-SC-001" and supply.get("passed") is True, "supply-chain result did not pass")
    ...
    _require(supply.get("accepted_v07_artifacts_unchanged") is True, "supply-chain audit did not preserve v0.7 artifacts")
    _require(
        _mapping(supply.get("accepted_v07_release"), "supply_chain.accepted_v07_release")
        == V07_EXTERNAL_RELEASE_BINDING,
        "supply-chain accepted v0.7 release binding differs",
    )
    ...
    _require(capture.get("schema_version") == "spell.v08.supply-chain/1", "supply-chain schema differs")
    ...
    _require(capture.get("test_id") == "V08-SC-001" and capture.get("passed") is True, "supply-chain capture did not pass")
    _require(capture.get("source_fingerprint_sha256") == source_fingerprint, "supply-chain capture source differs")
    _require(capture.get("accepted_v07_artifacts_unchanged") is True, "supply-chain capture changed v0.7 artifacts")
    _require(
        _mapping(capture.get("accepted_v07_release"), "supply-chain capture accepted_v07_release")
        == V07_EXTERNAL_RELEASE_BINDING
        and capture.get("accepted_v07_release") == supply.get("accepted_v07_release"),
        "supply-chain capture accepted v0.7 release binding differs",
    )
...
def create_release_tag_message_v08(
...
def validate_release_tag_message_v08(raw: bytes, expected: str) -> None:
    try:
        message = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.8.0 tag message is not strict UTF-8") from exc
    _require(
        message == expected,
        "v0.8.0 tag message differs from the exact ordered release record",
    )


def _release_tag_message_from_object(raw: bytes, release_head: str) -> bytes:
    headers, separator, message = raw.partition(b"\n\n")
    _require(bool(separator), "v0.8.0 tag lacks a message boundary")
    lowered = raw.lower()
    _require(
        not any(marker in lowered for marker in TAG_SIGNATURE_MARKERS),
        "v0.8.0 tag carries an unclaimed cryptographic signature",
    )
    ...
    _require(
        ...
        "v0.8.0 tag headers differ",
    )
    try:
        header_lines[3].decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.8.0 tagger header is not UTF-8") from exc
    return message
...
def validate_release_tag(
    ...
    *,
    ...
    _require(
        ...
        "forbidden lightweight or secondary v0.8 tag exists",
    )
    object_id = _git_line(root, ["show-ref", "--verify", "--hash", RELEASE_TAG_REF], RELEASE_TAG_REF)
    _require(_git_line(root, ["cat-file", "-t", object_id], "v0.8.0 tag type") == "tag", "v0.8.0 is not annotated")
    ...
    expected_message = create_release_tag_message_v08(
        policy,
    ...
    validate_release_tag_message_v08(message, expected_message)
    tagged = _git_line(root, ["rev-parse", f"{RELEASE_TAG_REF}^{{commit}}"], "v0.8.0 tagged commit")
    _require(tagged == release_head, "v0.8.0 tag does not target release HEAD")
...
def validate_release_manifest_shape(manifest: Mapping[str, Any]) -> None:
    _exact_keys(
        ...
        (
            ...
            "work_package", "gate_0b", "toolchain", "final_qualification", "final_suites", "inherited_v07",
...
def validate_release_evidence(
    ...
    *,
...
) -> ReleaseEvidenceValidationV08:
    source_root = root.resolve()
    canonical_root = (evidence_root or (source_root / "artifacts/v0.8")).resolve()
    _require(canonical_root == (source_root / "artifacts/v0.8").resolve(), "release evidence root must be canonical artifacts/v0.8")
    ...
    from scripts.source_fingerprint_v08 import source_fingerprint_v08
    from scripts.build_reproducible_v08 import product_package_sha256_v08

    source_fingerprint = source_fingerprint_v08(source_root)
    declared_source = manifest["qualified_source"]["source_fingerprint_sha256"]
    _require(source_fingerprint == declared_source, "current v0.8 source fingerprint differs")
    product_package = product_package_sha256_v08(source_root)
    ...
    validate_v07_unchanged(source_root, _mapping(manifest["inherited_v07"], "inherited_v07"))
    ...
    return ReleaseEvidenceValidationV08(
...
def validate_release_evidence_v08(
    root: Path,
    evidence_root: Path | None = None,
    *,
    require_tag: bool = False,
