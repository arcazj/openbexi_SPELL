...
$project = "spell-v07-final-$runId"
$qualificationTag = "openbexi-spell-v07-final-qualification:$runId"
$postgresTag = "openbexi-spell-v07-final-postgres:$runId"
...
function Assert-V06ArtifactsUnchanged {
  ...
  $expectedTree = "18cb672a45c538539f78278962ea5822b9f52441"
  $tree = @(& git rev-parse "${Commit}:artifacts/v0.6") -join "`n"
  Assert-NativeSuccess "cannot inspect accepted v0.6 artifact tree"
  if ($tree.Trim() -cne $expectedTree) { throw "accepted v0.6 artifacts changed in source commit" }
  & git diff --quiet v0.6.0 $Commit -- artifacts/v0.6
  Assert-NativeSuccess "accepted v0.6 artifact bytes differ from v0.6.0"
  & git diff --quiet -- artifacts/v0.6
  Assert-NativeSuccess "accepted v0.6 artifacts have a working-tree change"
  $status = @(& git status --porcelain --untracked-files=all -- artifacts/v0.6)
  Assert-NativeSuccess "cannot inspect accepted v0.6 artifact worktree"
  if ($status.Count -ne 0) { throw "accepted v0.6 artifacts have an untracked change" }
...
function New-JUnitSuiteDeclaration {
  ...
  return [ordered]@{
    kind = "junit"
    capture = "artifacts/v0.7/final/tests/$($Id.Replace('_', '-')).xml"
...
function Assert-FinalToolchain {
  ...
  $candidatePath = Join-Path $root "artifacts/v0.7/work-package/qualification.json"
...
function Assert-FinalPublication {
  ...
  Assert-ExactProperties $summary @(
    ...
    "toolchain", "suites", "teardown", "accepted_v06_release", "accepted_exceptions", "overall_pass"
  ) "final qualification summary"
  if (
    $summary.schema_version -cne "spell.v07.final-qualification/1" -or
    $summary.product_version -cne "0.7.0" -or
  ...
  Assert-ExactProperties $summary.accepted_v06_release @(
    "archive_sha256", "sidecar_sha256", "tag_object", "tag_archive_claim"
  ) "final qualification accepted v0.6 release"
  if (
    [string]$summary.accepted_v06_release.archive_sha256 -cne $v06ArchiveSha256 -or
    [string]$summary.accepted_v06_release.sidecar_sha256 -cne $v06SidecarSha256 -or
    [string]$summary.accepted_v06_release.tag_object -cne "b6dc64dc8fb6cfe9845f454904a078ec6f3c0919" -or
    [string]$summary.accepted_v06_release.tag_archive_claim -cne "Final archive SHA-256: $v06ArchiveSha256"
  ) { throw "final qualification accepted v0.6 release binding differs" }
  ...
  foreach ($id in $finalSuiteIds) {
    ...
    if (-not $finalFull.Equals([IO.Path]::GetFullPath((Join-Path $root "artifacts/v0.7/final")))) {
...
if ($ValidateOnly) {
  $validated = Assert-FinalPublication (Join-Path $root "artifacts/v0.7/final")
  [ordered]@{
    schema_version = "spell.v07.final-qualification-validation/1"
...
if ($LocalConfirmation -cne "LOCAL_SYNTHETIC_NON_CUI_ONLY") {
  throw "v0.7 final qualification requires the exact local synthetic non-CUI confirmation"
}

$tempRoot = Join-Path $env:TEMP "spell-v07-final-$runId"
...
try {
  $status = @(& git status --porcelain=v1 --untracked-files=all -- . `
    ":(exclude)artifacts/v0.7/final/**" ":(exclude)artifacts/v0.7/.qualification/**")
  ...
  Assert-V06ArtifactsUnchanged $resolvedSource
  if ($resolvedSource -cne $headCommit) {
    ...
    foreach ($path in $closeout) {
      if (
        -not ([string]$path).StartsWith("artifacts/v0.7/", [StringComparison]::Ordinal) -or
        ([string]$path).Contains("/.qualification/") -or
        ([string]$path).EndsWith(".tar.gz") -or ([string]$path).EndsWith(".tar.gz.sha256")
      ) { throw "release HEAD differs from frozen source outside canonical v0.7 evidence: $path" }
    }
  }
  ...
  foreach ($required in @(
    ...
    "artifacts/v0.7/work-package/qualification.json",
...
print(source_fingerprint_v07(root))
print(product_package_sha256_v07(root))
'@
  ...
  $candidateValidator = Join-Path $root "scripts/validate_candidate_evidence_v07.py"
  ...
  $candidateEvidencePath = Join-Path $sourceRoot "artifacts/v0.7/work-package/qualification.json"
  $candidateEvidence = Get-Content -LiteralPath $candidateEvidencePath -Raw | ConvertFrom-Json
  $schemaRelative = "artifacts/v0.7/work-package/schema.json"
  ...
  $qualificationDockerfile = Join-Path $sourceRoot "scripts/qualification-v07.Dockerfile"
  & $script:DockerExe build --pull=false --provenance=false `
    --label "com.docker.compose.project=$project" `
    --label "org.openbexi.spell.v07.implementation.commit=$candidateCommit" `
    --label "org.openbexi.spell.v07.source.commit=$resolvedSource" `
    --label "org.openbexi.spell.v07.source.tree=$resolvedTree" `
...
'@
  ...
  $databasePassword = "v07-final-pg-$runId"
...
'@
  ...
  $v06InheritedSource = Join-Path $root $v06InheritedRelative
  $v06InheritedItem = if (Test-Path -LiteralPath $v06InheritedSource -PathType Container) {
    Get-Item -LiteralPath $v06InheritedSource -Force
  } else { $null }
  if (
    $null -eq $v06InheritedItem -or
    ($v06InheritedItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -or
    @(Get-ChildItem -LiteralPath $v06InheritedSource -Recurse -Force | Where-Object {
      $_.Attributes -band [IO.FileAttributes]::ReparsePoint
    }).Count -ne 0
  ) { throw "accepted-v0.6 inherited regression input is missing or unsafe" }
  $v06InheritedDestination = Join-Path $v06Worktree $v06InheritedRelative
  ...
  try {
    ...
    $buildDocument = [ordered]@{
      schema_version = "spell.v07.frontend-build/1"
    ...
    try {
      ...
      $browserDbPassword = "v07-final-browser-db-$runId"
      $browserJwtSecret = "v07-final-browser-jwt-$runId-material"
      ...
      & $composeExe -p $browserProject --profile driver -f compose.yaml up -d --build --wait
      ...
      $tokenLines = @(& $composeExe -p $browserProject --profile driver -f compose.yaml `
        exec -T backend python /app/scripts/issue_dev_token.py `
        --subject v07-final-browser --role admin --lifetime 900)
      ...
      $seedLines = @(& $composeExe -p $browserProject --profile driver -f compose.yaml `
      ...
      $realSpecs = @("e2e/integration.spec.ts", "e2e/telemetry-observation-real.spec.ts")
        ...
        try {
          ...
          try {
            $down = Invoke-ComposeCleanup @(
              "-p", $browserProject, "--profile", "driver", "-f", "compose.yaml", "down",
...
try {
  ...
  $suites.frontend_unit = [ordered]@{
    kind = "vitest-json"; capture = "artifacts/v0.7/final/tests/frontend-unit.json"
  ...
  $suites.frontend_build = [ordered]@{
    kind = "frontend-build"; capture = "artifacts/v0.7/final/tests/frontend-build.json"
  ...
  $workPackageRelative = "artifacts/v0.7/work-package/qualification.json"
  $workPackageValidatorRelative = "scripts/validate_candidate_evidence_v07.py"
  $gateScopeRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/scopes/v0.7-gate-0b.json"
  )
  $gateDocumentRelative = "SPELL_v0.7_Gate_0B.md"
  $gateValidatorRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/validate_v07_gate_0b.py"
  )
  ...
  $summary = [ordered]@{
    schema_version = "spell.v07.final-qualification/1"
    product_version = "0.7.0"
    ...
    accepted_v06_release = [ordered]@{
      archive_sha256 = $v06ArchiveSha256
      sidecar_sha256 = $v06SidecarSha256
      tag_object = "b6dc64dc8fb6cfe9845f454904a078ec6f3c0919"
      tag_archive_claim = "Final archive SHA-256: $v06ArchiveSha256"
    }
    teardown = [ordered]@{
  ...
  if (Test-Path -LiteralPath $canonicalFinal) {
    if (-not $Replace) { throw "final v0.7 qualification already exists; pass -Replace to republish" }
  ...
  [ordered]@{
    schema_version = "spell.v07.final-qualification-publication/1"
