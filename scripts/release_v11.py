"""Shared fail-closed release controls for SPELL v0.11."""
...
POLICY_PATH = Path("contracts/v11/release_policy.json")
QUALIFICATION_PATH = Path("artifacts/v0.11/release-qualification.json")
RELEASE_MANIFEST_PATH = Path("artifacts/v0.11/release-manifest.json")
PACKAGE_PATH = Path("artifacts/v0.11/openbexi-spell-v0.11.0.tar.gz")
SIDECAR_PATH = Path(str(PACKAGE_PATH) + ".sha256")
PRODUCT_VERSION = "0.11.0"
RELEASE_TAG = "v0.11.0"
ACCEPTED_PREDECESSOR_TAG = "v0.10.0"
ACCEPTED_PREDECESSOR_COMMIT = "c33d1893d90f9d42c36eedd19cb83f079bf39a9f"

EXPECTED_GATES = {
    "backend_v10": {"tests": 443, "skipped": 1},
    "backend_v11": {"tests": 197, "skipped": 0},
    "backend_full": {"tests": 1563, "skipped": 19},
    "backend_postgresql": {"tests": 16, "skipped": 0},
    "backend_compose": {"tests": 3, "skipped": 0},
    "frontend_unit": {"tests": 112, "skipped": 0},
    "audit_tool": {"tests": 3, "skipped": 0},
...
GATE_COMMANDS = {
    ...
    "backend_v11": "python -m pytest <eight v0.11 backend test modules> --junitxml=<capture>",
    ...
    "audit_tool": "python -m pytest scripts/tests/test_spell_auditor_tool.py --junitxml=<capture>",
    "release_tooling": "python -m pytest scripts/tests/test_release_v11.py --junitxml=<capture>",
    "documentation": "python -m pytest <documentation validation tests> --junitxml=<capture>",
}

ALLOWED_V11_ARTIFACTS = {
    "artifacts/v0.11/browser-e2e/results/.last-run.json",
    QUALIFICATION_PATH.as_posix(),
}
FORBIDDEN_SOURCE_MARKERS = (
    re.compile(rb"v0\.12", re.IGNORECASE),
    re.compile(rb"0\.12\.0"),
    re.compile(rb"contracts[/\\]v12", re.IGNORECASE),
    re.compile(rb"ir_v12", re.IGNORECASE),
    re.compile(rb"telecommand_v12", re.IGNORECASE),
)
SECRET_PATH_NAMES = {
    ".env",
    "credentials.json",
...
class ReleaseV11Error(ValueError):
    """Raised when a v0.11 release invariant is not satisfied."""
...
def assert_clean_worktree(root: Path) -> None:
    ...
    if status:
        raise ReleaseV11Error("v0.11 release operation requires a clean worktree")
...
def _assert_ancestor(root: Path, ancestor: str, descendant: str = "HEAD") -> None:
    ...
    if result.returncode != 0:
        raise ReleaseV11Error(f"required commit is not an ancestor: {ancestor}")
...
def load_policy(root: Path = ROOT) -> dict:
    ...
    try:
        ...
        raise ReleaseV11Error("v0.11 release policy is missing or invalid") from exc
    expected_keys = {
        "schema_version",
        "product_version",
        "release_tag",
    ...
    if set(policy) != expected_keys:
        raise ReleaseV11Error("v0.11 release policy fields differ from the contract")
    if (
        policy["schema_version"] != "spell.v11.release-policy/1"
        or policy["product_version"] != PRODUCT_VERSION
        or policy["release_tag"] != RELEASE_TAG
        or policy["accepted_predecessor_tag"] != ACCEPTED_PREDECESSOR_TAG
        or policy["accepted_predecessor_commit"] != ACCEPTED_PREDECESSOR_COMMIT
        or policy["scope"] != "simulator-telecommand-semantics"
        or policy["operational_authorization"] is not False
    ):
        raise ReleaseV11Error("v0.11 release policy identity differs")
    if not re.fullmatch(r"[0-9a-f]{40}", policy["candidate_commit"]):
        raise ReleaseV11Error("v0.11 candidate commit is invalid")
    if not isinstance(policy["reference_inputs"], list) or not isinstance(
        policy["release_evidence"], list
    ):
        raise ReleaseV11Error("v0.11 release policy inventories are invalid")
    package_policy = policy["package_policy"]
    if (
        ...
        or package_policy["archive_path"] != PACKAGE_PATH.as_posix()
        or any(
            not isinstance(package_policy[key], list)
            for key in (
                "exclude_prefixes",
                "exclude_suffixes",
                "forbidden_release_prefixes",
        ...
        raise ReleaseV11Error("v0.11 package policy differs")
    return policy
...
def _validate_hash_inventory(root: Path, rows: list[dict], label: str) -> dict[str, str]:
    ...
    for row in rows:
        if set(row) != {"path", "sha256", "role"}:
            raise ReleaseV11Error(f"{label} inventory row has unexpected fields")
        relative = row["path"]
        path = root / relative
        if not path.is_file() or path.is_symlink():
            raise ReleaseV11Error(f"required {label} file is missing or unsafe: {relative}")
        digest = sha256_file(path)
        if digest != row["sha256"]:
            raise ReleaseV11Error(f"required {label} hash differs: {relative}")
        if relative in result:
            raise ReleaseV11Error(f"duplicate {label} path: {relative}")
...
def _validate_predecessor(root: Path, policy: dict) -> None:
    tag = policy["accepted_predecessor_tag"]
    if str(_git(root, "cat-file", "-t", f"refs/tags/{tag}")) != "tag":
        raise ReleaseV11Error("v0.11 predecessor must be an annotated tag")
    target = str(_git(root, "rev-parse", f"refs/tags/{tag}^{{commit}}"))
    if target != policy["accepted_predecessor_commit"]:
        raise ReleaseV11Error("v0.11 predecessor tag target differs")
    _assert_ancestor(root, target, policy["candidate_commit"])
...
def _validate_product_identity(root: Path, tracked: list[str]) -> None:
    ...
    if not required_paths.issubset(tracked):
        raise ReleaseV11Error("required v0.11 product paths are missing")
    forbidden_paths = [
        path
        for path in tracked
        if path.startswith(("contracts/v12/", "artifacts/v0.12/"))
        or re.search(r"(^|/)[^/]*_v12(?:\.|/|$)", path, re.IGNORECASE)
    ]
    if forbidden_paths:
        raise ReleaseV11Error(f"v0.12 paths entered v0.11: {forbidden_paths}")

    identity_files = {
        "backend/version.py": 'PRODUCT_VERSION = "0.11.0"',
        "backend/__init__.py": '__version__ = "0.11.0"',
        "frontend/package.json": '"version": "0.11.0"',
        "frontend/package-lock.json": '"version": "0.11.0"',
    ...
    for relative, marker in identity_files.items():
        if marker not in (root / relative).read_text(encoding="utf-8"):
            raise ReleaseV11Error(f"v0.11 identity marker is missing: {relative}")

    procedures = sorted(path.name for path in (root / "procedures").glob("*.spell.py"))
    if procedures != ["language_reference_244.spell.py"]:
        raise ReleaseV11Error(f"v0.11 procedure catalog differs: {procedures}")
    ...
    for relative in sorted(set(source_paths)):
        ...
        if any(pattern.search(data) for pattern in FORBIDDEN_SOURCE_MARKERS):
            raise ReleaseV11Error(f"v0.12 source marker entered v0.11: {relative}")
...
def _validate_reference_evidence(root: Path) -> None:
    ...
    if example_matrix.get("example_count") != 195 or len(example_matrix.get("examples", [])) != 195:
        raise ReleaseV11Error("inherited v0.10 example matrix is not exactly 195 entries")
    if (
        ...
        raise ReleaseV11Error("inherited v0.10 variant matrix totals differ")
    if evidence.get("summary") != {
        ...
        raise ReleaseV11Error("inherited v0.10 example qualification summary differs")
    if evidence.get("variant_summary") != {
        ...
        raise ReleaseV11Error("inherited v0.10 variant qualification summary differs")
    results = evidence.get("results")
    if not isinstance(results, list) or [row.get("example_number") for row in results] != list(range(1, 196)):
        raise ReleaseV11Error("inherited v0.10 qualification identities are not exactly 1..195")
    if any(row.get("status") != "PASS" for row in results):
        raise ReleaseV11Error("inherited v0.10 qualification contains a non-PASS result")

    catalog = json.loads(
        (root / "contracts/v11/telecommand_catalog.json").read_text(encoding="utf-8")
    )
    commands = catalog.get("commands")
    sequences = catalog.get("sequences")
    if (
        catalog.get("schema_version") != "spell.v11.telecommand-catalog/1"
        or catalog.get("simulator_only") is not True
        or catalog.get("live_dispatch") is not False
        or not isinstance(commands, list)
        or len(commands) != 7
        or len({row.get("name") for row in commands}) != 7
        or not isinstance(sequences, list)
        or len(sequences) != 2
    ):
        raise ReleaseV11Error("v0.11 telecommand catalog contract differs")
    execution = json.loads(
        (root / "contracts/v11/telecommand_execution.json").read_text(
            encoding="utf-8"
        )
    )
    rules = execution.get("rules")
    if (
        execution.get("schema_version") != "spell.v11.telecommand-execution/1"
        or execution.get("simulator_only") is not True
        or execution.get("live_dispatch") is not False
        or not isinstance(rules, dict)
        or rules.get("automatic_resend") is not False
        or rules.get("reconciliation_dispatches_commands") is not False
        or rules.get("critical_commands_require_explicit_preflight_confirmation")
        is not True
    ):
        raise ReleaseV11Error("v0.11 telecommand execution contract differs")

    last_run = json.loads(
        (root / "artifacts/v0.11/browser-e2e/results/.last-run.json").read_text(
            encoding="utf-8"
        )
    )
    if last_run != {"status": "passed", "failedTests": []}:
        raise ReleaseV11Error("v0.11 browser run did not finish with PASS")
    browser_json = sorted(
        (root / "artifacts/v0.11/browser-e2e/results").glob(
            "*/language-reference-example-195-*-evidence.json"
        )
    )
    if len(browser_json) != 2:
        raise ReleaseV11Error("v0.11 requires exactly two canonical browser events")
    for path in browser_json:
        ...
        if (
            ...
            raise ReleaseV11Error(f"browser evidence differs: {path.relative_to(root)}")
...
def validate_repository(root: Path = ROOT) -> dict[str, object]:
    ...
    _validate_predecessor(root, policy)
    ...
    return {
        ...
        "accepted_predecessor_tag": policy["accepted_predecessor_tag"],
        "accepted_predecessor_commit": policy["accepted_predecessor_commit"],
...
def package_files(root: Path = ROOT) -> list[Path]:
    ...
    for relative in _tracked_paths(root):
        ...
        if normalized.startswith(forbidden):
            raise ReleaseV11Error(f"forbidden release path is tracked: {normalized}")
        ...
        if normalized.startswith("artifacts/"):
            if normalized.startswith("artifacts/v0.11/browser-e2e/results/"):
                if not normalized.endswith(".json"):
                    continue
            elif normalized not in ALLOWED_V11_ARTIFACTS:
                continue
        path = root / normalized
        if not path.is_file() or _is_reparse_or_link(path):
            raise ReleaseV11Error(f"package input is missing or unsafe: {normalized}")
        lowered_name = path.name.casefold()
        if lowered_name in SECRET_PATH_NAMES or path.suffix.casefold() in SECRET_SUFFIXES:
            raise ReleaseV11Error(f"secret-bearing path cannot enter package: {normalized}")
        paths.append(path)
...
def parse_junit(path: Path) -> dict[str, int | float]:
    try:
        ...
        raise ReleaseV11Error(f"JUnit capture is missing or invalid: {path}") from exc
    cases = list(root.iter("testcase"))
    if not cases:
        raise ReleaseV11Error(f"JUnit capture contains no tests: {path}")
...
def validate_gate_results(gates: dict[str, dict]) -> None:
    if set(gates) != set(EXPECTED_GATES):
        raise ReleaseV11Error("qualification gate inventory differs")
    for gate_id, expected in EXPECTED_GATES.items():
        ...
        if set(result) != {
            ...
            raise ReleaseV11Error(f"qualification gate fields differ: {gate_id}")
        if result["failures"] != 0 or result["errors"] != 0:
            raise ReleaseV11Error(f"qualification gate failed: {gate_id}")
        if result["skipped"] != expected["skipped"]:
            raise ReleaseV11Error(f"qualification skip count differs: {gate_id}")
        if "tests" in expected and result["tests"] != expected["tests"]:
            raise ReleaseV11Error(f"qualification test count differs: {gate_id}")
        if result["tests"] < expected.get("minimum_tests", 0):
            raise ReleaseV11Error(f"qualification test count is too small: {gate_id}")
        if result["passed"] + result["failures"] + result["errors"] + result["skipped"] != result["tests"]:
            raise ReleaseV11Error(f"qualification totals are inconsistent: {gate_id}")
    if gates["backend_postgresql"]["passed"] != 16 or gates["backend_compose"]["passed"] != 3:
        raise ReleaseV11Error("environment-selected backend skips are not fully resolved")
