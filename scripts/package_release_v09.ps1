...
$release = Join-Path $artifactRoot "openbexi-spell-v0.9.0.tar.gz"
...
$exportRoot = Join-Path $env:TEMP "spell-v09-package-exports-$runId"
$exportA = Join-Path $exportRoot "source-a"
$exportB = Join-Path $exportRoot "source-b"
$exportFirst = Join-Path $exportA "artifacts/v0.9/.qualification/package-a.tar.gz"
$exportSecond = Join-Path $exportB "artifacts/v0.9/.qualification/package-b.tar.gz"
$stagedRelease = Join-Path $artifactRoot ".openbexi-spell-v0.9.0.tar.gz.staging-$runId"
$stagedSidecar = "$stagedRelease.sha256"
$backupRelease = Join-Path $artifactRoot ".openbexi-spell-v0.9.0.tar.gz.backup-$runId"
...
function Assert-V08ArtifactsUnchanged {
  $tree = @(& git -C $root rev-parse "HEAD:artifacts/v0.8") -join "`n"
  if (
    $LASTEXITCODE -ne 0 -or
    $tree.Trim() -cne "899dd791fbfd5aa8720c3ce836d5cc2208bac6b9"
  ) { throw "accepted v0.8 artifact tree differs" }
  & git -C $root diff --quiet v0.8.0 HEAD -- artifacts/v0.8
  if ($LASTEXITCODE -ne 0) { throw "accepted v0.8 artifacts differ from v0.8.0" }
  & git -C $root diff --quiet -- artifacts/v0.8
  if ($LASTEXITCODE -ne 0) { throw "accepted v0.8 artifacts have a working-tree change" }
  $status = @(& git -C $root status --porcelain --untracked-files=all -- artifacts/v0.8)
  if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
    throw "accepted v0.8 artifacts have an untracked change"
  }
...
function Invoke-PackageBuild([string]$BuildRoot, [string]$OutputPath) {
  $lines = @(& $PythonExe -I (Join-Path $BuildRoot "scripts/build_reproducible_v09.py") `
    --root $BuildRoot --output $OutputPath)
  if ($LASTEXITCODE -ne 0) { throw "v0.9 deterministic package build failed" }
  $jsonLine = $lines | Where-Object { $_ -cmatch '^\{.+\}$' } | Select-Object -Last 1
  if (-not $jsonLine) { throw "v0.9 deterministic package build emitted no JSON" }
  return $jsonLine | ConvertFrom-Json
}

function Publish-V09PackagePair {
  [CmdletBinding()]
  param(
  ...
  foreach ($path in @($StagedRelease, $StagedSidecar, $BackupRelease, $BackupSidecar)) {
    if (Test-Path -LiteralPath $path) {
      ...
      throw "v0.9 package transaction path already exists: $path"
    }
  }

  $expectedSidecar = "$ExpectedHash  openbexi-spell-v0.9.0.tar.gz`n"
  ...
  try {
    ...
    if (
      ...
      [IO.File]::ReadAllText($StagedSidecar, [Text.Encoding]::ASCII) -cne
        $expectedSidecar
    ) { throw "staged v0.9 package pair differs" }
    ...
    try {
      ...
      if ($TestOnlyFault -cne "None") {
        throw "injected v0.9 package publication failure"
      }
      ...
      if (
        ...
        [IO.File]::ReadAllText($Sidecar, [Text.Encoding]::ASCII) -cne
          $expectedSidecar
      ) { throw "published v0.9 package pair failed final verification" }
    }
    catch {
      ...
      if ($rollbackErrors.Count -ne 0) {
        throw "v0.9 package publication failed and rollback was incomplete; recovery files retained"
      }
...
) { throw "v0.9 package scratch path escapes its owned artifact directory" }

$scratchOwned = $false
$worktreeAOwned = $false
$worktreeBOwned = $false
try {
  Assert-V08ArtifactsUnchanged
  ...
  if ($LASTEXITCODE -ne 0 -or $releaseCommit -cnotmatch '^[0-9a-f]{40}$') {
    throw "cannot freeze the explicit v0.9 package source commit"
  }
  foreach ($path in @($artifactRoot, $qualificationRoot)) {
    ...
    if (-not $item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
      throw "refusing unsafe v0.9 package scratch parent: $path"
    }
  }
  if (Test-Path -LiteralPath $scratchRoot) {
    throw "v0.9 package scratch path already exists"
  }
  ...
  ) { throw "v0.9 package scratch directory is unsafe" }
  ...
  if (-not $exportFull.StartsWith($tempPrefix, [StringComparison]::OrdinalIgnoreCase)) {
    throw "v0.9 package export path escapes the temporary root"
  }
  if (Test-Path -LiteralPath $exportRoot) { throw "v0.9 package export path already exists" }
  ...
  foreach ($export in @($exportA, $exportB)) {
    ...
    & (Join-Path $PSScriptRoot "assert_accepted_v08_release_v09.ps1") -Root $export | Out-Null
  }
  ...
  if (
    ...
    $a.accepted_v08_release.archive_sha256 -cne "87cd104d3c7a92764b3b848a0424f8fddb8522e0b7d8ae6e93310b2ce7e42deb" -or
    $a.accepted_v08_release.sidecar_sha256 -cne "1527927c7f767a460de3bcd4df127db1be38b58084f2ec73f164389b9660c817" -or
    $a.accepted_v08_release.tag_object -cne "0dcf4f539fd1a9036fe4db4bc159cde04c35cfae" -or
    $a.accepted_v08_release.tag_archive_claim -cne "Final archive SHA-256: 87cd104d3c7a92764b3b848a0424f8fddb8522e0b7d8ae6e93310b2ce7e42deb" -or
    ($a.accepted_v08_release | ConvertTo-Json -Compress) -cne
      ($b.accepted_v08_release | ConvertTo-Json -Compress)
  ) { throw "independent v0.9 package processes produced different results" }

  Copy-Item -LiteralPath $exportFirst -Destination $first
  Copy-Item -LiteralPath $exportSecond -Destination $second

  Publish-V09PackagePair -SourceRelease $first -StagedRelease $stagedRelease `
  ...
  [ordered]@{
    schema_version = "spell.v09.package-publication/1"
    ...
    output = "artifacts/v0.9/openbexi-spell-v0.9.0.tar.gz"
  ...
  if ($scratchOwned -and (Test-Path -LiteralPath $scratchRoot)) {
    ...
    ) { throw "refusing unsafe v0.9 package scratch cleanup" }
    $unsafeChild = Get-ChildItem -Force -Recurse -LiteralPath $scratchRoot |
      Where-Object { $_.Attributes -band [IO.FileAttributes]::ReparsePoint } |
      Select-Object -First 1
    if ($null -ne $unsafeChild) {
      throw "refusing v0.9 package scratch cleanup containing a reparse point"
    }
