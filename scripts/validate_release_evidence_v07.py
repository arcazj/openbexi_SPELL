#!/usr/bin/env python3
"""Validate the bounded SPELL v0.7.0 final release evidence and tag."""
...
from scripts.source_fingerprint_v07 import (  # noqa: E402
    is_link_or_reparse_v07,
    path_has_link_or_reparse_v07,
)
...
DEFAULT_EVIDENCE_ROOT = ROOT / "artifacts" / "v0.7"
...
# The Gate 0A and accepted v0.6 identities are immutable governance inputs.
...
V06_COMMIT = V06_RELEASE_COMMIT
V06_ARTIFACT_TREE = "18cb672a45c538539f78278962ea5822b9f52441"
V06_EXTERNAL_RELEASE_BINDING = {
...
RELEASE_TAG = "v0.7.0"
RELEASE_TAG_REF = "refs/tags/v0.7.0"
FORBIDDEN_ALIAS_REF = "refs/tags/v0.7"
RELEASE_TAG_TITLE = "SPELL v0.7.0"
RELEASE_PACKAGE_PATH = "artifacts/v0.7/openbexi-spell-v0.7.0.tar.gz"
...
WORK_PACKAGE_PATH = "artifacts/v0.7/work-package/qualification.json"
WORK_PACKAGE_VALIDATOR = "scripts/validate_candidate_evidence_v07.py"
GATE_0B_SCOPE = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/"
    "scopes/v0.7-gate-0b.json"
)
GATE_0B_DOCUMENT = "SPELL_v0.7_Gate_0B.md"
GATE_0B_VALIDATOR = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/"
    "validate_v07_gate_0b.py"
)
GATE_0B_MARKER = (
...
FINAL_QUALIFICATION_PATH = "artifacts/v0.7/final/qualification.json"
CANONICAL_EVIDENCE_PATHS = frozenset(
    {
        WORK_PACKAGE_PATH,
        "artifacts/v0.7/work-package/schema.json",
        "artifacts/v0.7/work-package/tests/backend-sqlite.xml",
        "artifacts/v0.7/work-package/tests/backend-postgresql.xml",
        "artifacts/v0.7/work-package/tests/backend-docker-host.xml",
        "artifacts/v0.7/work-package/tests/backend-v07-soak.json",
        "artifacts/v0.7/work-package/tests/driver-host.xml",
        "artifacts/v0.7/work-package/tests/tooling.xml",
        "artifacts/v0.7/work-package/tests/frontend-vitest.xml",
        "artifacts/v0.7/work-package/tests/frontend-build.json",
        "artifacts/v0.7/work-package/tests/frontend-playwright-mocked.xml",
        "artifacts/v0.7/work-package/tests/frontend-playwright-live.xml",
        "artifacts/v0.7/work-package/browser/desktop-as-run-report.png",
        "artifacts/v0.7/work-package/browser/desktop-v03-validation.png",
        "artifacts/v0.7/work-package/browser/mobile-durable-prompt.png",
        "artifacts/v0.7/work-package/browser/telemetry-observation-desktop.png",
        "artifacts/v0.7/work-package/browser/telemetry-observation-mobile.png",
        "artifacts/v0.7/work-package/browser/telemetry-observation-desktop.json",
        "artifacts/v0.7/work-package/browser/telemetry-observation-mobile.json",
        FINAL_QUALIFICATION_PATH,
        "artifacts/v0.7/final/tests/backend-sqlite.xml",
        "artifacts/v0.7/final/tests/backend-postgresql.xml",
        "artifacts/v0.7/final/tests/backend-docker-host.xml",
        "artifacts/v0.7/final/tests/driver-host.xml",
        "artifacts/v0.7/final/tests/tooling.xml",
        "artifacts/v0.7/final/tests/frontend-unit.json",
        "artifacts/v0.7/final/tests/frontend-build.json",
        "artifacts/v0.7/final/tests/browser-mocked.xml",
        "artifacts/v0.7/final/tests/browser-real.xml",
        "artifacts/v0.7/sbom/backend.cdx.json",
        "artifacts/v0.7/sbom/driver.cdx.json",
        "artifacts/v0.7/sbom/frontend.cdx.json",
        "artifacts/v0.7/sbom/proxy.cdx.json",
        "artifacts/v0.7/sbom/SHA256SUMS",
        "artifacts/v0.7/supply-chain.json",
    }
)
JUNIT_SUITE_IDS = (
    "backend_sqlite",
...
SBOM_SUBJECTS = {
    "backend.cdx.json": "openbexi_spell-backend:0.7",
    "driver.cdx.json": "openbexi_spell-driver:0.7",
    "frontend.cdx.json": "openbexi_spell-frontend:0.7",
    "proxy.cdx.json": "openbexi_spell-proxy:0.7",
}
...
class ReleaseEvidenceError(ValueError):
    """Raised when final v0.7 evidence cannot be accepted."""
...
@dataclass(frozen=True)
class ReleaseEvidenceValidationV07:
...
def read_strict_json(
    ...
    *,
    ...
    _require(
        path.is_file() and not is_link_or_reparse_v07(path),
...
def validate_frontend_build(
    ...
    _require(relative == "artifacts/v0.7/final/tests/frontend-build.json", "frontend build capture path differs")
    ...
    _require(capture.get("schema_version") == "spell.v07.frontend-build/1", "frontend build schema differs")
...
def validate_version_metadata(root: Path) -> None:
    ...
    _require(
        pyproject_path.is_file() and not is_link_or_reparse_v07(pyproject_path),
    ...
    _require(supply.get("release_artifact_root") == "artifacts/v0.7", "v0.7 artifact root differs")
    ...
    for relative in ("driver_host/Dockerfile", "driver_host/pki.Dockerfile"):
        ...
        _require("ARG SPELL_PACKAGE_VERSION=0.7.0" not in lines, f"{relative} incorrectly advances driver identity")


def _allowed_candidate_path(path: str) -> bool:
    return path in ALLOWED_CANDIDATE_DELTA or path.startswith("artifacts/v0.7/work-package/")
...
def validate_release_closeout_paths(
    ...
    *,
    ...
    for path in paths:
        _require(
            path.startswith("artifacts/v0.7/"),
            f"release closeout changes source outside canonical v0.7 evidence: {path}",
        )
        _require(
...
def validate_committed_release_package(root: Path, release_head: str) -> str:
    package_raw = _git_blob_bytes(
        root, release_head, RELEASE_PACKAGE_PATH, "v0.7.0 release package"
    )
    sidecar_raw = _git_blob_bytes(
        root, release_head, RELEASE_PACKAGE_SIDECAR_PATH, "v0.7.0 package sidecar"
    )
    package_path = _regular_relative(
        root, RELEASE_PACKAGE_PATH, "v0.7.0 release package"
    )
    sidecar_path = _regular_relative(
        root, RELEASE_PACKAGE_SIDECAR_PATH, "v0.7.0 package sidecar"
    )
    _require(
        package_path.read_bytes() == package_raw,
        "v0.7.0 release package differs from the release commit",
    )
    _require(
        sidecar_path.read_bytes() == sidecar_raw,
        "v0.7.0 package sidecar differs from the release commit",
    )
    archive_sha = sha256_bytes(package_raw)
    try:
        sidecar = sidecar_raw.decode("ascii")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.7.0 package sidecar is not ASCII") from exc
    _require(
        sidecar == f"{archive_sha}  {package_path.name}\n",
        "v0.7.0 package sidecar differs",
    )
    return archive_sha
...
def validate_v06_unchanged(root: Path, inherited: Mapping[str, Any]) -> None:
    _exact_keys(
        inherited,
        (
            "classification", "supports", "direct_v07_proof", "accepted_tag_ref",
    ...
    _require(inherited.get("classification") == "ACCEPTED_RELEASE_BASELINE_NOT_DIRECT_V07_PROOF", "v0.6 evidence classification differs")
    _require(inherited.get("supports") == ["release-lineage", "compatibility", "toolchain"], "v0.6 support scope differs")
    _require(inherited.get("direct_v07_proof") is False, "v0.6 evidence is misclassified as direct v0.7 proof")
    expected = {
        "accepted_tag_ref": V06_TAG_REF,
        "accepted_tag_object": V06_TAG_OBJECT,
        "accepted_commit": V06_COMMIT,
        "accepted_artifact_tree": V06_ARTIFACT_TREE,
        "accepted_archive_path": V06_ARCHIVE_RELATIVE,
        "accepted_archive_sha256": V06_ARCHIVE_SHA256,
        "accepted_sidecar_path": V06_SIDECAR_RELATIVE,
        "accepted_sidecar_sha256": V06_SIDECAR_SHA256,
        "accepted_tag_archive_claim": V06_TAG_ARCHIVE_CLAIM,
    }
    for key, value in expected.items():
        _require(inherited.get(key) == value, f"inherited_v06.{key} differs")
    try:
        validate_accepted_v06_release(root)
    except ValueError as exc:
        raise ReleaseEvidenceError(str(exc)) from exc
    _require(_git_line(root, ["show-ref", "--verify", "--hash", V06_TAG_REF], V06_TAG_REF) == V06_TAG_OBJECT, "v0.6.0 tag moved")
    _require(_git_line(root, ["rev-parse", "HEAD:artifacts/v0.6"], "HEAD v0.6 artifact tree") == V06_ARTIFACT_TREE, "accepted v0.6 artifacts changed in release HEAD")
    _require(_git_line(root, ["rev-parse", f"{V06_COMMIT}:artifacts/v0.6"], "tagged v0.6 artifact tree") == V06_ARTIFACT_TREE, "accepted v0.6 tag artifact tree differs")
    diff = _run_git(root, ["diff", "--quiet", V06_COMMIT, "--", "artifacts/v0.6"], accepted=(0, 1))
    _require(diff.returncode == 0, "accepted v0.6 artifacts have a working-tree change")
    status = _run_git(
        root,
        ["status", "--porcelain", "-z", "--untracked-files=all", "--", "artifacts/v0.6"],
    ).stdout
    _require(status == b"", "accepted v0.6 artifacts have an untracked change")


def canonical_evidence_files(root: Path, evidence_root: Path) -> dict[str, Path]:
    _require(
        evidence_root.is_dir()
        and not path_has_link_or_reparse_v07(root, evidence_root),
        "v0.7 evidence root is missing or unsafe",
    )
    files: dict[str, Path] = {}
    for path in evidence_root.rglob("*"):
        relative_evidence = path.relative_to(evidence_root)
        relative_root = path.relative_to(root).as_posix()
        if path_has_link_or_reparse_v07(root, path):
            raise ReleaseEvidenceError(
                f"v0.7 evidence contains a link or reparse point: {relative_root}"
            )
        ...
        if path.name.startswith("."):
            raise ReleaseEvidenceError(f"v0.7 canonical evidence contains a temporary file: {relative_root}")
        if path.name.endswith((".tar.gz", ".tar.gz.sha256")):
            _require(
                relative_root in RELEASE_PACKAGE_PATHS,
                f"v0.7 evidence contains a noncanonical package output: {relative_root}",
            )
            continue
        _require(path.stat().st_size <= MAX_EVIDENCE_FILE_BYTES, f"v0.7 evidence file is oversized: {relative_root}")
        files[relative_root] = path
    _require(
        set(files) == CANONICAL_EVIDENCE_PATHS,
        "canonical v0.7 evidence file inventory differs",
    )
    return dict(sorted(files.items()))


def evidence_fingerprint_v07(files: Mapping[str, Path]) -> str:
...
def _scan_secret_bytes(payload: bytes, label: str) -> None:
    ...
    from scripts.build_reproducible_v07 import SECRET_PATTERNS
...
def validate_evidence_inventory(
    ...
    _require(set(declared) == set(files), "canonical v0.7 evidence file inventory differs")
    ...
    fingerprint = evidence_fingerprint_v07(files)
...
def validate_work_package(
    ...
    from scripts.validate_candidate_evidence_v07 import validate_candidate_evidence

    result = validate_candidate_evidence(root, root / "artifacts/v0.7/work-package")
...
def _validate_suite_manifest(
    ...
    expected_path = f"artifacts/v0.7/final/tests/{suite_id.replace('_', '-')}.{extension}"
    ...
    if suite_id in {"browser_mocked", "browser_real"}:
        ...
        _require(
            all(
                any(fragment in node for node in result.nodes)
    ...
    *,
    ...
    _exact_keys(
        ...
        (
            ...
            "toolchain", "suites", "teardown", "accepted_v06_release",
            "accepted_exceptions", "overall_pass",
        ),
        "final qualification summary",
    )
    _require(
        summary.get("schema_version") == "spell.v07.final-qualification/1",
    ...
    _require(
        _mapping(summary.get("accepted_v06_release"), "final qualification accepted_v06_release")
        == V06_EXTERNAL_RELEASE_BINDING,
        "final qualification accepted v0.6 release binding differs",
    )
    _require(
...
def validate_sbom(
    ...
    _require(sbom.get("directory") == "artifacts/v0.7/sbom", "SBOM directory differs")
    _require(sbom.get("checksum_manifest") == "artifacts/v0.7/sbom/SHA256SUMS", "SBOM checksum path differs")
    _require(sbom.get("source_fingerprint_sha256") == source_fingerprint, "SBOM declared source fingerprint differs")
    directory = root / "artifacts/v0.7/sbom"
    _require(
        directory.is_dir() and not path_has_link_or_reparse_v07(root, directory),
        "SBOM directory is missing or unsafe",
    )
    discovered = {
        path.name
        for path in directory.iterdir()
        if path.is_file() and not is_link_or_reparse_v07(path)
    }
...
def validate_supply_chain(
    ...
    _exact_keys(
        ...
        (
            ...
            "accepted_v06_artifacts_unchanged", "accepted_v06_release",
        ),
        "supply_chain",
    )
    expected_path = "artifacts/v0.7/supply-chain.json"
    _require(supply.get("capture") == expected_path, "supply-chain capture path differs")
    _require(supply.get("test_id") == "V07-SC-001" and supply.get("passed") is True, "supply-chain result did not pass")
    ...
    _require(supply.get("accepted_v06_artifacts_unchanged") is True, "supply-chain audit did not preserve v0.6 artifacts")
    _require(
        _mapping(supply.get("accepted_v06_release"), "supply_chain.accepted_v06_release")
        == V06_EXTERNAL_RELEASE_BINDING,
        "supply-chain accepted v0.6 release binding differs",
    )
    ...
    _require(capture.get("schema_version") == "spell.v07.supply-chain/1", "supply-chain schema differs")
    ...
    _require(capture.get("test_id") == "V07-SC-001" and capture.get("passed") is True, "supply-chain capture did not pass")
    _require(capture.get("source_fingerprint_sha256") == source_fingerprint, "supply-chain capture source differs")
    _require(capture.get("accepted_v06_artifacts_unchanged") is True, "supply-chain capture changed v0.6 artifacts")
    _require(
        _mapping(capture.get("accepted_v06_release"), "supply-chain capture accepted_v06_release")
        == V06_EXTERNAL_RELEASE_BINDING
        and capture.get("accepted_v06_release") == supply.get("accepted_v06_release"),
        "supply-chain capture accepted v0.6 release binding differs",
    )
...
def create_release_tag_message_v07(
...
def validate_release_tag_message_v07(raw: bytes, expected: str) -> None:
    try:
        message = raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.7.0 tag message is not strict UTF-8") from exc
    _require(
        message == expected,
        "v0.7.0 tag message differs from the exact ordered release record",
    )


def _release_tag_message_from_object(raw: bytes, release_head: str) -> bytes:
    headers, separator, message = raw.partition(b"\n\n")
    _require(bool(separator), "v0.7.0 tag lacks a message boundary")
    lowered = raw.lower()
    _require(
        not any(marker in lowered for marker in TAG_SIGNATURE_MARKERS),
        "v0.7.0 tag carries an unclaimed cryptographic signature",
    )
    ...
    _require(
        ...
        "v0.7.0 tag headers differ",
    )
    try:
        header_lines[3].decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ReleaseEvidenceError("v0.7.0 tagger header is not UTF-8") from exc
    return message
...
def validate_release_tag(
    ...
    *,
    ...
    _require(
        ...
        "forbidden lightweight or secondary v0.7 tag exists",
    )
    object_id = _git_line(root, ["show-ref", "--verify", "--hash", RELEASE_TAG_REF], RELEASE_TAG_REF)
    _require(_git_line(root, ["cat-file", "-t", object_id], "v0.7.0 tag type") == "tag", "v0.7.0 is not annotated")
    ...
    expected_message = create_release_tag_message_v07(
        policy,
    ...
    validate_release_tag_message_v07(message, expected_message)
    tagged = _git_line(root, ["rev-parse", f"{RELEASE_TAG_REF}^{{commit}}"], "v0.7.0 tagged commit")
    _require(tagged == release_head, "v0.7.0 tag does not target release HEAD")
...
def validate_release_manifest_shape(manifest: Mapping[str, Any]) -> None:
    _exact_keys(
        ...
        (
            ...
            "work_package", "gate_0b", "toolchain", "final_qualification", "final_suites", "inherited_v06",
...
def validate_release_evidence(
    ...
    *,
...
) -> ReleaseEvidenceValidationV07:
    source_root = root.resolve()
    canonical_root = (evidence_root or (source_root / "artifacts/v0.7")).resolve()
    _require(canonical_root == (source_root / "artifacts/v0.7").resolve(), "release evidence root must be canonical artifacts/v0.7")
    ...
    from scripts.source_fingerprint_v07 import source_fingerprint_v07
    from scripts.build_reproducible_v07 import product_package_sha256_v07

    source_fingerprint = source_fingerprint_v07(source_root)
    declared_source = manifest["qualified_source"]["source_fingerprint_sha256"]
    _require(source_fingerprint == declared_source, "current v0.7 source fingerprint differs")
    product_package = product_package_sha256_v07(source_root)
    ...
    validate_v06_unchanged(source_root, _mapping(manifest["inherited_v06"], "inherited_v06"))
    ...
    return ReleaseEvidenceValidationV07(
...
def validate_release_evidence_v07(
    root: Path,
    evidence_root: Path | None = None,
    *,
    require_tag: bool = False,
