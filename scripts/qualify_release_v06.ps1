...
$gate0aCommit = "f6eba8be0f7ca9e2f1d466aea66902152fb1bbc1"
...
$project = "spell-v06-final-$runId"
$qualificationTag = "openbexi-spell-v06-final-qualification:$runId"
$postgresTag = "openbexi-spell-v06-final-postgres:$runId"
...
function Assert-V05ArtifactsUnchanged {
  ...
  $expectedTree = "27542ec5db41ff29ba2af62824c8d39f442b95e8"
  $tree = @(& git rev-parse "${Commit}:artifacts/v0.5") -join "`n"
  Assert-NativeSuccess "cannot inspect accepted v0.5 artifact tree"
  if ($tree.Trim() -cne $expectedTree) { throw "accepted v0.5 artifacts changed in source commit" }
  & git diff --quiet v0.5.0 $Commit -- artifacts/v0.5
  Assert-NativeSuccess "accepted v0.5 artifact bytes differ from v0.5.0"
  & git diff --quiet -- artifacts/v0.5
  Assert-NativeSuccess "accepted v0.5 artifacts have a working-tree change"
  $status = @(& git status --porcelain --untracked-files=all -- artifacts/v0.5)
  Assert-NativeSuccess "cannot inspect accepted v0.5 artifact worktree"
  if ($status.Count -ne 0) { throw "accepted v0.5 artifacts have an untracked change" }
...
function New-JUnitSuiteDeclaration {
  ...
  return [ordered]@{
    kind = "junit"
    capture = "artifacts/v0.6/final/tests/$($Id.Replace('_', '-')).xml"
...
function Assert-FinalToolchain {
  ...
  $candidatePath = Join-Path $root "artifacts/v0.6/work-package/qualification.json"
...
function Assert-FinalPublication {
  ...
  Assert-ExactProperties $summary @(
    ...
    "toolchain", "suites", "teardown", "accepted_v05_release", "accepted_exceptions", "overall_pass"
  ) "final qualification summary"
  if (
    $summary.schema_version -cne "spell.v06.final-qualification/1" -or
    $summary.product_version -cne "0.6.0" -or
    $summary.scope_profile -cne "LOCAL_SYNTHETIC_NON_CUI_SIMULATOR" -or
    $summary.overall_pass -ne $true -or @($summary.accepted_exceptions).Count -ne 0
  ) { throw "final qualification summary header differs" }
  Assert-ExactProperties $summary.accepted_v05_release @(
    "archive_sha256", "sidecar_sha256", "tag_object", "tag_archive_claim"
  ) "final qualification accepted v0.5 release"
  if (
    [string]$summary.accepted_v05_release.archive_sha256 -cne $v05ArchiveSha256 -or
    [string]$summary.accepted_v05_release.sidecar_sha256 -cne $v05SidecarSha256 -or
    [string]$summary.accepted_v05_release.tag_object -cne "a1b277d74d2fb19062ca3e4388e9104d45c50ec4" -or
    [string]$summary.accepted_v05_release.tag_archive_claim -cne "Final archive SHA-256: $v05ArchiveSha256"
  ) { throw "final qualification accepted v0.5 release binding differs" }
  ...
  foreach ($id in $finalSuiteIds) {
    ...
    if (-not $finalFull.Equals([IO.Path]::GetFullPath((Join-Path $root "artifacts/v0.6/final")))) {
...
if ($ValidateOnly) {
  $validated = Assert-FinalPublication (Join-Path $root "artifacts/v0.6/final")
  [ordered]@{
    schema_version = "spell.v06.final-qualification-validation/1"
...
if ($LocalConfirmation -cne "LOCAL_SYNTHETIC_NON_CUI_ONLY") {
  throw "v0.6 final qualification requires the exact local synthetic non-CUI confirmation"
}

$tempRoot = Join-Path $env:TEMP "spell-v06-final-$runId"
...
try {
  $status = @(& git status --porcelain=v1 --untracked-files=all -- . `
    ":(exclude)artifacts/v0.6/final/**" ":(exclude)artifacts/v0.6/.qualification/**")
  ...
  Assert-V05ArtifactsUnchanged $resolvedSource
  if ($resolvedSource -cne $headCommit) {
    ...
    foreach ($path in $closeout) {
      if (
        -not ([string]$path).StartsWith("artifacts/v0.6/", [StringComparison]::Ordinal) -or
        ([string]$path).Contains("/.qualification/") -or
        ([string]$path).EndsWith(".tar.gz") -or ([string]$path).EndsWith(".tar.gz.sha256")
      ) { throw "release HEAD differs from frozen source outside canonical v0.6 evidence: $path" }
    }
  }
  ...
  foreach ($required in @(
    ...
    "artifacts/v0.6/work-package/qualification.json",
...
print(source_fingerprint_v06(root))
print(product_package_sha256_v06(root))
'@
  ...
  $candidateValidator = Join-Path $root "scripts/validate_candidate_evidence_v06.py"
  ...
  $candidateEvidencePath = Join-Path $sourceRoot "artifacts/v0.6/work-package/qualification.json"
  $candidateEvidence = Get-Content -LiteralPath $candidateEvidencePath -Raw | ConvertFrom-Json
  $schemaRelative = "artifacts/v0.6/work-package/schema.json"
  ...
  $qualificationDockerfile = Join-Path $sourceRoot "scripts/qualification-v06.Dockerfile"
  & $script:DockerExe build --pull=false --provenance=false `
    --label "com.docker.compose.project=$project" `
    --label "org.openbexi.spell.v06.implementation.commit=$candidateCommit" `
    --label "org.openbexi.spell.v06.source.commit=$resolvedSource" `
    --label "org.openbexi.spell.v06.source.tree=$resolvedTree" `
...
'@
  ...
  [string[]]$backendNodes = @(Get-CollectedNodes "backend/tests" "backend/tests/")
  [string[]]$driverNodes = @(Get-CollectedNodes "driver_host/tests" "driver_host/tests/")
  [string[]]$toolingNodes = @(Get-CollectedNodes "scripts/tests" "scripts/tests/")
  ...
  $databasePassword = "v06-final-pg-$runId"
...
'@
  ...
  try {
    ...
    try {
      & $lockedPython -I -c $pytestCode $sitePackages $sourceRoot `
        $runtimeNodes[0] $runtimeNodes[1] -q "--junitxml=$postgresRuntimeXml"
  ...
  $hostToolingNodes = @(
    $toolingNodes | Where-Object {
      $_ -in @(
        ...
        "scripts/tests/test_release_v06.py::test_v06_package_scans_candidate_manifest_and_tooling_structurally",
        "scripts/tests/test_release_v06.py::test_v06_package_rejects_duplicate_or_mislocated_evidence_canaries",
  ...
  if ($hostToolingNodes.Count -ne 14) { throw "host-only tooling node inventory differs" }
  ...
  foreach ($node in $rootExternalToolingNodes) {
    if ($node -notin $toolingNodes) { throw "external accepted-v0.5 tooling node is absent: $node" }
  }
  ...
  foreach ($node in @($hostToolingNodes) + @($rootExternalToolingNodes) + @($v05ExportToolingNodes)) {
  ...
  try {
    ...
    Assert-NativeSuccess "locked-host accepted-v0.5 external release tests failed"
  }
  finally { Pop-Location }
  ...
  $v05ReleaseDestination = Join-Path $v05Worktree "artifacts/v0.5"
  New-Item -ItemType Directory -Path $v05ReleaseDestination -Force | Out-Null
  foreach ($releaseName in @("openbexi-spell-v0.5.0.tar.gz", "openbexi-spell-v0.5.0.tar.gz.sha256")) {
    ...
    $releaseItem = if (Test-Path -LiteralPath $releaseSource -PathType Leaf) {
      Get-Item -LiteralPath $releaseSource -Force
    } else { $null }
    if ($null -eq $releaseItem -or ($releaseItem.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
    ...
    $releaseDestination = Join-Path $v05ReleaseDestination $releaseName
    ...
    if ((Get-LowerSha256 $releaseDestination) -cne $expectedReleaseSha) {
      throw "accepted-v0.5 staged release input differs: $releaseName"
    }
  }
  if ([IO.File]::ReadAllText((Join-Path $v05ReleaseDestination "openbexi-spell-v0.5.0.tar.gz.sha256"), [Text.Encoding]::ASCII) -cne $v05SidecarText) {
    throw "accepted-v0.5 staged release sidecar bytes differ"
  }

  $v05InheritedRelative = (
    "artifacts/v0.4/.qualification/runtime-captures/" +
    "46949e783e85b72e68f70d1607c6d44bb5234586c248888b2bd4a3d2cf06f17d/regression"
  )
  $v05InheritedSource = Join-Path $root $v05InheritedRelative
  $v05InheritedItem = if (Test-Path -LiteralPath $v05InheritedSource -PathType Container) {
    Get-Item -LiteralPath $v05InheritedSource -Force
  } else { $null }
  if (
    $null -eq $v05InheritedItem -or
    ($v05InheritedItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -or
    @(Get-ChildItem -LiteralPath $v05InheritedSource -Recurse -Force | Where-Object {
      $_.Attributes -band [IO.FileAttributes]::ReparsePoint
    }).Count -ne 0
  ) { throw "accepted-v0.5 inherited regression input is missing or unsafe" }
  $v05InheritedDestination = Join-Path $v05Worktree $v05InheritedRelative
  New-Item -ItemType Directory -Path (Split-Path -Parent $v05InheritedDestination) -Force | Out-Null
  Copy-Item -LiteralPath $v05InheritedSource -Destination $v05InheritedDestination -Recurse
  ...
  Merge-JUnit @($toolingBaseXml, $toolingHostXml, $toolingRootXml, $toolingV05Xml) $toolingXml
  ...
  try {
    ...
    $actualFrontendNodes = @($vitest.testResults | ForEach-Object {
      ...
      foreach ($assertion in @($_.assertionResults)) {
        "frontend/$($path.Substring($prefix.Length).Replace('\', '/'))::$([string]$assertion.fullName)"
      }
    })
    ...
    $savedBrowser = @{}
    foreach ($name in @(
      "CI", "SPELL_REAL_BACKEND", "SPELL_E2E_TOKEN", "SPELL_E2E_BASE_URL",
    ...
    try {
      ...
      $browserDbPassword = "v06-final-browser-db-$runId"
      $browserJwtSecret = "v06-final-browser-jwt-$runId-material"
      ...
      & $composeExe -p $browserProject -f compose.yaml up -d --build --wait backend proxy
      Assert-NativeSuccess "real-browser simulator stack failed to become healthy"
      $tokenLines = @(& $composeExe -p $browserProject -f compose.yaml run --rm --no-deps `
        -e SPELL_ALLOW_LOCAL_DEV_TOKEN=true backend python /app/scripts/issue_dev_token.py `
        --subject v06-final-browser --role admin --lifetime 900)
      Assert-NativeSuccess "cannot issue short-lived real-browser token"
      $browserToken = $tokenLines | Where-Object { $_ -cmatch '^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$' } | Select-Object -Last 1
      $tokenLines = $null
      if (-not $browserToken) { throw "real-browser token output is invalid" }
      Pop-Location
      Push-Location $frontendRoot
      $env:SPELL_REAL_BACKEND = "true"
      $env:SPELL_E2E_TOKEN = $browserToken
        ...
        try {
          ...
          try {
            $down = Invoke-ComposeCleanup @(
              "-p", $browserProject, "-f", "compose.yaml", "down",
...
try {
  ...
  $suites.frontend_unit = [ordered]@{
    kind = "vitest-json"; capture = "artifacts/v0.6/final/tests/frontend-unit.json"
  ...
  $suites.frontend_build = [ordered]@{
    kind = "frontend-build"; capture = "artifacts/v0.6/final/tests/frontend-build.json"
  ...
  $workPackageRelative = "artifacts/v0.6/work-package/qualification.json"
  $workPackageValidatorRelative = "scripts/validate_candidate_evidence_v06.py"
  $gateScopeRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/scopes/v0.6-gate-0b.json"
  )
  $gateDocumentRelative = "SPELL_v0.6_Gate_0B.md"
  $gateValidatorRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/validate_v06_gate_0b.py"
  )
  ...
  $summary = [ordered]@{
    ...
    accepted_v05_release = [ordered]@{
      archive_sha256 = $v05ArchiveSha256
      sidecar_sha256 = $v05SidecarSha256
      tag_object = "a1b277d74d2fb19062ca3e4388e9104d45c50ec4"
      tag_archive_claim = "Final archive SHA-256: $v05ArchiveSha256"
    }
    teardown = [ordered]@{
  ...
  if (Test-Path -LiteralPath $canonicalFinal) {
    if (-not $Replace) { throw "final v0.6 qualification already exists; pass -Replace to republish" }
  ...
  [ordered]@{
    schema_version = "spell.v06.final-qualification-publication/1"
