...
$fingerprintTag = "openbexi-spell-v04-sbom-source:$runId"
$validatorTag = "openbexi-spell-v04-sbom-validator:$runId"
...
$definitions = [ordered]@{
  "backend.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v04-backend-sbom:$runId"
    Dockerfile = "backend/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-backend:0.4"
    Image = $null
    Licenses = [ordered]@{
  ...
  "driver.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v04-driver-sbom:$runId"
    Dockerfile = "driver_host/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-driver:0.4"
    Image = $null
    Licenses = [ordered]@{ "grpcio" = "Apache-2.0"; "protobuf" = "BSD-3-Clause" }
  }
  "frontend.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v04-frontend-sbom:$runId"
    Dockerfile = "proxy/Dockerfile"
    Target = "frontend-build"
    Subject = "openbexi_spell-frontend:0.4"
    Image = $null
    Licenses = [ordered]@{
  ...
  "proxy.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v04-proxy-sbom:$runId"
    Dockerfile = "proxy/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-proxy:0.4"
    Image = $null
...
function Get-SourceFingerprint {
  ...
  if ($LASTEXITCODE -ne 0) { throw "v0.4 SBOM fingerprint image build failed" }
  $image = (& $DockerExe image inspect $fingerprintTag --format "{{.Id}}").Trim()
  $runImageIds.Add($image)
  $script:fingerprintRunCount += 1
  $container = "spell-v04-sbom-fingerprint-$PID-$script:fingerprintRunCount-$runId"
  ...
  if (-not $value) { throw "v0.4 SBOM source fingerprint is invalid" }
  return $value
}
...
foreach ($candidate in @($output, $staging, $backup)) {
  ...
  if (-not $candidateFull.StartsWith("$artifactRootFull$([IO.Path]::DirectorySeparatorChar)")) {
    throw "refusing to use an SBOM path outside artifacts/v0.4: $candidateFull"
  }
}
...
try {
  ...
  try {
    ...
    if (@($definitions.Values | ForEach-Object { $_.Image } | Sort-Object -Unique).Count -ne 4) {
      throw "the four v0.4 SBOM inputs do not have distinct image identities"
    }
    $source = Get-SourceFingerprint
    if ($source -cne $sourceBefore) { throw "source changed during v0.4 image builds" }
    ...
    $validatorContainer = "spell-v04-sbom-validator-$PID-$runId"
    ...
    if ((Get-SourceFingerprint) -cne $source) { throw "source changed during v0.4 SBOM generation" }
  }
