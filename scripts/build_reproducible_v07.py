#!/usr/bin/env python3
"""Build and byte-verify the bounded SPELL v0.7 source release package."""
...
EVIDENCE_DIRECTORY = Path("artifacts/v0.7")
DEFAULT_OUTPUT = EVIDENCE_DIRECTORY / "openbexi-spell-v0.7.0.tar.gz"
...
WORK_PACKAGE_MANIFEST = "artifacts/v0.7/work-package/qualification.json"
...
def _validate_path(relative: Path) -> None:
    ...
    if (
        ...
        and relative.parts[1] != "v0.7"
    ):
        raise ValueError(
            f"accepted prior-release evidence cannot enter v0.7 package: {relative.as_posix()}"
        )
    if relative.suffix.casefold() == ".pdf" or suffixes & ARCHIVE_SUFFIXES:
        raise ValueError(f"PDF/archive input cannot enter v0.7 package: {relative.as_posix()}")
    if relative.suffix.casefold() in RUNTIME_SUFFIXES or lowered_parts & RUNTIME_PARTS:
        raise ValueError(f"runtime journal cannot enter v0.7 package: {relative.as_posix()}")
    if lowered_name in SECRET_NAMES or relative.suffix.casefold() in SECRET_SUFFIXES or "secrets" in lowered_parts:
        raise ValueError(f"secret-bearing path cannot enter v0.7 package: {relative.as_posix()}")
...
def _release_evidence_scanner_input(relative: str, data: bytes) -> bytes:
    ...
    if relative in STRUCTURED_SECRET_EVIDENCE_PATHS:
        from scripts.validate_release_evidence_v07 import _secret_scannable_evidence
...
def _validate_bytes(relative: Path, data: bytes) -> None:
    ...
    if PEM_PRIVATE_KEY_BEGIN in scanner_input:
        ...
        if literals is None:
            raise ValueError(
                f"high-confidence secret material cannot enter v0.7 package: {normalized}"
            )
    ...
    for pattern in SECRET_PATTERNS:
        if pattern.search(scanner_input):
            raise ValueError(
                f"high-confidence secret material cannot enter v0.7 package: {relative.as_posix()}"
            )
...
def _walk(root: Path, tree: Path) -> Iterable[Path]:
    for current, directories, filenames in os.walk(tree):
        ...
        for name in directories:
            ...
            if path_has_link_or_reparse_v07(root, candidate):
                raise ValueError(
                    "v0.7 release input must not be a link or reparse point: "
...
def product_files_v07(root: Path) -> list[Path]:
    ...
    for tree_name in FINGERPRINT_TREES:
        ...
        if not tree.is_dir() or path_has_link_or_reparse_v07(source_root, tree):
            raise FileNotFoundError(f"required v0.7 release tree is missing: {tree_name}")
    ...
    for file_name in FINGERPRINT_FILES:
        ...
        if not path.is_file() or path_has_link_or_reparse_v07(source_root, path):
            raise FileNotFoundError(f"required v0.7 release file is missing: {file_name}")
...
def release_files_v07(root: Path) -> list[Path]:
    source_root = root.resolve()
    candidates = product_files_v07(source_root)
    evidence_root = source_root / EVIDENCE_DIRECTORY
    if not evidence_root.is_dir() or path_has_link_or_reparse_v07(
        source_root, evidence_root
    ):
        raise FileNotFoundError("required v0.7 evidence directory is missing")
    for path in evidence_root.rglob("*"):
        ...
        if path_has_link_or_reparse_v07(source_root, path):
            raise ValueError(
                "v0.7 evidence input must not be a link or reparse point: "
                f"{relative.as_posix()}"
            )
        if not path.is_file():
            continue
    ...
    if not any(path.relative_to(source_root).as_posix() == "artifacts/v0.7/release-qualification.json" for path in paths):
        raise FileNotFoundError("required v0.7 release qualification is missing")
    return paths
...
def _assert_head_bytes(root: Path, paths: Iterable[Path]) -> str:
    ...
    if _git(source_root, "status", "--porcelain").strip():
        raise ValueError("v0.7 release package requires a clean worktree")
...
def product_package_sha256_v07(root: Path) -> str:
    return hashlib.sha256(_archive_bytes(root.resolve(), product_files_v07(root))).hexdigest()


def build_reproducible_v07(root: Path, output: Path | None = None) -> dict[str, object]:
    source_root = root.resolve()
    ...
    if path_has_link_or_reparse_v07(source_root, lexical_artifact_root):
        raise ValueError("v0.7 package artifact root must not contain a link or reparse point")
    if path_has_link_or_reparse_v07(source_root, lexical_output.parent):
        raise ValueError("v0.7 package output path must not contain a link or reparse point")
    artifact_root = lexical_artifact_root.resolve()
    output_path = lexical_output.resolve()
    try:
        output_path.relative_to(artifact_root)
    except ValueError as exc:
        raise ValueError("v0.7 package output must stay under artifacts/v0.7") from exc
    if not output_path.name.endswith(".tar.gz"):
        raise ValueError("v0.7 package output must use .tar.gz")

    from scripts.validate_release_evidence_v07 import validate_release_evidence_v07

    validation_before = validate_release_evidence_v07(
        source_root, require_package=False
    )
    paths_before = release_files_v07(source_root)
    release_commit = _assert_head_bytes(source_root, paths_before)
    first = _archive_bytes(source_root, paths_before)
    paths_after = release_files_v07(source_root)
    second = _archive_bytes(source_root, paths_after)
    validation_after = validate_release_evidence_v07(
        source_root, require_package=False
    )
    if validation_before != validation_after:
        raise ValueError("v0.7 evidence changed during package construction")
    if [path.relative_to(source_root) for path in paths_before] != [
        path.relative_to(source_root) for path in paths_after
    ]:
        raise ValueError("v0.7 package input manifest changed between builds")
    if first != second:
        raise ValueError("v0.7 package builds are not byte-identical")
    if source_fingerprint_v07(source_root) != validation_before.source_fingerprint_sha256:
        raise ValueError("v0.7 source fingerprint changed during package construction")
    ...
    for path in (output_path, sidecar):
        if path.exists() and (
            not path.is_file()
            or path_has_link_or_reparse_v07(source_root, path)
        ):
            raise ValueError(f"refusing unsafe v0.7 package output: {path}")
    ...
    return {
        "schema_version": "spell.v07.package-result/1",
        ...
        "product_package_sha256": product_package_sha256_v07(source_root),
        "final_archive_sha256": digest,
        "package_build_count": 2,
        "package_byte_identical": True,
        "accepted_v06_release": {
...
def main() -> int:
    ...
    print(json.dumps(build_reproducible_v07(args.root, args.output), sort_keys=True))
    return 0
