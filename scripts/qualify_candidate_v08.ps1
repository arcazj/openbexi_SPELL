...
$project = "spell-v08-candidate-$runId"
$browserProject = "$project-browser"
$qualificationTag = "openbexi-spell-v08-candidate-qualification:$runId"
$postgresTag = "openbexi-spell-v08-candidate-postgres:$runId"
...
if ($LocalConfirmation -cne "LOCAL_SYNTHETIC_NON_CUI_ONLY") {
  throw "v0.8 candidate qualification requires the exact local synthetic non-CUI confirmation"
}
...
$script:Validator = Join-Path $root "scripts/validate_candidate_evidence_v08.py"
...
try {
  ...
  if ($sourceParents -cne "451c065740e7b6501f86094d9be79578b30b1591") {
  ...
  git merge-base --is-ancestor v0.7.0 $SourceCommit
  Assert-NativeSuccess "v0.7.0 is not an ancestor of the candidate source"
  git diff --quiet v0.7.0 $SourceCommit -- artifacts/v0.7
  Assert-NativeSuccess "tracked v0.7 artifacts changed in the candidate source"
  & $script:LockedPython -I (Join-Path $root "scripts/accepted_v07_release_v08.py") --root $root | Out-Null
  Assert-NativeSuccess "accepted external v0.7 release validation failed"
  ...
  $gateOutput = @(& $script:LockedPython -I (Join-Path $root "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/validate_v08_gate_0a.py"))
  ...
  & $script:DockerExe build --pull=false --provenance=false `
    ...
    --label "org.openbexi.spell.v08.source.commit=$SourceCommit" `
    -f (Join-Path $candidateRoot "scripts/qualification-v08.Dockerfile") `
  ...
  $databasePassword = "v08-candidate-pg-$runId"
  ...
  Merge-JUnit @($toolingBaseXml, $toolingV05Xml, $toolingV06Xml, $toolingLegacyXml, $toolingHostXml) $toolingXml
  ...
  $soakContainer = "$project-backend-v08-soak"
...
document = {"profile":"v08-cross-feature-replay-soak","iterations":5,"nodes":sorted(nodes),"runs":runs,"passed":all(item["exit_code"] == 0 for item in runs)}
...
'@
  ...
  Assert-NativeSuccess "cannot create v0.8 soak container"
  & $script:DockerExe start --attach $soakContainer
  Assert-NativeSuccess "v0.8 replay soak failed"
  $soakJson = Join-Path $captureRoot "backend-v08-soak.json"
  & $script:DockerExe cp "${soakContainer}:/qualification-output/result.json" $soakJson | Out-Null
  Assert-NativeSuccess "cannot copy v0.8 soak capture"
  ...
  try {
    ...
    try {
      ...
      & $npxExe playwright test e2e/operator-workspace.spec.ts e2e/driver-projection.spec.ts e2e/data-services.spec.ts --reporter=junit
...
'@
    ...
    foreach ($name in @("DATABASE_URL", "SPELL_DATA_DIR", "SPELL_V0007_BACKUP_DIR", "SPELL_PROCEDURES_DIR", "SPELL_JWT_HS256_SECRET", "SPELL_ALLOW_LOCAL_DEV_TOKEN", "SPELL_DRIVER_ENABLED", "SPELL_REAL_BACKEND", "SPELL_E2E_TOKEN", "SPELL_E2E_BASE_URL", "SPELL_E2E_ARTIFACT_DIRECTORY", "PLAYWRIGHT_JUNIT_OUTPUT_FILE", "CI")) {
    ...
    try {
      ...
      $env:SPELL_JWT_HS256_SECRET = "v08-candidate-jwt-$runId"
      ...
      $env:CI = "1"
      & $npxExe playwright test e2e/integration.spec.ts e2e/data-services-real.spec.ts --reporter=junit
    ...
    try {
      ...
      $env:SPELL_DB_PASSWORD = "v08-browser-db-$runId"
      $env:SPELL_JWT_HS256_SECRET = "v08-browser-service-secret-$runId"
      ...
      $tokenLines = @(
        & $script:ComposeExe -p $browserProject --profile driver `
          ...
          --subject v08-browser-viewer --role viewer --lifetime 600
      )
...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $candidateRoot "artifacts/v0.8/work-package/schema.json") -Destination (Join-Path $stageRoot "schema.json")
  $captureMap = [ordered]@{
    ...
    "tests/backend-v08-soak.json" = Join-Path $captureRoot "backend-v08-soak.json"
  ...
  $soak = Get-Content -LiteralPath (Join-Path $stageRoot "tests/backend-v08-soak.json") -Raw | ConvertFrom-Json
  ...
  $suites = [ordered]@{
    ...
    backend_v08_soak = New-SuiteRecord "backend_v08_soak" "soak" "tests/backend-v08-soak.json" $soakSummary "internal"
  ...
  $baselineV07 = (@(& $script:LockedPython -I $script:Validator --root $root --tree-fingerprint v0.7.0 artifacts/v0.7)[0] | ConvertFrom-Json).sha256
  $sourceV07 = (@(& $script:LockedPython -I $script:Validator --root $root --tree-fingerprint $SourceCommit artifacts/v0.7)[0] | ConvertFrom-Json).sha256
  ...
  $contractPaths = @(
    "contracts/v08/manifest.json", "contracts/v08/typed_values.json",
    "contracts/v08/catalog_uri_dependency.json", "contracts/v08/dictionary_exchange.json",
    "contracts/v08/data_containers.json", "contracts/v08/shared_data.json",
    "contracts/v08/virtual_files.json", "contracts/v08/data_api_authorization.json",
  ...
  $toolchainPaths = @(
    "scripts/release-toolchain-v04.json", "scripts/qualification-v08.Dockerfile",
    "scripts/qualification-v08.Dockerfile.dockerignore", "frontend/package-lock.json",
  ...
  $skipRecords = @(
    $toolingSkips | ForEach-Object {
      $reason = if ($_ -like "*supply_chain_v04.py*") { "WINDOWS_POWERSHELL_HOST_CONTRACT_NOT_APPLICABLE_IN_LOCKED_LINUX_IMAGE" }
        elseif ($_ -like "*release_v05.py*" -or $_ -like "*release_v06.py*" -or $_ -like "*release_v07.py*" -or $_ -like "*release_v08.py*" -or $_ -like "*qualify_release_v04.py*" -or $_ -like "*qualify_release_v05.py*" -or $_ -like "*qualify_release_v06.py*" -or $_ -like "*qualify_release_v07.py*" -or $_ -like "*qualify_release_v08.py*") { "POWERSHELL_UNAVAILABLE_IN_LOCKED_LINUX_IMAGE" }
  ...
  function New-AssuranceRecord([string[]]$Suffixes) {
    ...
    $proofCount = 0
    foreach ($id in $ids) {
      if ($id -cnotmatch '^(V08-DATA-00[1-9])-[A-Z0-9-]+$') {
  ...
  $manifest = [ordered]@{
    ...
    gate_0a = [ordered]@{ gate_id = "V08-GATE-0A"; status = "PASS"; validator_output = $gateMarker; owner_marker = "V08-GATE-0A OWNER-APPROVAL: APPROVED"; files_sha256 = $gateHashes }
    contracts = [ordered]@{ manifest_schema = "spell.v08.contract-manifest/1"; files_sha256 = $contractHashes }
    toolchain = [ordered]@{
    ...
    v0_7_immutability = [ordered]@{
      baseline_tag = "v0.7.0"; baseline_tree_fingerprint_sha256 = [string]$baselineV07
      source_tree_fingerprint_sha256 = [string]$sourceV07; git_diff_empty = $true
      accepted_archive_path = "artifacts/v0.7/openbexi-spell-v0.7.0.tar.gz"
      accepted_archive_sha256 = $acceptedArchiveSha256
      accepted_sidecar_path = "artifacts/v0.7/openbexi-spell-v0.7.0.tar.gz.sha256"
      accepted_sidecar_sha256 = $acceptedSidecarSha256
      accepted_tag_object = "70e4d46a46d158dee3c63ec37a5d1922b3b61668"
      accepted_tag_archive_claim = "Final archive SHA-256: $acceptedArchiveSha256"
    }
    secret_scan = [ordered]@{
      scanner = "validate_candidate_evidence_v08.py/bounded-patterns-1"
