#!/usr/bin/env python3
"""Create the canonical SPELL v0.5.0 release-qualification manifest."""
...
from scripts import validate_release_evidence_v05 as release  # noqa: E402
from scripts.build_reproducible_v05 import product_package_sha256_v05  # noqa: E402
from scripts.source_fingerprint_v05 import source_fingerprint_v05  # noqa: E402


MANIFEST_RELATIVE = Path("artifacts/v0.5") / release.MANIFEST_NAME
WORK_PACKAGE_EVIDENCE_PATHS = frozenset(
    path
    for path in release.CANONICAL_EVIDENCE_PATHS
    if path.startswith("artifacts/v0.5/work-package/")
)
...
@dataclass(frozen=True)
class ReleaseQualificationProductionV05:
    path: str
    sha256: str
...
def _assert_generation_git_state(
    ...
    *,
    ...
    ancestry = release._run_git(
        root,
        ["merge-base", "--is-ancestor", release.QUALIFICATION_SOURCE_COMMIT, commit],
        accepted=(0, 1),
    )
    _require(ancestry.returncode == 0, "qualification source is not an ancestor of source freeze")
    candidate_delta = sorted(
        release._git_name_list(
            root,
            ["diff", "--name-only", "-z", release.CANDIDATE_COMMIT, commit],
    ...
    tracked_evidence = sorted(
        release._git_name_list(
            root,
            ["ls-tree", "-r", "--name-only", "-z", "HEAD", "--", "artifacts/v0.5"],
    ...
    return QualifiedSourceV05(commit, tree, tuple(candidate_delta))


def _qualification_source_binding() -> dict[str, Any]:
    return {
        "commit": release.QUALIFICATION_SOURCE_COMMIT,
        "tree": release.QUALIFICATION_SOURCE_TREE,
        "parent": release.QUALIFICATION_SOURCE_PARENT,
        "correction": {
            "path": release.QUALIFICATION_CORRECTION_PATH,
            "blob": release.QUALIFICATION_CORRECTION_BLOB,
            "sha256": release.QUALIFICATION_CORRECTION_SHA256,
        },
...
def _validate_final_toolchain_transition(
    ...
    release._exact_keys(candidate, expected_keys, "candidate toolchain")
    release._exact_keys(final, expected_keys, "final toolchain")
    for key in expected_keys:
        if key != "qualification_image_id":
            _require(
                final.get(key) == candidate.get(key),
                f"final toolchain immutable binding differs: {key}",
            )
    image_id = final.get("qualification_image_id")
    _require(
        isinstance(image_id, str) and release.IMAGE_ID_RE.fullmatch(image_id) is not None,
        "final toolchain qualification image identity is invalid",
    )
...
def _sbom(root: Path, source_fingerprint: str) -> dict[str, Any]:
    directory_relative = "artifacts/v0.5/sbom"
...
def _supply_chain(root: Path, source_fingerprint: str) -> dict[str, Any]:
    relative = "artifacts/v0.5/supply-chain.json"
    ...
    return {
        ...
        "accepted_v04_artifacts_unchanged": capture.get(
...
def _evidence(root: Path) -> dict[str, Any]:
    files = release.canonical_evidence_files(root, root / "artifacts/v0.5")
    declarations = {
...
def _validate_manifest_components(root: Path, manifest: Mapping[str, Any]) -> None:
    ...
    release.validate_v04_unchanged(root, _mapping(manifest["inherited_v04"], "inherited_v04"))
...
def build_release_qualification_v05(root: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "qualified_source": {
            ...
            "tree": source.tree,
            "candidate_commit": release.CANDIDATE_COMMIT,
            "qualification_source": _qualification_source_binding(),
        ...
        "inherited_v04": {
            ...
            "accepted_tag_ref": release.V04_TAG_REF,
            "accepted_tag_object": release.V04_TAG_OBJECT,
            "accepted_commit": release.V04_COMMIT,
            "accepted_artifact_tree": release.V04_ARTIFACT_TREE,
        },
    ...
    _require(
        source_fingerprint_v05(source_root) == source_fingerprint,
        "source changed while release qualification was assembled",
    )
    _require(
        product_package_sha256_v05(source_root) == product_package,
...
def create_release_qualification_v05(
    root: Path,
) -> ReleaseQualificationProductionV05:
    source_root = root.resolve()
    output = source_root / MANIFEST_RELATIVE
    manifest = build_release_qualification_v05(source_root)
    raw = _manifest_bytes(manifest)
    output.parent.mkdir(parents=True, exist_ok=True)
    _require(not output.is_symlink(), "release qualification output must not be a symlink")
    if output.exists():
    ...
    return ReleaseQualificationProductionV05(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = create_release_qualification_v05(args.root)
    print(
        json.dumps(
            {
                "schema_version": release.SCHEMA_VERSION,
