...
ENV PYTHONDONTWRITEBYTECODE=1 \
    PYTHONUNBUFFERED=1 \
    PIP_NO_CACHE_DIR=1 \
    SPELL_QUALIFICATION_RELEASE=v0.9.0
...
COPY scripts/supply-chain-requirements.hashes.lock /tmp/supply-chain.lock
RUN python -m pip install --require-hashes \
        -r /tmp/backend.lock \
        -r /tmp/pki.lock \
        -r /tmp/generator.lock \
        -r /tmp/supply-chain.lock \
    && rm -f /tmp/backend.lock /tmp/pki.lock /tmp/generator.lock /tmp/supply-chain.lock
