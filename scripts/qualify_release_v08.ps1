...
$gate0aCommit = "451c065740e7b6501f86094d9be79578b30b1591"
...
$project = "spell-v08-final-$runId"
$qualificationTag = "openbexi-spell-v08-final-qualification:$runId"
$postgresTag = "openbexi-spell-v08-final-postgres:$runId"
...
function Assert-V07ArtifactsUnchanged {
  ...
  $expectedTree = "b6b4a9239e36eaea61da8e7d87cc5bffecfd064f"
  $tree = @(& git rev-parse "${Commit}:artifacts/v0.7") -join "`n"
  Assert-NativeSuccess "cannot inspect accepted v0.7 artifact tree"
  if ($tree.Trim() -cne $expectedTree) { throw "accepted v0.7 artifacts changed in source commit" }
  & git diff --quiet v0.7.0 $Commit -- artifacts/v0.7
  Assert-NativeSuccess "accepted v0.7 artifact bytes differ from v0.7.0"
  & git diff --quiet -- artifacts/v0.7
  Assert-NativeSuccess "accepted v0.7 artifacts have a working-tree change"
  $status = @(& git status --porcelain --untracked-files=all -- artifacts/v0.7)
  Assert-NativeSuccess "cannot inspect accepted v0.7 artifact worktree"
  if ($status.Count -ne 0) { throw "accepted v0.7 artifacts have an untracked change" }
...
function New-JUnitSuiteDeclaration {
  ...
  return [ordered]@{
    kind = "junit"
    capture = "artifacts/v0.8/final/tests/$($Id.Replace('_', '-')).xml"
...
function Assert-FinalToolchain {
  ...
  $candidatePath = Join-Path $root "artifacts/v0.8/work-package/qualification.json"
...
function Assert-FinalPublication {
  ...
  Assert-ExactProperties $summary @(
    ...
    "toolchain", "suites", "teardown", "accepted_v07_release", "accepted_exceptions", "overall_pass"
  ) "final qualification summary"
  if (
    $summary.schema_version -cne "spell.v08.final-qualification/1" -or
    $summary.product_version -cne "0.8.0" -or
    $summary.scope_profile -cne "LOCAL_SYNTHETIC_NON_CUI_DATA_SERVICE" -or
    $summary.overall_pass -ne $true -or @($summary.accepted_exceptions).Count -ne 0
  ) { throw "final qualification summary header differs" }
  Assert-ExactProperties $summary.accepted_v07_release @(
    "archive_sha256", "sidecar_sha256", "tag_object", "tag_archive_claim"
  ) "final qualification accepted v0.7 release"
  if (
    [string]$summary.accepted_v07_release.archive_sha256 -cne $v07ArchiveSha256 -or
    [string]$summary.accepted_v07_release.sidecar_sha256 -cne $v07SidecarSha256 -or
    [string]$summary.accepted_v07_release.tag_object -cne "70e4d46a46d158dee3c63ec37a5d1922b3b61668" -or
    [string]$summary.accepted_v07_release.tag_archive_claim -cne "Final archive SHA-256: $v07ArchiveSha256"
  ) { throw "final qualification accepted v0.7 release binding differs" }
  ...
  foreach ($id in $finalSuiteIds) {
    ...
    if (-not $finalFull.Equals([IO.Path]::GetFullPath((Join-Path $root "artifacts/v0.8/final")))) {
...
if ($ValidateOnly) {
  $validated = Assert-FinalPublication (Join-Path $root "artifacts/v0.8/final")
  [ordered]@{
    schema_version = "spell.v08.final-qualification-validation/1"
...
if ($LocalConfirmation -cne "LOCAL_SYNTHETIC_NON_CUI_ONLY") {
  throw "v0.8 final qualification requires the exact local synthetic non-CUI confirmation"
}

$tempRoot = Join-Path $env:TEMP "spell-v08-final-$runId"
...
try {
  $status = @(& git status --porcelain=v1 --untracked-files=all -- . `
    ":(exclude)artifacts/v0.8/final/**" ":(exclude)artifacts/v0.8/.qualification/**")
  ...
  Assert-V07ArtifactsUnchanged $resolvedSource
  if ($resolvedSource -cne $headCommit) {
    ...
    foreach ($path in $closeout) {
      if (
        -not ([string]$path).StartsWith("artifacts/v0.8/", [StringComparison]::Ordinal) -or
        ([string]$path).Contains("/.qualification/") -or
        ([string]$path).EndsWith(".tar.gz") -or ([string]$path).EndsWith(".tar.gz.sha256")
      ) { throw "release HEAD differs from frozen source outside canonical v0.8 evidence: $path" }
    }
  }
  ...
  foreach ($required in @(
    ...
    "artifacts/v0.8/work-package/qualification.json",
...
print(source_fingerprint_v08(root))
print(product_package_sha256_v08(root))
'@
  ...
  $candidateValidator = Join-Path $root "scripts/validate_candidate_evidence_v08.py"
  ...
  $candidateEvidencePath = Join-Path $sourceRoot "artifacts/v0.8/work-package/qualification.json"
  $candidateEvidence = Get-Content -LiteralPath $candidateEvidencePath -Raw | ConvertFrom-Json
  $schemaRelative = "artifacts/v0.8/work-package/schema.json"
  ...
  $qualificationDockerfile = Join-Path $sourceRoot "scripts/qualification-v08.Dockerfile"
  & $script:DockerExe build --pull=false --provenance=false `
    --label "com.docker.compose.project=$project" `
    --label "org.openbexi.spell.v08.implementation.commit=$candidateCommit" `
    --label "org.openbexi.spell.v08.source.commit=$resolvedSource" `
    --label "org.openbexi.spell.v08.source.tree=$resolvedTree" `
...
'@
  ...
  $databasePassword = "v08-final-pg-$runId"
...
'@
  ...
  if ($hostToolingNodes.Count -ne 24) { throw "host-only tooling node inventory differs" }
  ...
  foreach ($node in $rootExternalToolingNodes) {
    if ($node -notin $toolingNodes) { throw "external accepted-v0.7 tooling node is absent: $node" }
  }
  if ($rootExternalToolingNodes.Count -ne 22) { throw "current-root external tooling node inventory differs" }
  ...
  foreach ($node in @($hostToolingNodes) + @($rootExternalToolingNodes) + @($v05ExportToolingNodes) + @($v06ExportToolingNodes) + @($v07ExportToolingNodes)) {
  ...
  try {
    ...
    Assert-NativeSuccess "locked-host accepted-v0.7 external release tests failed"
  }
  finally { Pop-Location }
  ...
  Merge-JUnit @($toolingBaseXml, $toolingHostXml, $toolingRootXml, $toolingV05Xml, $toolingV06Xml, $toolingV07Xml) $toolingXml
  ...
  try {
    ...
    $buildDocument = [ordered]@{
      schema_version = "spell.v08.frontend-build/1"
    ...
    try {
      ...
      $browserDbPassword = "v08-final-browser-db-$runId"
      $browserJwtSecret = "v08-final-browser-jwt-$runId-material"
      ...
      $env:SPELL_TELEMETRY_CONTEXT_ID = "v08-telemetry-synthetic-context"
      Push-Location $sourceRoot
      ...
      $tokenLines = @(& $composeExe -p $browserProject --profile driver `
        -f (Join-Path $sourceRoot "compose.yaml") `
        exec -T backend python /app/scripts/issue_dev_token.py `
        --subject v08-final-browser --role admin --lifetime 900)
...
try {
  ...
  $suites.frontend_unit = [ordered]@{
    kind = "vitest-json"; capture = "artifacts/v0.8/final/tests/frontend-unit.json"
  ...
  $suites.frontend_build = [ordered]@{
    kind = "frontend-build"; capture = "artifacts/v0.8/final/tests/frontend-build.json"
  ...
  $workPackageRelative = "artifacts/v0.8/work-package/qualification.json"
  $workPackageValidatorRelative = "scripts/validate_candidate_evidence_v08.py"
  $gateScopeRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/scopes/v0.8-gate-0b.json"
  )
  $gateDocumentRelative = "SPELL_v0.8_Gate_0B.md"
  $gateValidatorRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/validate_v08_gate_0b.py"
  )
  ...
  $summary = [ordered]@{
    schema_version = "spell.v08.final-qualification/1"
    product_version = "0.8.0"
    ...
    accepted_v07_release = [ordered]@{
      archive_sha256 = $v07ArchiveSha256
      sidecar_sha256 = $v07SidecarSha256
      tag_object = "70e4d46a46d158dee3c63ec37a5d1922b3b61668"
      tag_archive_claim = "Final archive SHA-256: $v07ArchiveSha256"
    }
    teardown = [ordered]@{
  ...
  if (Test-Path -LiteralPath $canonicalFinal) {
    if (-not $Replace) { throw "final v0.8 qualification already exists; pass -Replace to republish" }
  ...
  [ordered]@{
    schema_version = "spell.v08.final-qualification-publication/1"
