...
$candidateCommit = "aefa658ce01d49a7879d0471b50425ac3bcf9e2d"
...
$project = "spell-v05-final-$runId"
$qualificationTag = "openbexi-spell-v05-final-qualification:$runId"
$postgresTag = "openbexi-spell-v05-final-postgres:$runId"
...
$finalSuiteIds = @(
  "backend_sqlite", "backend_postgresql", "driver_host", "tooling",
...
function New-JUnitSuiteDeclaration {
  ...
  return [ordered]@{
    kind = "junit"
    capture = "artifacts/v0.5/final/tests/$($Id.Replace('_', '-')).xml"
...
function Assert-FinalToolchain {
  ...
  $fields = @(
    "lock_path", "lock_sha256", "python_version", "python_sha256",
    "qualification_dockerfile_path", "qualification_dockerfile_sha256",
    "qualification_dockerignore_path", "qualification_dockerignore_sha256",
    "qualification_image_id", "external_manuals"
  )
  Assert-ExactProperties $Toolchain $fields "final qualification toolchain"
  if ([string]$Toolchain.qualification_image_id -cnotmatch '^sha256:[0-9a-f]{64}$') {
    throw "final qualification image identity is invalid"
  }
  $candidatePath = Join-Path $root "artifacts/v0.5/work-package/qualification.json"
  $candidate = Get-Content -LiteralPath $candidatePath -Raw | ConvertFrom-Json
  Assert-ExactProperties $candidate.toolchain $fields "candidate qualification toolchain"
  foreach ($field in @(
    "lock_path", "lock_sha256", "python_version", "python_sha256",
    "qualification_dockerfile_path", "qualification_dockerfile_sha256",
    "qualification_dockerignore_path", "qualification_dockerignore_sha256"
  ...
  if (
    ($Toolchain.external_manuals | ConvertTo-Json -Compress -Depth 100) -cne
    ($candidate.toolchain.external_manuals | ConvertTo-Json -Compress -Depth 100)
  ) { throw "final qualification external-manual binding differs" }
}
...
function Assert-FinalPublication {
  ...
  $expectedCaptureNames = @(
    "backend-sqlite.xml", "backend-postgresql.xml", "driver-host.xml", "tooling.xml",
  ...
  if (
    $summary.schema_version -cne "spell.v05.final-qualification/1" -or
    $summary.product_version -cne "0.5.0" -or
  ...
  foreach ($id in $finalSuiteIds) {
    ...
    if (-not $finalFull.Equals([IO.Path]::GetFullPath((Join-Path $root "artifacts/v0.5/final")))) {
...
if ($ValidateOnly) {
  $validated = Assert-FinalPublication (Join-Path $root "artifacts/v0.5/final")
  [ordered]@{
    schema_version = "spell.v05.final-qualification-validation/1"
...
if ($LocalConfirmation -cne "LOCAL_SYNTHETIC_NON_CUI_ONLY") {
  throw "v0.5 final qualification requires the exact local synthetic non-CUI confirmation"
}

$tempRoot = Join-Path $env:TEMP "spell-v05-final-$runId"
...
try {
  $status = @(& git status --porcelain=v1 --untracked-files=all -- . `
    ":(exclude)artifacts/v0.5/final/**" ":(exclude)artifacts/v0.5/.qualification/**")
  ...
  & git merge-base --is-ancestor $qualificationCommit $resolvedSource
  Assert-NativeSuccess "qualification correction is not included in frozen source"
  ...
  if ($resolvedSource -cne $headCommit) {
    ...
    foreach ($path in $closeout) {
      if (
        -not ([string]$path).StartsWith("artifacts/v0.5/", [StringComparison]::Ordinal) -or
        ([string]$path).Contains("/.qualification/") -or
        ([string]$path).EndsWith(".tar.gz") -or ([string]$path).EndsWith(".tar.gz.sha256")
      ) { throw "release HEAD differs from frozen source outside canonical v0.5 evidence: $path" }
    }
  }
  ...
  foreach ($required in @(
    ...
    "artifacts/v0.5/work-package/qualification.json",
...
print(source_fingerprint_v05(root))
print(product_package_sha256_v05(root))
'@
  ...
  $candidateValidator = Join-Path $root "scripts/validate_candidate_evidence_v05.py"
  ...
  $qualificationDockerfile = Join-Path $sourceRoot "scripts/qualification-v05.Dockerfile"
  & $script:DockerExe build --pull=false --provenance=false `
    --label "com.docker.compose.project=$project" `
    --label "org.openbexi.spell.v05.implementation.commit=$candidateCommit" `
    --label "org.openbexi.spell.v05.qualification.commit=$qualificationCommit" `
    --label "org.openbexi.spell.v05.source.commit=$resolvedSource" `
    --label "org.openbexi.spell.v05.source.tree=$resolvedTree" `
  ...
  [string[]]$backendNodes = @(Get-CollectedNodes "backend/tests" "backend/tests/" 269 `
    "802b3aa602e209cca204a4d17e1e71eb65cec408d59f575b0b3269d2424d771b")
  [string[]]$driverNodes = @(Get-CollectedNodes "driver_host/tests" "driver_host/tests/" 77 `
  ...
  $candidateEvidencePath = Join-Path $sourceRoot "artifacts/v0.5/work-package/qualification.json"
  ...
  if (
    $candidateToolingNodes.Count -ne 334 -or
  ...
  $databasePassword = "v05-final-pg-$runId"
...
'@
  ...
  if ($hostToolingNodes.Count -ne 7) { throw "Windows-only tooling node inventory differs" }
  ...
  Merge-JUnit @($toolingBaseXml, $toolingHostXml) $toolingXml
  ...
  try {
    ...
    try {
      ...
      $mockListRaw = @(& npx playwright test e2e/auth.spec.ts e2e/console.spec.ts --list --reporter=json)
      Assert-NativeSuccess "mocked browser collection failed"
      [string[]]$mockNodes = @(
        Get-PlaywrightNodes (($mockListRaw -join "`n") | ConvertFrom-Json)
      )
      & npx playwright test e2e/auth.spec.ts e2e/console.spec.ts --reporter=junit --retries=0
      ...
      $browserDbPassword = "v05-final-browser-db-$runId"
      $browserJwtSecret = "v05-final-browser-jwt-$runId-material"
      ...
      $tokenLines = @(& $composeExe -p $browserProject -f compose.yaml run --rm --no-deps `
        ...
        --subject v05-final-browser --role admin --lifetime 900)
      ...
      $realListRaw = @(& npx playwright test e2e/integration.spec.ts --list --reporter=json)
  ...
  $postgresResult = Assert-JUnitContract $postgresXml $backendNodes @() "backend PostgreSQL"
  ...
  if ($script:DockerExe) {
    ...
    foreach ($ownerProject in @($project, $browserProject)) {
      ...
      foreach ($query in $queries) {
        $inspection = Invoke-DockerCleanup ([string[]]$query)
...
try {
  ...
  $suites.frontend_unit = [ordered]@{
    kind = "vitest-json"; capture = "artifacts/v0.5/final/tests/frontend-unit.json"
  ...
  $suites.frontend_build = [ordered]@{
    kind = "frontend-build"; capture = "artifacts/v0.5/final/tests/frontend-build.json"
  ...
  $workPackageRelative = "artifacts/v0.5/work-package/qualification.json"
  $workPackageValidatorRelative = "scripts/validate_candidate_evidence_v05.py"
  $gateScopeRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/scopes/v0.5-gate-0b.json"
  )
  $gateDocumentRelative = "SPELL_v0.5_Gate_0B.md"
  $gateValidatorRelative = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/validate_v05_gate_0b.py"
  )
  $finalToolchain = [ordered]@{
    lock_path = [string]$candidateEvidence.toolchain.lock_path
    lock_sha256 = [string]$candidateEvidence.toolchain.lock_sha256
    python_version = [string]$candidateEvidence.toolchain.python_version
    python_sha256 = [string]$candidateEvidence.toolchain.python_sha256
    qualification_dockerfile_path = [string]$candidateEvidence.toolchain.qualification_dockerfile_path
    qualification_dockerfile_sha256 = [string]$candidateEvidence.toolchain.qualification_dockerfile_sha256
    qualification_dockerignore_path = [string]$candidateEvidence.toolchain.qualification_dockerignore_path
    qualification_dockerignore_sha256 = [string]$candidateEvidence.toolchain.qualification_dockerignore_sha256
    qualification_image_id = [string]$script:QualificationImage
    external_manuals = $candidateEvidence.toolchain.external_manuals
  }
  $summary = [ordered]@{
  ...
  if (Test-Path -LiteralPath $canonicalFinal) {
    if (-not $Replace) { throw "final v0.5 qualification already exists; pass -Replace to republish" }
  ...
  [ordered]@{
    schema_version = "spell.v05.final-qualification-publication/1"
