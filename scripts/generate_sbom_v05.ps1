...
$fingerprintTag = "openbexi-spell-v05-sbom-source:$runId"
$validatorTag = "openbexi-spell-v05-sbom-validator:$runId"
...
$definitions = [ordered]@{
  "backend.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v05-backend-sbom:$runId"
    Dockerfile = "backend/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-backend:0.5"
    Image = $null
    Licenses = [ordered]@{
  ...
  "driver.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v05-driver-sbom:$runId"
    Dockerfile = "driver_host/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-driver:0.5"
    Image = $null
    Licenses = [ordered]@{ "grpcio" = "Apache-2.0"; "protobuf" = "BSD-3-Clause" }
  }
  "frontend.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v05-frontend-sbom:$runId"
    Dockerfile = "proxy/Dockerfile"
    Target = "frontend-build"
    Subject = "openbexi_spell-frontend:0.5"
    Image = $null
    Licenses = [ordered]@{
  ...
  "proxy.cdx.json" = [ordered]@{
    Tag = "openbexi-spell-v05-proxy-sbom:$runId"
    Dockerfile = "proxy/Dockerfile"
    Target = $null
    Subject = "openbexi_spell-proxy:0.5"
    Image = $null
...
function Get-SourceFingerprint {
  & $DockerExe build --pull=false --provenance=false -f scripts/package-v05.Dockerfile `
    -t $fingerprintTag . | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "v0.5 SBOM fingerprint image build failed" }
  $image = (& $DockerExe image inspect $fingerprintTag --format "{{.Id}}").Trim()
  $runImageIds.Add($image)
  $script:fingerprintRunCount += 1
  $container = "spell-v05-sbom-fingerprint-$PID-$script:fingerprintRunCount-$runId"
  $runContainers.Add($container)
  $value = @(
    & $DockerExe run --rm --name $container --network none --entrypoint python $image `
      scripts/source_fingerprint_v05.py --root /workspace
  ) | Where-Object { $_ -cmatch '^[0-9a-f]{64}$' } | Select-Object -Last 1
  if (-not $value) { throw "v0.5 SBOM source fingerprint is invalid" }
  return $value
}
...
foreach ($candidate in @($output, $staging, $backup)) {
  ...
  if (-not $candidateFull.StartsWith("$artifactRootFull$([IO.Path]::DirectorySeparatorChar)")) {
    throw "refusing to use an SBOM path outside artifacts/v0.5: $candidateFull"
  }
}
...
try {
  ...
  try {
    ...
    if (@($definitions.Values | ForEach-Object { $_.Image } | Sort-Object -Unique).Count -ne 4) {
      throw "the four v0.5 SBOM inputs do not have distinct image identities"
    }
    $source = Get-SourceFingerprint
    if ($source -cne $sourceBefore) { throw "source changed during v0.5 image builds" }
    ...
    $validatorContainer = "spell-v05-sbom-validator-$PID-$runId"
    ...
    if ((Get-SourceFingerprint) -cne $source) { throw "source changed during v0.5 SBOM generation" }
  }
