#!/usr/bin/env python3
"""Build or verify the deterministic SPELL v0.11 source package."""
...
def build(root: Path = ROOT, *, verify: bool = False) -> dict:
    ...
    if [path.relative_to(root) for path in paths_before] != [path.relative_to(root) for path in paths_after]:
        raise ReleaseV11Error("v0.11 package input inventory changed during build")
    if first != second:
        raise ReleaseV11Error("v0.11 package builds are not byte-identical")
    ...
    manifest = {
        "schema_version": "spell.v11.release-manifest/1",
        ...
        "prior_release_artifacts_packaged": False,
        "v11_contracts_packaged": True,
        "v12_packaged": False,
        "decision": "PASS",
    }
    ...
    if verify:
        if package_path.read_bytes() != first or sidecar_path.read_bytes() != sidecar:
            raise ReleaseV11Error("published v0.11 package bytes differ from rebuild")
        assert published is not None
        if published != manifest:
            raise ReleaseV11Error("published v0.11 release manifest differs from rebuild")
        validate_release_evidence_v11(root, require_package=True)
        return manifest
...
def main() -> int:
    ...
    except (OSError, json.JSONDecodeError, ReleaseV11Error) as exc:
        print(f"v0.11-reproducible-package=FAIL reason={exc}")
        return 1
    print(
        "v0.11-reproducible-package=PASS "
