#!/usr/bin/env python3
"""Validate the bounded SPELL v0.5.0 final release evidence and tag."""
...
QUALIFICATION_SOURCE_COMMIT = "ef26e53f5ecccabef1fff03ec86d71b0c93edd2b"
QUALIFICATION_SOURCE_TREE = "f646a40bcd70ec9ebc28f3ebf3783e54c1c8f9a1"
QUALIFICATION_SOURCE_PARENT = "05a1c89c21b500270476b172d86d0758945d23d7"
QUALIFICATION_CORRECTION_PATH = "backend/tests/test_driver_isolation.py"
QUALIFICATION_CORRECTION_BLOB = "60ed5164ffe190ccbb5cee91ffe619eba7c8c9c2"
QUALIFICATION_CORRECTION_SHA256 = (
    "d0eca2c56705068027b910991719971838d5f84033806f9a0ff9de0f7b3e0756"
)
V04_TAG_REF = "refs/tags/v0.4.0"
V04_TAG_OBJECT = "86390c90e8d5f96f872be43274cbc9d789a34c2d"
V04_COMMIT = "4546d313a2d8f50504b2bc602d56b3b459ca7597"
V04_ARTIFACT_TREE = "e3ef54e822891f1e83c59da442b2901d3d0e323e"
...
WORK_PACKAGE_PATH = "artifacts/v0.5/work-package/qualification.json"
WORK_PACKAGE_VALIDATOR = "scripts/validate_candidate_evidence_v05.py"
GATE_0B_SCOPE = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/requirements/compatibility/"
    "scopes/v0.5-gate-0b.json"
)
GATE_0B_DOCUMENT = "SPELL_v0.5_Gate_0B.md"
GATE_0B_VALIDATOR = (
    "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/"
    "validate_v05_gate_0b.py"
)
GATE_0B_MARKER = (
...
FINAL_QUALIFICATION_PATH = "artifacts/v0.5/final/qualification.json"
CANONICAL_EVIDENCE_PATHS = frozenset(
    {
        WORK_PACKAGE_PATH,
        "artifacts/v0.5/work-package/tests/backend-sqlite.xml",
        "artifacts/v0.5/work-package/tests/backend-postgresql.xml",
        "artifacts/v0.5/work-package/tests/driver-host.xml",
        "artifacts/v0.5/work-package/tests/tooling.xml",
        FINAL_QUALIFICATION_PATH,
        "artifacts/v0.5/final/tests/backend-sqlite.xml",
        "artifacts/v0.5/final/tests/backend-postgresql.xml",
        "artifacts/v0.5/final/tests/driver-host.xml",
        "artifacts/v0.5/final/tests/tooling.xml",
        "artifacts/v0.5/final/tests/frontend-unit.json",
        "artifacts/v0.5/final/tests/frontend-build.json",
        "artifacts/v0.5/final/tests/browser-mocked.xml",
        "artifacts/v0.5/final/tests/browser-real.xml",
        "artifacts/v0.5/sbom/backend.cdx.json",
        "artifacts/v0.5/sbom/driver.cdx.json",
        "artifacts/v0.5/sbom/frontend.cdx.json",
        "artifacts/v0.5/sbom/proxy.cdx.json",
        "artifacts/v0.5/sbom/SHA256SUMS",
        "artifacts/v0.5/supply-chain.json",
    }
)
JUNIT_SUITE_IDS = (
    "backend_sqlite",
...
SBOM_SUBJECTS = {
    "backend.cdx.json": "openbexi_spell-backend:0.5",
    "driver.cdx.json": "openbexi_spell-driver:0.5",
    "frontend.cdx.json": "openbexi_spell-frontend:0.5",
    "proxy.cdx.json": "openbexi_spell-proxy:0.5",
}
...
ALLOWED_CANDIDATE_DELTA = {
    ...
    (
        "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/quality/tools/"
        "test_validate_v05_gate_0b.py"
    ),
    "scripts/audit_supply_chain_v05.ps1",
    "scripts/build_reproducible_v05.py",
    "scripts/create_release_qualification_v05.py",
    "scripts/generate_sbom_v05.ps1",
    "scripts/package-v04.Dockerfile.dockerignore",
    "scripts/package-v05.Dockerfile",
    "scripts/package_release_v05.ps1",
    "scripts/qualification-v05.Dockerfile",
    "scripts/qualification-v05.Dockerfile.dockerignore",
    "scripts/qualify_candidate_v05.ps1",
    "scripts/qualify_release_v05.ps1",
    "scripts/source_fingerprint_v05.py",
    "scripts/tests/test_qualify_release_v05.py",
    "scripts/tests/test_create_release_qualification_v05.py",
...
class ReleaseEvidenceError(ValueError):
    """Raised when final v0.5 evidence cannot be accepted."""
...
@dataclass(frozen=True)
class ReleaseEvidenceValidationV05:
...
def validate_frontend_build(
    ...
    _require(relative == "artifacts/v0.5/final/tests/frontend-build.json", "frontend build capture path differs")
    ...
    _require(capture.get("schema_version") == "spell.v05.frontend-build/1", "frontend build schema differs")
...
def validate_version_metadata(root: Path) -> None:
    ...
    _require(pyproject_path.is_file() and not pyproject_path.is_symlink(), "pyproject.toml is missing or unsafe")
    try:
    ...
    _require(supply.get("release_artifact_root") == "artifacts/v0.5", "v0.5 artifact root differs")
    ...
    for relative in ("driver_host/Dockerfile", "driver_host/pki.Dockerfile"):
        path = root / relative
        _require(path.is_file() and not path.is_symlink(), f"{relative} is missing or unsafe")
        lines = path.read_text(encoding="utf-8").splitlines()
        _require(lines.count("ARG SPELL_PACKAGE_VERSION=0.4.0") == 1, f"{relative} retained driver package version differs")
        _require("ARG SPELL_PACKAGE_VERSION=0.5.0" not in lines, f"{relative} incorrectly advances driver identity")


def _allowed_candidate_path(path: str) -> bool:
    return path in ALLOWED_CANDIDATE_DELTA or path.startswith("artifacts/v0.5/work-package/")


def validate_release_closeout_paths(paths: Sequence[str]) -> None:
    package_paths: set[str] = set()
    for path in paths:
        _require(
            path.startswith("artifacts/v0.5/"),
            f"release closeout changes source outside canonical v0.5 evidence: {path}",
        )
        _require(
    ...
    _require(
        not package_paths or package_paths == RELEASE_PACKAGE_PATHS,
...
def validate_source_git(
    ...
    _exact_keys(
        source,
        (
            "commit", "tree", "candidate_commit", "qualification_source",
    ...
    _require(source.get("candidate_commit") == CANDIDATE_COMMIT, "qualified source candidate binding differs")
    qualification = _mapping(
        source.get("qualification_source"), "qualified_source.qualification_source"
    )
    expected_qualification = {
        "commit": QUALIFICATION_SOURCE_COMMIT,
        "tree": QUALIFICATION_SOURCE_TREE,
        "parent": QUALIFICATION_SOURCE_PARENT,
        "correction": {
            "path": QUALIFICATION_CORRECTION_PATH,
            "blob": QUALIFICATION_CORRECTION_BLOB,
            "sha256": QUALIFICATION_CORRECTION_SHA256,
        },
    ...
    _require(qualification == expected_qualification, "qualified source correction binding differs")
    ...
    _require(
        _git_line(
            root,
            ["rev-parse", f"{QUALIFICATION_SOURCE_COMMIT}^{{tree}}"],
            "qualification source tree",
        )
        == QUALIFICATION_SOURCE_TREE,
        "qualification source Git tree differs",
    )
    _require(
        _git_line(
            root,
            ["rev-parse", f"{QUALIFICATION_SOURCE_COMMIT}^"],
            "qualification source parent",
        )
        == QUALIFICATION_SOURCE_PARENT,
        "qualification source Git parent differs",
    )
    qualification_delta = _git_name_list(
        root,
        [
            "diff-tree",
            "--no-commit-id",
        ...
        "qualification source delta",
    )
    _require(
        qualification_delta == [QUALIFICATION_CORRECTION_PATH],
        "qualification source changed-path inventory differs",
    )
    _require(
        _git_line(
            root,
            [
                "rev-parse",
                f"{QUALIFICATION_SOURCE_COMMIT}:{QUALIFICATION_CORRECTION_PATH}",
            ],
            "qualification correction blob",
        )
        == QUALIFICATION_CORRECTION_BLOB,
        "qualification correction blob differs",
    )
    ...
    _require(
        sha256_bytes(correction_bytes) == QUALIFICATION_CORRECTION_SHA256,
        "qualification correction SHA-256 differs",
    )
    qualification_ancestry = _run_git(
        root,
        ["merge-base", "--is-ancestor", QUALIFICATION_SOURCE_COMMIT, commit],
        accepted=(0, 1),
    )
    _require(
        qualification_ancestry.returncode == 0,
        "qualification source is not an ancestor of qualified source",
    )
    ...
    validate_release_closeout_paths(closeout)
...
def validate_v04_unchanged(root: Path, inherited: Mapping[str, Any]) -> None:
    _exact_keys(
        inherited,
        (
            "classification", "supports", "direct_v05_proof", "accepted_tag_ref",
            "accepted_tag_object", "accepted_commit", "accepted_artifact_tree",
        ),
        "inherited_v04",
    )
    _require(inherited.get("classification") == "INHERITED_SUPPORT_ONLY_NOT_DIRECT_V05_PROOF", "v0.4 evidence classification differs")
    _require(inherited.get("supports") == ["compatibility", "browser", "performance"], "v0.4 support scope differs")
    _require(inherited.get("direct_v05_proof") is False, "v0.4 evidence is misclassified as direct v0.5 proof")
    expected = {
        "accepted_tag_ref": V04_TAG_REF,
        "accepted_tag_object": V04_TAG_OBJECT,
        "accepted_commit": V04_COMMIT,
        "accepted_artifact_tree": V04_ARTIFACT_TREE,
    }
    for key, value in expected.items():
        _require(inherited.get(key) == value, f"inherited_v04.{key} differs")
    _require(_git_line(root, ["show-ref", "--verify", "--hash", V04_TAG_REF], V04_TAG_REF) == V04_TAG_OBJECT, "v0.4.0 tag moved")
    _require(_git_line(root, ["rev-parse", "HEAD:artifacts/v0.4"], "HEAD v0.4 artifact tree") == V04_ARTIFACT_TREE, "accepted v0.4 artifacts changed in release HEAD")
    _require(_git_line(root, ["rev-parse", f"{V04_COMMIT}:artifacts/v0.4"], "tagged v0.4 artifact tree") == V04_ARTIFACT_TREE, "accepted v0.4 tag artifact tree differs")
    diff = _run_git(root, ["diff", "--quiet", V04_COMMIT, "--", "artifacts/v0.4"], accepted=(0, 1))
    _require(diff.returncode == 0, "accepted v0.4 artifacts have a working-tree change")


def canonical_evidence_files(root: Path, evidence_root: Path) -> dict[str, Path]:
    _require(evidence_root.is_dir() and not evidence_root.is_symlink(), "v0.5 evidence root is missing or unsafe")
    files: dict[str, Path] = {}
    for path in evidence_root.rglob("*"):
        relative_evidence = path.relative_to(evidence_root)
        relative_root = path.relative_to(root).as_posix()
        if path.is_symlink():
            raise ReleaseEvidenceError(f"v0.5 evidence contains a symlink: {relative_root}")
        ...
        if path.name.startswith("."):
            raise ReleaseEvidenceError(f"v0.5 canonical evidence contains a temporary file: {relative_root}")
        if path.name.endswith((".tar.gz", ".tar.gz.sha256")):
            _require(
                relative_root in RELEASE_PACKAGE_PATHS,
                f"v0.5 evidence contains a noncanonical package output: {relative_root}",
            )
            continue
        _require(path.stat().st_size <= MAX_EVIDENCE_FILE_BYTES, f"v0.5 evidence file is oversized: {relative_root}")
        files[relative_root] = path
    _require(
        set(files) == CANONICAL_EVIDENCE_PATHS,
        "canonical v0.5 evidence file inventory differs",
    )
    return dict(sorted(files.items()))


def evidence_fingerprint_v05(files: Mapping[str, Path]) -> str:
...
def validate_evidence_inventory(
    ...
    _require(set(declared) == set(files), "canonical v0.5 evidence file inventory differs")
...
def validate_work_package(
    ...
    _require(
        tuple(TOOLING_SYNTHETIC_NODES) == TOOLING_SECRET_CANARY_NODES,
        "candidate/final tooling secret-canary contracts differ",
    )

    result = validate_candidate_evidence(root)
    _require(result.candidate_commit == CANDIDATE_COMMIT, "candidate validator returned a different commit")
    _require(result.suite_count == 4 and result.identity_count == 6, "candidate validator result cardinality differs")
...
def validate_toolchain(root: Path, toolchain: Mapping[str, Any]) -> None:
    from scripts.validate_candidate_evidence_v05 import validate_toolchain as validate_candidate_toolchain

    validate_candidate_toolchain(root, toolchain)
...
def _validate_suite_manifest(
    ...
    expected_path = f"artifacts/v0.5/final/tests/{suite_id.replace('_', '-')}.{extension}"
...
def validate_final_suites(
    ...
    _require(sqlite_nodes == postgres_nodes, "SQLite and PostgreSQL final backend inventories differ")
    ...
    *,
    ...
    _exact_keys(
        ...
        (
            ...
            "toolchain", "suites", "teardown", "accepted_exceptions", "overall_pass",
        ),
        "final qualification summary",
    )
    _require(
        summary.get("schema_version") == "spell.v05.final-qualification/1",
...
def _parse_sha256sums(path: Path, expected_names: set[str]) -> dict[str, str]:
    _require(path.is_file() and not path.is_symlink(), "SBOM SHA256SUMS is missing or unsafe")
    try:
...
def validate_sbom(
    ...
    _require(sbom.get("directory") == "artifacts/v0.5/sbom", "SBOM directory differs")
    _require(sbom.get("checksum_manifest") == "artifacts/v0.5/sbom/SHA256SUMS", "SBOM checksum path differs")
    ...
    _require(directory.is_dir() and not directory.is_symlink(), "SBOM directory is missing or unsafe")
    discovered = {path.name for path in directory.iterdir() if path.is_file() and not path.is_symlink()}
...
def validate_supply_chain(
    ...
    _exact_keys(
        ...
        (
            ...
            "accepted_v04_artifacts_unchanged",
        ),
        "supply_chain",
    )
    expected_path = "artifacts/v0.5/supply-chain.json"
    _require(supply.get("capture") == expected_path, "supply-chain capture path differs")
    _require(supply.get("test_id") == "V05-SC-001" and supply.get("passed") is True, "supply-chain result did not pass")
    ...
    _require(supply.get("accepted_v04_artifacts_unchanged") is True, "supply-chain audit did not preserve v0.4 artifacts")
    ...
    _require(capture.get("schema_version") == "spell.v05.supply-chain/1", "supply-chain schema differs")
    ...
    _require(capture.get("test_id") == "V05-SC-001" and capture.get("passed") is True, "supply-chain capture did not pass")
    _require(capture.get("source_fingerprint_sha256") == source_fingerprint, "supply-chain capture source differs")
    _require(capture.get("accepted_v04_artifacts_unchanged") is True, "supply-chain capture changed v0.4 artifacts")
...
def validate_release_tag(
    ...
    *,
    ...
    _require(
        ...
        "forbidden lightweight or secondary v0.5 tag exists",
    )
...
def validate_release_manifest_shape(manifest: Mapping[str, Any]) -> None:
    _exact_keys(
        ...
        (
            ...
            "work_package", "gate_0b", "toolchain", "final_qualification", "final_suites", "inherited_v04",
...
def validate_release_evidence(
    ...
    *,
...
) -> ReleaseEvidenceValidationV05:
    source_root = root.resolve()
    canonical_root = (evidence_root or (source_root / "artifacts/v0.5")).resolve()
    _require(canonical_root == (source_root / "artifacts/v0.5").resolve(), "release evidence root must be canonical artifacts/v0.5")
    ...
    qualified_source, release_head = validate_source_git(
        source_root, _mapping(manifest["qualified_source"], "qualified_source")
    )
    from scripts.source_fingerprint_v05 import source_fingerprint_v05
    from scripts.build_reproducible_v05 import product_package_sha256_v05

    source_fingerprint = source_fingerprint_v05(source_root)
    declared_source = manifest["qualified_source"]["source_fingerprint_sha256"]
    _require(source_fingerprint == declared_source, "current v0.5 source fingerprint differs")
    product_package = product_package_sha256_v05(source_root)
    ...
    validate_v04_unchanged(source_root, _mapping(manifest["inherited_v04"], "inherited_v04"))
    ...
    return ReleaseEvidenceValidationV05(
...
def validate_release_evidence_v05(
    root: Path,
    evidence_root: Path | None = None,
    *,
    require_tag: bool = False,
    ...
    return validate_release_evidence(root, evidence_root, require_tag=require_tag)
