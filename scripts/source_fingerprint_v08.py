"""Compute the source fingerprint for the bounded SPELL v0.8 release."""
...
def is_link_or_reparse_v08(path: Path) -> bool:
...
def path_has_link_or_reparse_v08(root: Path, path: Path) -> bool:
    ...
    try:
        ...
        return True
    cursor = lexical_root
    if is_link_or_reparse_v08(cursor):
        return True
...
def source_fingerprint_inputs_v08(root: Path) -> list[Path]:
    ...
    for relative_tree in FINGERPRINT_TREES:
        ...
        if not tree.is_dir() or path_has_link_or_reparse_v08(source_root, tree):
            raise FileNotFoundError(
                f"required v0.8 fingerprint tree is missing: {relative_tree}"
            )
        for path in tree.rglob("*"):
            ...
            if path_has_link_or_reparse_v08(source_root, path):
                raise ValueError(
                    "v0.8 fingerprint input must not be a link or reparse point: "
                    f"{relative.as_posix()}"
                )
            if path.is_file():
                paths.append(path)
    ...
    for relative_file in FINGERPRINT_FILES:
        ...
        if not path.is_file() or path_has_link_or_reparse_v08(source_root, path):
            raise FileNotFoundError(
                f"required v0.8 fingerprint file is missing: {relative_file}"
            )
        paths.append(path)
...
def source_fingerprint_v08(root: Path) -> str:
    source_root = root.resolve()
    digest = hashlib.sha256()
    for path in source_fingerprint_inputs_v08(source_root):
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(source_fingerprint_v08(args.root.resolve()))
    return 0
