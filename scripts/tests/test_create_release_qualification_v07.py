...
from scripts import create_release_qualification_v07 as producer
from scripts import validate_release_evidence_v07 as release


def _source() -> producer.QualifiedSourceV07:
    return producer.QualifiedSourceV07(
        commit="a" * 40,
        tree="b" * 40,
        parent="c" * 40,
...
def test_sbom_declaration_is_derived_from_checksum_subject_and_image(
    ...
    directory = tmp_path / "artifacts/v0.7/sbom"
...
def test_supply_chain_declaration_uses_canonical_capture_metrics(
    ...
    source = "d" * 64
    path = tmp_path / "artifacts/v0.7/supply-chain.json"
...
def test_manifest_builder_binds_all_release_primitives(
    ...
    monkeypatch.setattr(producer, "source_fingerprint_v07", lambda root: fingerprint)
    monkeypatch.setattr(producer, "product_package_sha256_v07", lambda root: product)
    monkeypatch.setattr(
        producer,
    ...
    manifest = producer.build_release_qualification_v07(tmp_path)
...
def test_canonical_writer_is_atomic_idempotent_and_refuses_replacement(
    ...
    monkeypatch.setattr(
        producer, "build_release_qualification_v07", lambda root: manifest
    )
    monkeypatch.setattr(
        producer,
        "_assert_generation_git_state",
    ...
    first = producer.create_release_qualification_v07(tmp_path)
    second = producer.create_release_qualification_v07(tmp_path)
    ...
    with pytest.raises(
        ...
        producer.create_release_qualification_v07(tmp_path)
