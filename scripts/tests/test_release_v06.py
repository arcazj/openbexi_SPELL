...
from scripts import build_reproducible_v06 as release


def test_v06_reparse_detector_recognizes_windows_file_attributes(
    ...
    assert release.path_has_link_or_reparse_v06(Path("root"), Path("root/file"))


def test_v06_archive_bytes_are_deterministic_and_normalized(tmp_path: Path) -> None:
...
def test_v06_package_rejects_forbidden_paths(relative: str) -> None:
    with pytest.raises(ValueError):
        release._validate_path(Path(relative))


def test_v06_package_rejects_private_key_bytes(tmp_path: Path) -> None:
...
def test_v06_scanner_literal_contract_matches_every_live_source_file() -> None:
...
def test_v06_scanner_literal_contract_rejects_path_and_byte_mutations(
    relative: str,
) -> None:
    raw = (release.ROOT / relative).read_bytes()
...
def test_current_v06_product_package_fingerprint_is_constructible() -> None:
    missing = [
        relative
    ...
    if missing:
        with pytest.raises(FileNotFoundError):
            release.product_package_sha256_v06(release.ROOT)
        return
    digest = release.product_package_sha256_v06(release.ROOT)
    assert len(digest) == 64
    assert set(digest) <= set("0123456789abcdef")


def test_v06_fingerprint_image_uses_a_current_source_context_policy() -> None:
    dockerfile = (release.ROOT / "scripts/package-v06.Dockerfile").read_text(
        encoding="utf-8"
    )
    ignore = (
        release.ROOT / "scripts/package-v06.Dockerfile.dockerignore"
    ).read_text(encoding="utf-8")

    assert "COPY . /workspace" in dockerfile
    assert "artifacts/v0.6" in ignore.splitlines()
    assert "SPELL-DOCUMENTATION" in ignore.splitlines()
    assert "NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI" not in ignore
    assert "SPELL_v0.6_Gate_0B.md" not in ignore


def test_v06_package_publication_is_a_rollback_transaction() -> None:
    package = (release.ROOT / "scripts/package_release_v06.ps1").read_text(
        encoding="utf-8"
    )

    assert "existing v0.6 package publication is incomplete" in package
    assert "$backupRelease" in package and "$backupSidecar" in package
    assert "refusing to use unsafe v0.6 package publication path" in package
    assert "published v0.6 package pair failed final verification" in package
    ...
    final_verification = package.index(
        "published v0.6 package pair failed final verification"
    )
    ...
    function_cleanup = package[package.index("function Publish-V06PackagePair") :]
...
def test_v06_package_build_outputs_use_owned_qualification_scratch() -> None:
    package = (release.ROOT / "scripts/package_release_v06.ps1").read_text(
        encoding="utf-8"
    )
    ...
    assert "v0.6 package scratch path already exists" in package
    assert "v0.6 package scratch directory is unsafe" in package
    assert "refusing v0.6 package scratch cleanup containing a reparse point" in package
...
def test_v06_package_publication_fault_rolls_back_executably(
    tmp_path: Path,
) -> None:
    ...
    package_script = release.ROOT / "scripts/package_release_v06.ps1"
...
$definitions = @($ast.FindAll({
  ...
  $node -is [Management.Automation.Language.FunctionDefinitionAst] -and
    $node.Name -ceq "Publish-V06PackagePair"
}, $true))
...
try {
  Publish-V06PackagePair `
    ...
    -Release (Join-Path $WorkRoot "openbexi-spell-v0.6.0.tar.gz") `
    -Sidecar (Join-Path $WorkRoot "openbexi-spell-v0.6.0.tar.gz.sha256") `
...
if ($caught -cne "injected v0.6 package publication failure") {
...
''',
    ...
    for existing_pair in (False, True):
        ...
        release_path = work_root / "openbexi-spell-v0.6.0.tar.gz"
        sidecar_path = work_root / "openbexi-spell-v0.6.0.tar.gz.sha256"
        if existing_pair:
