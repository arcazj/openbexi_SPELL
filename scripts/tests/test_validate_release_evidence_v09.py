...
from scripts import validate_release_evidence_v09 as release
...
def _inherited_v08() -> dict:
    return {
        "classification": "ACCEPTED_RELEASE_BASELINE_NOT_DIRECT_V09_PROOF",
        "supports": ["release-lineage", "compatibility", "toolchain"],
        "direct_v09_proof": False,
        "accepted_tag_ref": release.V08_TAG_REF,
        "accepted_tag_object": release.V08_TAG_OBJECT,
        "accepted_commit": release.V08_COMMIT,
        "accepted_artifact_tree": release.V08_ARTIFACT_TREE,
        "accepted_archive_path": release.V08_ARCHIVE_RELATIVE,
        "accepted_archive_sha256": release.V08_ARCHIVE_SHA256,
        "accepted_sidecar_path": release.V08_SIDECAR_RELATIVE,
        "accepted_sidecar_sha256": release.V08_SIDECAR_SHA256,
        "accepted_tag_archive_claim": release.V08_TAG_ARCHIVE_CLAIM,
    }


def test_v09_inherited_v08_binding_includes_external_archive_sidecar_and_tag() -> None:
    inherited = _inherited_v08()
    release.validate_v08_unchanged(release.ROOT, inherited)
    ...
    for key in (
        ...
        mutation[key] = "0" * 64
        with pytest.raises(release.ReleaseEvidenceError, match=f"inherited_v08.{key}"):
            release.validate_v08_unchanged(release.ROOT, mutation)
...
def test_validator_direct_path_bootstraps_repo_imports_under_isolated_python(
    ...
    code = (
        ...
        "import scripts.validate_candidate_evidence_v09 as candidate;"
    ...
    assert direct.stdout.startswith("usage: validate_release_evidence_v09.py ")
    ...
    assert completed.stdout.strip() == (
        "spell.v09.release-qualification/1 "
        "scripts.validate_candidate_evidence_v09"
    )
...
def test_tooling_suite_requires_the_exact_secret_canary_nodes(tmp_path: Path) -> None:
    path = tmp_path / "artifacts/v0.9/final/tests/tooling.xml"
    cases: list[tuple[str, str, str]] = []
    required_nodes = (
        *release.TOOLING_SECRET_CANARY_NODES,
        release.OFFLINE_PACKAGE_NODE,
    )
    ...
    suite = {
        "kind": "junit",
        "capture": "artifacts/v0.9/final/tests/tooling.xml",
    ...
    offline_index = required_nodes.index(release.OFFLINE_PACKAGE_NODE)
    missing_offline = cases[:offline_index] + cases[offline_index + 1 :]
    _write_junit(path, missing_offline, reported_tests=len(missing_offline) + 36)
    suite["sha256"] = hashlib.sha256(path.read_bytes()).hexdigest()
    suite["test_count"] = len(missing_offline)
    suite["passed_count"] = len(missing_offline)
    with pytest.raises(release.ReleaseEvidenceError, match="offline package proof"):
        release._validate_suite_manifest("tooling", suite, tmp_path, "a" * 64)


def test_mocked_browser_requires_the_inherited_v07_operator_workflow(
    tmp_path: Path,
) -> None:
    path = tmp_path / "artifacts/v0.9/final/tests/browser-mocked.xml"
    cases = [
        (
        ...
        *[
            (
                f"browser_{project}.development-workspace.spec.ts",
                name,
                "passed",
            )
            for project in ("chromium", "mobile")
            for name in release.DEVELOPMENT_MOCKED_TEST_NAMES
        ],
    ]
    _write_junit(path, cases)
    suite = {
        "kind": "junit",
        "capture": "artifacts/v0.9/final/tests/browser-mocked.xml",
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "test_count": len(cases),
        "subtest_count": 0,
        "passed_count": len(cases),
    ...
    renamed = [
        (classname, name.replace("v0.6", "v0.9"), status)
        for classname, name, status in cases
    ]
...
def test_real_browser_requires_the_development_workflow_on_both_projects(
    tmp_path: Path,
) -> None:
    path = tmp_path / "artifacts/v0.9/final/tests/browser-real.xml"
    cases = [
        *[
            (
                f"browser_{project}.integration.spec.ts",
                "controls a durable prompt workflow and loads its report",
                "passed",
            )
            for project in ("chromium", "mobile")
        ],
        *[
            (
                f"browser_{project}.telemetry-observation-real.spec.ts",
        ...
        *[
            (
                f"browser_{project}.development-workspace-real.spec.ts",
                name,
                "passed",
            )
            for project in ("chromium", "mobile")
            for name in release.DEVELOPMENT_REAL_TEST_NAMES
        ],
    ]
    _write_junit(path, cases)
    suite = {
        "kind": "junit",
        "capture": "artifacts/v0.9/final/tests/browser-real.xml",
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "test_count": len(cases),
        "subtest_count": 0,
        "passed_count": len(cases),
    ...
    result, count = release._validate_suite_manifest(
        "browser_real", suite, tmp_path, "a" * 64
    )
    assert result is not None and count == len(cases)

    _write_junit(path, cases[:-1])
    suite.update(
        sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        test_count=len(cases) - 1,
        passed_count=len(cases) - 1,
    )
    with pytest.raises(release.ReleaseEvidenceError, match="acceptance workflow"):
        release._validate_suite_manifest("browser_real", suite, tmp_path, "a" * 64)
...
def test_vitest_requires_every_suite_and_assertion_to_pass(tmp_path: Path) -> None:
    ...
    noncanonical = _vitest_capture()
    noncanonical["testResults"][0]["name"] = str(
        tmp_path / "frontend/src/release.test.ts"
    )
    path.write_text(json.dumps(noncanonical), encoding="utf-8")
    with pytest.raises(release.ReleaseEvidenceError, match="path is not canonical"):
        release.parse_vitest(path, "vitest")


def test_frontend_build_capture_is_source_and_dist_bound(tmp_path: Path) -> None:
    source = "a" * 64
    capture_path = tmp_path / "artifacts/v0.9/final/tests/frontend-build.json"
    ...
    suite = {
        "kind": "frontend-build",
        "capture": "artifacts/v0.9/final/tests/frontend-build.json",
...
def test_final_qualification_summary_is_hash_source_suite_and_teardown_bound(
    ...
    summary = {
        ...
        "suites": suites,
        "accepted_v08_release": copy.deepcopy(release.V08_EXTERNAL_RELEASE_BINDING),
        "teardown": {
...
def _write_version_fixture(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        '[project]\nversion = "0.9.0"\n'
        '[tool.spell.supply-chain]\nrelease_artifact_root = "artifacts/v0.9"\n'
...
def test_candidate_delta_allowlist_excludes_product_and_dependency_changes() -> None:
    ...
    assert release._allowed_candidate_path("scripts/package-v09.Dockerfile.dockerignore")
    assert release._allowed_candidate_path("scripts/create_release_qualification_v09.py")
    assert release._allowed_candidate_path("scripts/qualification-v09.Dockerfile.dockerignore")
    assert release._allowed_candidate_path("scripts/tests/test_qualify_release_v09.py")
    assert release._allowed_candidate_path("artifacts/v0.9/work-package/qualification.json")
...
def test_release_closeout_allows_only_the_complete_canonical_package_pair() -> None:
    evidence = ["artifacts/v0.9/release-qualification.json"]
    ...
    with pytest.raises(release.ReleaseEvidenceError, match="noncanonical package"):
        release.validate_release_closeout_paths(
            [*evidence, "artifacts/v0.9/unapproved.tar.gz"]
        )
    ...
    with pytest.raises(release.ReleaseEvidenceError, match="scratch evidence"):
        release.validate_release_closeout_paths(
            [*evidence, "artifacts/v0.9/.qualification/run.json"]
        )
...
def test_evidence_inventory_is_exact_hash_bound_and_secret_scanned(
    ...
    evidence_root = tmp_path / "artifacts/v0.9"
    assert {
    ...
    } <= release.CANONICAL_EVIDENCE_PATHS
    files: dict[str, Path] = {}
    ...
    declared = {
        ...
        "evidence_fingerprint_sha256": release.evidence_fingerprint_v09(files),
    }
    ...
    declared["evidence_fingerprint_sha256"] = release.evidence_fingerprint_v09(files)
...
def test_tooling_secret_testcases_are_exact_structured_exemptions() -> None:
    from scripts.validate_candidate_evidence_v09 import TOOLING_SYNTHETIC_NODES
...
def test_backend_secret_testcases_are_exact_structured_exemptions() -> None:
    from scripts.validate_candidate_evidence_v09 import (
...
def test_backend_secret_testcases_reject_duplicates_mutations_and_mislocation() -> None:
    relative = "artifacts/v0.9/final/tests/backend-postgresql.xml"
    ...
    misplaced = "artifacts/v0.9/final/tests/backend-docker-host.xml"
...
def test_tooling_secret_testcases_reject_duplicates_mutations_and_mislocation() -> None:
    relative = "artifacts/v0.9/final/tests/tooling.xml"
...
def test_tooling_xml_enforces_builder_token_patterns_outside_exempt_names() -> None:
    from scripts import build_reproducible_v09 as builder
...
def test_evidence_fingerprint_is_path_and_byte_sensitive(tmp_path: Path) -> None:
    ...
    a = release.evidence_fingerprint_v09({"a.txt": first})
    b = release.evidence_fingerprint_v09({"b.txt": second})
    assert a != b
    first.write_bytes(b"changed")
    assert release.evidence_fingerprint_v09({"a.txt": first}) != a
...
def test_v09_sbom_subjects_are_version_scoped_in_producer_and_validator() -> None:
    assert release.SBOM_SUBJECTS == {
    ...
    producer = (root / "scripts/generate_sbom_v09.ps1").read_text(encoding="utf-8")
    for subject in release.SBOM_SUBJECTS.values():
        assert producer.count(f'Subject = "{subject}"') == 1
    assert "openbexi_spell-backend:0.8" not in producer
...
def _write_sboms(root: Path, source: str) -> dict:
    ...
    return {
        "directory": "artifacts/v0.9/sbom",
        "checksum_manifest": "artifacts/v0.9/sbom/SHA256SUMS",
...
def test_sbom_set_is_exact_distinct_checksum_and_source_bound(tmp_path: Path) -> None:
    ...
    backend = tmp_path / "artifacts/v0.9/sbom/backend.cdx.json"
...
def _write_supply_capture(root: Path, source: str) -> dict:
    path = root / "artifacts/v0.9/supply-chain.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    capture = {
    ...
    return {
        "capture": "artifacts/v0.9/supply-chain.json",
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "test_id": "V09-SC-001",
        "passed": True,
        "source_fingerprint_sha256": source,
...
def test_supply_chain_requires_zero_high_critical_and_unlocked_inputs(
    ...
    declaration = _write_supply_capture(tmp_path, source)
    declaration["accepted_v08_release"]["archive_sha256"] = "0" * 64
    with pytest.raises(
        release.ReleaseEvidenceError,
        match="accepted v0.8 release binding differs",
    ):
        release.validate_supply_chain(tmp_path, declaration, source)
...
def _tag_policy() -> dict:
    return {
        "tag_name": "v0.9.0",
        "tag_ref": "refs/tags/v0.9.0",
        "object_type": "tag",
        "target_policy": "CURRENT_RELEASE_HEAD",
        "package_path": "artifacts/v0.9/openbexi-spell-v0.9.0.tar.gz",
        "sidecar_path": "artifacts/v0.9/openbexi-spell-v0.9.0.tar.gz.sha256",
...
def test_tag_message_producer_is_exact_ordered_and_rejects_any_mutation() -> None:
    ...
    expected = release.create_release_tag_message_v09(policy, **values)
    ...
    release.validate_release_tag_message_v09(expected.encode("utf-8"), expected)

    reordered = lines.copy()
    reordered[2], reordered[3] = reordered[3], reordered[2]
    mutations = (
        expected.replace(release.RELEASE_TAG_TITLE, "SPELL v0.9", 1),
        "\n".join(reordered) + "\n",
    ...
    for mutation in mutations:
        with pytest.raises(release.ReleaseEvidenceError, match="exact ordered"):
            release.validate_release_tag_message_v09(
...
def test_require_tag_validates_real_annotated_object_target_markers_and_sidecar(
    ...
    package = tmp_path / "artifacts/v0.9/openbexi-spell-v0.9.0.tar.gz"
    ...
    work = "f" * 64
    policy = _tag_policy()
    expected_message = release.create_release_tag_message_v09(
        policy,
        release_head=head,
    ...
    _git(tmp_path, "tag", "-a", "v0.9.0", "-F", str(message))
    ...
    _git(tmp_path, "tag", "v0.9")
    with pytest.raises(release.ReleaseEvidenceError, match="secondary v0.9 tag"):
        release.validate_release_tag(
            tmp_path,
            policy,
            release_head=head,
...
def test_release_manifest_shape_rejects_exceptions_claims_and_unknown_fields() -> None:
    ...
    manifest = _manifest_shape()
    manifest["broader_v09_scope"] = True
...
def test_sqlite_skip_contract_is_exact_and_other_suites_have_no_allowlist() -> None:
    assert len(release.SQLITE_ALLOWED_SKIPS) == 18
    assert sum(
        "test_v0007_postgresql_schema_and_fingerprint_match_sqlite_contract" in node
        for node in release.SQLITE_ALLOWED_SKIPS
    ) == 1
    assert sum("test_migrations.py" in node for node in release.SQLITE_ALLOWED_SKIPS) == 5
    assert sum("test_driver_isolation.py" in node for node in release.SQLITE_ALLOWED_SKIPS) == 3
    assert sum("test_development_postgresql_v09.py" in node for node in release.SQLITE_ALLOWED_SKIPS) == 2
    ...
    assert release.FINAL_SUITE_INVENTORIES == {
...
def test_builder_compatible_api_returns_versioned_entry_point() -> None:
    ...
    assert callable(release.validate_release_evidence_v09)
    assert release.validate_release_evidence_v09.__doc__


def test_builder_is_the_only_explicit_prepublication_package_exception() -> None:
    source = (release.ROOT / "scripts/build_reproducible_v09.py").read_text(
        encoding="utf-8"
    )
