...
from scripts import build_reproducible_v09 as release


def test_v09_reparse_detector_recognizes_windows_file_attributes(
    ...
    assert release.path_has_link_or_reparse_v09(Path("root"), Path("root/file"))


def test_v09_archive_bytes_are_deterministic_and_normalized(tmp_path: Path) -> None:
...
def test_v09_package_rejects_forbidden_paths(relative: str) -> None:
    with pytest.raises(ValueError):
        release._validate_path(Path(relative))


def test_v09_package_omits_only_path_scoped_browser_screenshot_evidence() -> None:
    assert release._is_generated_browser_screenshot(
        Path("artifacts/v0.9/work-package/browser/v09-development-real-chromium.png")
    )
    assert release._is_generated_browser_screenshot(
        Path("artifacts/v0.9/work-package/browser/v09-development-real-mobile.png")
    )
    assert release._is_generated_browser_screenshot(
        Path(
            "artifacts/v0.9/work-package/browser/"
            "v09-development-real-continuity-chromium.png"
        )
    )
    assert release._is_generated_browser_screenshot(
        Path(
            "artifacts/v0.9/work-package/browser/"
            "v09-development-real-continuity-mobile.png"
        )
    )
    assert release._is_generated_browser_screenshot(
        Path("artifacts/v0.9/work-package/browser/development-desktop.png")
    )
    assert release._is_generated_browser_screenshot(
        Path("artifacts/v0.9/work-package/browser/development-mobile.webp")
    )
    assert not release._is_generated_browser_screenshot(
        Path("artifacts/v0.9/work-package/browser/development-desktop.json")
    )
    assert not release._is_generated_browser_screenshot(
        Path("NEW_SPELL_DOCUMENTATION_GENERATED_BY_AI/web/assets/edit-workspace.png")
    )
    assert not release._is_generated_browser_screenshot(
        Path("frontend/src/assets/development-environment.png")
    )


def test_v09_package_rejects_private_key_bytes(tmp_path: Path) -> None:
...
def test_v09_scanner_literal_contract_matches_every_live_source_file() -> None:
...
def test_v09_scanner_literal_contract_rejects_path_and_byte_mutations(
    relative: str,
) -> None:
    raw = (release.ROOT / relative).read_bytes()
...
def _evidence_canary_xml(
    ...
    from scripts import validate_release_evidence_v09 as evidence
...
def test_v09_package_scans_candidate_manifest_and_tooling_structurally() -> None:
    from scripts import validate_release_evidence_v09 as evidence
    ...
    release._validate_bytes(
        Path("artifacts/v0.9/work-package/tests/tooling.xml"),
        _evidence_canary_xml(evidence.FINAL_TOOLING_SECRET_TESTCASE_NODES),
    )
    release._validate_bytes(
        Path("artifacts/v0.9/final/tests/backend-postgresql.xml"),
        _evidence_canary_xml(evidence.BACKEND_SECRET_CANARY_NODES),
    )


def test_v09_package_rejects_duplicate_or_mislocated_evidence_canaries() -> None:
    from scripts import validate_release_evidence_v09 as evidence

    tooling_relative = "artifacts/v0.9/work-package/tests/tooling.xml"
    ...
    with pytest.raises(ValueError, match="secret material"):
        release._validate_bytes(
            Path("artifacts/v0.9/final/tests/not-tooling.xml"),
            _evidence_canary_xml(evidence.FINAL_TOOLING_SECRET_TESTCASE_NODES),
        )

    backend_relative = "artifacts/v0.9/final/tests/backend-postgresql.xml"
    with pytest.raises(ValueError, match="inventory differs"):
        release._validate_bytes(
    ...
    with pytest.raises(ValueError, match="secret material"):
        release._validate_bytes(
            Path("artifacts/v0.9/final/tests/backend-docker-host.xml"),
            _evidence_canary_xml(evidence.BACKEND_SECRET_CANARY_NODES),
        )


def test_current_v09_product_package_fingerprint_is_constructible() -> None:
    missing = [
        relative
    ...
    if missing:
        with pytest.raises(FileNotFoundError):
            release.product_package_sha256_v09(release.ROOT)
        return
    digest = release.product_package_sha256_v09(release.ROOT)
    assert len(digest) == 64
    assert set(digest) <= set("0123456789abcdef")


def test_v09_fingerprint_image_uses_a_current_source_context_policy() -> None:
    dockerfile = (release.ROOT / "scripts/package-v09.Dockerfile").read_text(
        encoding="utf-8"
    )
    ignore = (
        release.ROOT / "scripts/package-v09.Dockerfile.dockerignore"
    ).read_text(encoding="utf-8")

    assert "COPY . /workspace" in dockerfile
    ignore_lines = ignore.splitlines()
    assert "artifacts/v0.9" in ignore_lines
    ...
    assert "SPELL_v0.9_Gate_0B.md" not in ignore


def test_v09_host_python_producers_require_locked_isolated_mode() -> None:
    audit = (release.ROOT / "scripts/audit_supply_chain_v09.ps1").read_text(
        encoding="utf-8"
    )
    package = (release.ROOT / "scripts/package_release_v09.ps1").read_text(
        encoding="utf-8"
    )

    assert audit.count("& $PythonExe -I ") == 2
    assert (
        '& $PythonExe -I (Join-Path $PSScriptRoot "source_fingerprint_v09.py")'
        in audit
    )
    assert (
    ...
    assert (
        '& $PythonExe -I (Join-Path $BuildRoot "scripts/build_reproducible_v09.py")'
        in package
    )
    assert "& $PythonExe (Join-Path" not in package


def test_v09_package_publication_is_a_rollback_transaction() -> None:
    package = (release.ROOT / "scripts/package_release_v09.ps1").read_text(
        encoding="utf-8"
    )

    assert "existing v0.9 package publication is incomplete" in package
    assert "$backupRelease" in package and "$backupSidecar" in package
    assert "refusing to use unsafe v0.9 package publication path" in package
    assert "published v0.9 package pair failed final verification" in package
    ...
    final_verification = package.index(
        "published v0.9 package pair failed final verification"
    )
    ...
    function_cleanup = package[package.index("function Publish-V09PackagePair") :]
...
def test_v09_package_build_outputs_use_owned_qualification_scratch() -> None:
    package = (release.ROOT / "scripts/package_release_v09.ps1").read_text(
        encoding="utf-8"
    )
    ...
    assert "v0.9 package scratch path already exists" in package
    assert "v0.9 package scratch directory is unsafe" in package
    assert "refusing v0.9 package scratch cleanup containing a reparse point" in package
    ...
    assert 'assert_accepted_v08_release_v09.ps1") -Root $export' in package
    assert "$a.release_commit -cne $releaseCommit" in package
    assert "independent_export_count = 2" in package


def test_v09_package_publication_fault_rolls_back_executably(
    tmp_path: Path,
) -> None:
    ...
    package_script = release.ROOT / "scripts/package_release_v09.ps1"
...
$definitions = @($ast.FindAll({
  ...
  $node -is [Management.Automation.Language.FunctionDefinitionAst] -and
    $node.Name -ceq "Publish-V09PackagePair"
}, $true))
...
try {
  Publish-V09PackagePair `
    ...
    -Release (Join-Path $WorkRoot "openbexi-spell-v0.9.0.tar.gz") `
    -Sidecar (Join-Path $WorkRoot "openbexi-spell-v0.9.0.tar.gz.sha256") `
...
if ($caught -cne "injected v0.9 package publication failure") {
...
''',
    ...
    for existing_pair in (False, True):
        ...
        release_path = work_root / "openbexi-spell-v0.9.0.tar.gz"
        sidecar_path = work_root / "openbexi-spell-v0.9.0.tar.gz.sha256"
        if existing_pair:
