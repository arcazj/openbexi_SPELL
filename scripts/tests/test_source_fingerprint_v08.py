...
from scripts import source_fingerprint_v08 as fingerprint
...
def test_v08_source_fingerprint_is_bounded_deterministic_and_sensitive(
    tmp_path: Path,
) -> None:
    root = _minimal_source_tree(tmp_path / "source")
    first = fingerprint.source_fingerprint_v08(root)
    second = fingerprint.source_fingerprint_v08(root)
    assert first == second
    ...
    assert fingerprint.source_fingerprint_v08(root) == first

    (root / "backend/payload.py").write_text("VALUE = 2\n", encoding="ascii")
    assert fingerprint.source_fingerprint_v08(root) != first


def test_v08_source_fingerprint_includes_gate_0a_and_contract_inputs() -> None:
    inputs = {
        path.relative_to(ROOT).as_posix()
        for path in fingerprint.source_fingerprint_inputs_v08(ROOT)
    }
    assert "SPELL_v0.8_Pre-Implementation.md" in inputs
    assert (
    ...
    assert "contracts/v08/manifest.json" in inputs
    for relative in (
...
def test_v08_source_fingerprint_excludes_transient_qualification_captures(
    tmp_path: Path,
) -> None:
    root = _minimal_source_tree(tmp_path / "source")
    baseline = fingerprint.source_fingerprint_v08(root)
    capture = root / "frontend/artifacts/v0.8/.qualification/run/browser/proof.json"
    capture.parent.mkdir(parents=True)
    capture.write_text('{"transient":true}\n', encoding="ascii")
    assert fingerprint.source_fingerprint_v08(root) == baseline


def test_v08_source_fingerprint_rejects_link_inputs(tmp_path: Path) -> None:
    root = _minimal_source_tree(tmp_path / "source")
    ...
    with pytest.raises(ValueError, match="link or reparse point"):
        fingerprint.source_fingerprint_v08(root)


def test_v08_source_fingerprint_rejects_paths_outside_root(tmp_path: Path) -> None:
    root = tmp_path / "root"
    root.mkdir()
    outside = tmp_path / "outside"
    outside.write_text("outside\n", encoding="ascii")
    assert fingerprint.path_has_link_or_reparse_v08(root, outside)
