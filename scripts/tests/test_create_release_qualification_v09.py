...
from scripts import create_release_qualification_v09 as producer
from scripts import validate_release_evidence_v09 as release


def _source() -> producer.QualifiedSourceV09:
    return producer.QualifiedSourceV09(
        commit="a" * 40,
        tree="b" * 40,
        parent="c" * 40,
...
def test_sbom_declaration_is_derived_from_checksum_subject_and_image(
    ...
    directory = tmp_path / "artifacts/v0.9/sbom"
...
def test_supply_chain_declaration_uses_canonical_capture_metrics(
    ...
    source = "d" * 64
    path = tmp_path / "artifacts/v0.9/supply-chain.json"
    ...
    assert declaration["accepted_v08_artifacts_unchanged"] is True
...
def test_manifest_builder_binds_all_release_primitives(
    ...
    monkeypatch.setattr(producer, "source_fingerprint_v09", lambda root: fingerprint)
    monkeypatch.setattr(producer, "product_package_sha256_v09", lambda root: product)
    monkeypatch.setattr(
        producer,
    ...
    manifest = producer.build_release_qualification_v09(tmp_path)
...
def test_canonical_writer_is_atomic_idempotent_and_refuses_replacement(
    ...
    monkeypatch.setattr(
        producer, "build_release_qualification_v09", lambda root: manifest
    )
    monkeypatch.setattr(
        producer,
        "_assert_generation_git_state",
    ...
    first = producer.create_release_qualification_v09(tmp_path)
    second = producer.create_release_qualification_v09(tmp_path)
    ...
    with pytest.raises(
        ...
        producer.create_release_qualification_v09(tmp_path)
