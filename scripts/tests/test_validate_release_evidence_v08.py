...
def _inherited_v07() -> dict:
    return {
        "classification": "ACCEPTED_RELEASE_BASELINE_NOT_DIRECT_V08_PROOF",
        "supports": ["release-lineage", "compatibility", "toolchain"],
        "direct_v08_proof": False,
        "accepted_tag_ref": release.V07_TAG_REF,
        "accepted_tag_object": release.V07_TAG_OBJECT,
        "accepted_commit": release.V07_COMMIT,
        "accepted_artifact_tree": release.V07_ARTIFACT_TREE,
        "accepted_archive_path": release.V07_ARCHIVE_RELATIVE,
        "accepted_archive_sha256": release.V07_ARCHIVE_SHA256,
        "accepted_sidecar_path": release.V07_SIDECAR_RELATIVE,
        "accepted_sidecar_sha256": release.V07_SIDECAR_SHA256,
        "accepted_tag_archive_claim": release.V07_TAG_ARCHIVE_CLAIM,
    }


def test_v08_inherited_v07_binding_includes_external_archive_sidecar_and_tag() -> None:
    inherited = _inherited_v07()
    release.validate_v07_unchanged(release.ROOT, inherited)
    ...
    for key in (
        ...
        mutation[key] = "0" * 64
        with pytest.raises(release.ReleaseEvidenceError, match=f"inherited_v07.{key}"):
            release.validate_v07_unchanged(release.ROOT, mutation)
...
def test_validator_direct_path_bootstraps_repo_imports_under_isolated_python(
    ...
    code = (
        ...
        "import scripts.validate_candidate_evidence_v08 as candidate;"
    ...
    assert direct.stdout.startswith("usage: validate_release_evidence_v08.py ")
    ...
    assert completed.stdout.strip() == (
        "spell.v08.release-qualification/1 "
        "scripts.validate_candidate_evidence_v08"
    )
...
def test_tooling_suite_requires_the_exact_secret_canary_nodes(tmp_path: Path) -> None:
    path = tmp_path / "artifacts/v0.8/final/tests/tooling.xml"
    cases: list[tuple[str, str, str]] = []
    ...
    suite = {
        "kind": "junit",
        "capture": "artifacts/v0.8/final/tests/tooling.xml",
...
def test_mocked_browser_requires_the_inherited_v07_operator_workflow(
    ...
    path = tmp_path / "artifacts/v0.8/final/tests/browser-mocked.xml"
    cases = [
        (
    ...
    suite = {
        "kind": "junit",
        "capture": "artifacts/v0.8/final/tests/browser-mocked.xml",
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "test_count": len(cases),
        "subtest_count": 0,
        "passed_count": len(cases),
...
def test_frontend_build_capture_is_source_and_dist_bound(tmp_path: Path) -> None:
    source = "a" * 64
    capture_path = tmp_path / "artifacts/v0.8/final/tests/frontend-build.json"
    ...
    suite = {
        "kind": "frontend-build",
        "capture": "artifacts/v0.8/final/tests/frontend-build.json",
...
def test_final_qualification_summary_is_hash_source_suite_and_teardown_bound(
    ...
    summary = {
        ...
        "suites": suites,
        "accepted_v07_release": copy.deepcopy(release.V07_EXTERNAL_RELEASE_BINDING),
        "teardown": {
...
def _write_version_fixture(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        '[project]\nversion = "0.8.0"\n'
        '[tool.spell.supply-chain]\nrelease_artifact_root = "artifacts/v0.8"\n'
        'release_toolchain_lock = "scripts/release-toolchain-v04.json"\n',
        encoding="utf-8",
    )
    (root / "frontend/package.json").write_text(
        '{"version":"0.8.0"}', encoding="utf-8"
    )
    (root / "frontend/package-lock.json").write_text(
        '{"version":"0.8.0","packages":{"":{"version":"0.8.0"}}}',
        encoding="utf-8",
    )
...
def test_candidate_delta_allowlist_excludes_product_and_dependency_changes() -> None:
    ...
    assert release._allowed_candidate_path("scripts/package-v08.Dockerfile.dockerignore")
    assert release._allowed_candidate_path("scripts/create_release_qualification_v08.py")
    assert release._allowed_candidate_path("scripts/qualification-v08.Dockerfile.dockerignore")
    assert release._allowed_candidate_path("scripts/tests/test_qualify_release_v08.py")
    assert release._allowed_candidate_path("artifacts/v0.8/work-package/qualification.json")
...
def test_release_closeout_allows_only_the_complete_canonical_package_pair() -> None:
    evidence = ["artifacts/v0.8/release-qualification.json"]
    ...
    with pytest.raises(release.ReleaseEvidenceError, match="noncanonical package"):
        release.validate_release_closeout_paths(
            [*evidence, "artifacts/v0.8/unapproved.tar.gz"]
        )
    ...
    with pytest.raises(release.ReleaseEvidenceError, match="scratch evidence"):
        release.validate_release_closeout_paths(
            [*evidence, "artifacts/v0.8/.qualification/run.json"]
        )
...
def test_evidence_inventory_is_exact_hash_bound_and_secret_scanned(
    ...
    evidence_root = tmp_path / "artifacts/v0.8"
    files: dict[str, Path] = {}
    ...
    declared = {
        ...
        "evidence_fingerprint_sha256": release.evidence_fingerprint_v08(files),
    }
    ...
    declared["evidence_fingerprint_sha256"] = release.evidence_fingerprint_v08(files)
...
def test_tooling_secret_testcases_are_exact_structured_exemptions() -> None:
    from scripts.validate_candidate_evidence_v08 import TOOLING_SYNTHETIC_NODES
...
def test_backend_secret_testcases_are_exact_structured_exemptions() -> None:
    from scripts.validate_candidate_evidence_v08 import (
...
def test_backend_secret_testcases_reject_duplicates_mutations_and_mislocation() -> None:
    relative = "artifacts/v0.8/final/tests/backend-postgresql.xml"
    ...
    misplaced = "artifacts/v0.8/final/tests/backend-docker-host.xml"
...
def test_tooling_secret_testcases_reject_duplicates_mutations_and_mislocation() -> None:
    relative = "artifacts/v0.8/final/tests/tooling.xml"
...
def test_tooling_xml_enforces_builder_token_patterns_outside_exempt_names() -> None:
    from scripts import build_reproducible_v08 as builder
...
def test_evidence_fingerprint_is_path_and_byte_sensitive(tmp_path: Path) -> None:
    ...
    a = release.evidence_fingerprint_v08({"a.txt": first})
    b = release.evidence_fingerprint_v08({"b.txt": second})
    assert a != b
    first.write_bytes(b"changed")
    assert release.evidence_fingerprint_v08({"a.txt": first}) != a
...
def _write_sboms(root: Path, source: str) -> dict:
    ...
    return {
        "directory": "artifacts/v0.8/sbom",
        "checksum_manifest": "artifacts/v0.8/sbom/SHA256SUMS",
...
def test_sbom_set_is_exact_distinct_checksum_and_source_bound(tmp_path: Path) -> None:
    ...
    backend = tmp_path / "artifacts/v0.8/sbom/backend.cdx.json"
...
def _write_supply_capture(root: Path, source: str) -> dict:
    path = root / "artifacts/v0.8/supply-chain.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    capture = {
    ...
    return {
        "capture": "artifacts/v0.8/supply-chain.json",
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "test_id": "V08-SC-001",
        "passed": True,
        "source_fingerprint_sha256": source,
...
def test_supply_chain_requires_zero_high_critical_and_unlocked_inputs(
    ...
    declaration = _write_supply_capture(tmp_path, source)
    declaration["accepted_v07_release"]["archive_sha256"] = "0" * 64
    with pytest.raises(
        release.ReleaseEvidenceError,
        match="accepted v0.7 release binding differs",
    ):
        release.validate_supply_chain(tmp_path, declaration, source)
...
def _tag_policy() -> dict:
    return {
        ...
        "package_path": "artifacts/v0.8/openbexi-spell-v0.8.0.tar.gz",
        "sidecar_path": "artifacts/v0.8/openbexi-spell-v0.8.0.tar.gz.sha256",
...
def test_tag_message_producer_is_exact_ordered_and_rejects_any_mutation() -> None:
    ...
    expected = release.create_release_tag_message_v08(policy, **values)
    ...
    release.validate_release_tag_message_v08(expected.encode("utf-8"), expected)

    reordered = lines.copy()
    reordered[2], reordered[3] = reordered[3], reordered[2]
    mutations = (
        expected.replace(release.RELEASE_TAG_TITLE, "SPELL v0.8", 1),
        "\n".join(reordered) + "\n",
    ...
    for mutation in mutations:
        with pytest.raises(release.ReleaseEvidenceError, match="exact ordered"):
            release.validate_release_tag_message_v08(
...
def test_require_tag_validates_real_annotated_object_target_markers_and_sidecar(
    ...
    package = tmp_path / "artifacts/v0.8/openbexi-spell-v0.8.0.tar.gz"
    ...
    work = "f" * 64
    policy = _tag_policy()
    expected_message = release.create_release_tag_message_v08(
        policy,
        release_head=head,
    ...
    _git(tmp_path, "tag", "-a", "v0.8.0", "-F", str(message))
    ...
    _git(tmp_path, "tag", "v0.8")
    with pytest.raises(release.ReleaseEvidenceError, match="secondary v0.8 tag"):
        release.validate_release_tag(
            tmp_path,
            policy,
            release_head=head,
...
def test_release_manifest_shape_rejects_exceptions_claims_and_unknown_fields() -> None:
    ...
    manifest = _manifest_shape()
    manifest["broader_v08_scope"] = True
...
def test_sqlite_skip_contract_is_exact_and_other_suites_have_no_allowlist() -> None:
    assert len(release.SQLITE_ALLOWED_SKIPS) == 15
    assert sum(
...
def test_builder_compatible_api_returns_versioned_entry_point() -> None:
    ...
    assert callable(release.validate_release_evidence_v08)
    assert release.validate_release_evidence_v08.__doc__


def test_builder_is_the_only_explicit_prepublication_package_exception() -> None:
    source = (release.ROOT / "scripts/build_reproducible_v08.py").read_text(
        encoding="utf-8"
    )
