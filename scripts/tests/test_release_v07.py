...
from scripts import build_reproducible_v07 as release


def test_v07_reparse_detector_recognizes_windows_file_attributes(
    ...
    assert release.path_has_link_or_reparse_v07(Path("root"), Path("root/file"))


def test_v07_archive_bytes_are_deterministic_and_normalized(tmp_path: Path) -> None:
...
def test_v07_package_rejects_forbidden_paths(relative: str) -> None:
    with pytest.raises(ValueError):
        release._validate_path(Path(relative))


def test_v07_package_rejects_private_key_bytes(tmp_path: Path) -> None:
...
def test_v07_scanner_literal_contract_matches_every_live_source_file() -> None:
...
def test_v07_scanner_literal_contract_rejects_path_and_byte_mutations(
    relative: str,
) -> None:
    raw = (release.ROOT / relative).read_bytes()
...
def _evidence_canary_xml(
    ...
    from scripts import validate_release_evidence_v07 as evidence
...
def test_v07_package_scans_candidate_manifest_and_tooling_structurally() -> None:
    from scripts import validate_release_evidence_v07 as evidence
    ...
    release._validate_bytes(
        Path("artifacts/v0.7/work-package/tests/tooling.xml"),
        _evidence_canary_xml(evidence.FINAL_TOOLING_SECRET_TESTCASE_NODES),
    )
    release._validate_bytes(
        Path("artifacts/v0.7/final/tests/backend-postgresql.xml"),
        _evidence_canary_xml(evidence.BACKEND_SECRET_CANARY_NODES),
    )


def test_v07_package_rejects_duplicate_or_mislocated_evidence_canaries() -> None:
    from scripts import validate_release_evidence_v07 as evidence

    tooling_relative = "artifacts/v0.7/work-package/tests/tooling.xml"
    ...
    with pytest.raises(ValueError, match="secret material"):
        release._validate_bytes(
            Path("artifacts/v0.7/final/tests/not-tooling.xml"),
            _evidence_canary_xml(evidence.FINAL_TOOLING_SECRET_TESTCASE_NODES),
        )

    backend_relative = "artifacts/v0.7/final/tests/backend-postgresql.xml"
    with pytest.raises(ValueError, match="inventory differs"):
        release._validate_bytes(
    ...
    with pytest.raises(ValueError, match="secret material"):
        release._validate_bytes(
            Path("artifacts/v0.7/final/tests/backend-docker-host.xml"),
            _evidence_canary_xml(evidence.BACKEND_SECRET_CANARY_NODES),
        )


def test_current_v07_product_package_fingerprint_is_constructible() -> None:
    missing = [
        relative
    ...
    if missing:
        with pytest.raises(FileNotFoundError):
            release.product_package_sha256_v07(release.ROOT)
        return
    digest = release.product_package_sha256_v07(release.ROOT)
    assert len(digest) == 64
    assert set(digest) <= set("0123456789abcdef")


def test_v07_fingerprint_image_uses_a_current_source_context_policy() -> None:
    dockerfile = (release.ROOT / "scripts/package-v07.Dockerfile").read_text(
        encoding="utf-8"
    )
    ignore = (
        release.ROOT / "scripts/package-v07.Dockerfile.dockerignore"
    ).read_text(encoding="utf-8")

    assert "COPY . /workspace" in dockerfile
    ignore_lines = ignore.splitlines()
    assert "artifacts/v0.7" in ignore_lines
    ...
    assert "SPELL_v0.7_Gate_0B.md" not in ignore


def test_v07_package_publication_is_a_rollback_transaction() -> None:
    package = (release.ROOT / "scripts/package_release_v07.ps1").read_text(
        encoding="utf-8"
    )

    assert "existing v0.7 package publication is incomplete" in package
    assert "$backupRelease" in package and "$backupSidecar" in package
    assert "refusing to use unsafe v0.7 package publication path" in package
    assert "published v0.7 package pair failed final verification" in package
    ...
    final_verification = package.index(
        "published v0.7 package pair failed final verification"
    )
    ...
    function_cleanup = package[package.index("function Publish-V07PackagePair") :]
...
def test_v07_package_build_outputs_use_owned_qualification_scratch() -> None:
    package = (release.ROOT / "scripts/package_release_v07.ps1").read_text(
        encoding="utf-8"
    )
    ...
    assert "v0.7 package scratch path already exists" in package
    assert "v0.7 package scratch directory is unsafe" in package
    assert "refusing v0.7 package scratch cleanup containing a reparse point" in package
    ...
    assert "Stage-AcceptedV06ExternalRelease" not in package
    assert (
        'assert_accepted_v06_release_v07.ps1") -Root $export'
        in package
    )
...
def test_v07_package_publication_fault_rolls_back_executably(
    tmp_path: Path,
) -> None:
    ...
    package_script = release.ROOT / "scripts/package_release_v07.ps1"
...
$definitions = @($ast.FindAll({
  ...
  $node -is [Management.Automation.Language.FunctionDefinitionAst] -and
    $node.Name -ceq "Publish-V07PackagePair"
}, $true))
...
try {
  Publish-V07PackagePair `
    ...
    -Release (Join-Path $WorkRoot "openbexi-spell-v0.7.0.tar.gz") `
    -Sidecar (Join-Path $WorkRoot "openbexi-spell-v0.7.0.tar.gz.sha256") `
...
if ($caught -cne "injected v0.7 package publication failure") {
...
''',
    ...
    for existing_pair in (False, True):
        ...
        release_path = work_root / "openbexi-spell-v0.7.0.tar.gz"
        sidecar_path = work_root / "openbexi-spell-v0.7.0.tar.gz.sha256"
        if existing_pair:
