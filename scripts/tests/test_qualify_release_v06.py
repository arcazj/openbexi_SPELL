...
RUNNER = ROOT / "scripts" / "qualify_release_v06.ps1"
QUALIFICATION_IGNORE = ROOT / "scripts" / "qualification-v06.Dockerfile.dockerignore"


def test_v06_final_runner_parses_as_powershell() -> None:
...
def test_v06_final_runner_freezes_source_and_never_qualifies_live_bytes() -> None:
    ...
    assert '"scripts/qualification-v06.Dockerfile"' in runner
    assert '"scripts/qualification-v06.Dockerfile.dockerignore"' in runner
    ...
    assert '$gate0aCommit = "f6eba8be0f7ca9e2f1d466aea66902152fb1bbc1"' in runner
...
def test_v06_final_runner_normalizes_only_collected_node_paths() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v06_final_runner_flattens_every_collected_inventory_explicitly() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v06_final_runner_uses_native_argv_without_inline_subprocess_replay() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    snippets = re.findall(
        r"\$[A-Za-z]+(?:Code|Probe) = @'\n(.*?)\n'@",
        runner,
...
def test_v06_final_runner_accepts_leaf_playwright_suites() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v06_final_runner_accepts_intentional_empty_skip_inventories() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    boundaries = (
...
def test_v06_final_runner_stages_only_ledger_bound_external_manuals() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v06_final_runner_probes_exact_manual_and_schema_bytes_in_image() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ignore_lines = QUALIFICATION_IGNORE.read_text(encoding="utf-8").splitlines()

    assert '"artifacts/v0.6/work-package/schema.json"' in runner
    ...
    assert ignore_lines.index("artifacts/v0.6") < ignore_lines.index(
        "!artifacts/v0.6/"
    )
    assert ignore_lines.index("!artifacts/v0.6/") < ignore_lines.index(
        "artifacts/v0.6/*"
    )
    assert ignore_lines.index("artifacts/v0.6/work-package/*") < ignore_lines.index(
        "!artifacts/v0.6/work-package/schema.json"
    )


def test_v06_final_runner_enforces_full_bijections_and_skip_contracts() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ...
    assert "$hostToolingNodes.Count -ne 14" in runner
    assert (
        'scripts/tests/test_release_v06.py::'
        "test_v06_package_scans_candidate_manifest_and_tooling_structurally"
    ) in runner
    assert (
        'scripts/tests/test_release_v06.py::'
        "test_v06_package_rejects_duplicate_or_mislocated_evidence_canaries"
    ) in runner
    assert (
        'scripts/tests/test_release_v06.py::'
    ...
    for node in (
        ...
        "test_v05_final_runner_parses_as_powershell",
    ...
    assert (
        "Merge-JUnit @($toolingBaseXml, $toolingHostXml, $toolingRootXml, $toolingV05Xml)"
        in runner
    )
    ...
    assert "locked-host accepted-v0.5 external release tests failed" in runner
    ...
    assert 'Copy-Item -LiteralPath $stagedManualRoot -Destination $v05ManualDestination -Recurse' in runner
    assert '"openbexi-spell-v0.5.0.tar.gz", "openbexi-spell-v0.5.0.tar.gz.sha256"' in runner
    assert "46949e783e85b72e68f70d1607c6d44bb5234586c248888b2bd4a3d2cf06f17d/regression" in runner
    assert 'Copy-Item -LiteralPath $v05InheritedSource -Destination $v05InheritedDestination -Recurse' in runner
    assert "git -C $root worktree remove --force $v05Worktree" in runner
...
def test_v06_final_runner_uses_typed_fail_closed_cleanup_inspection() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    cleanup = runner[
...
def test_v06_final_runner_captures_exact_nine_suite_contract() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ...
    assert 'schema_version = "spell.v06.frontend-build/1"' in runner
...
def test_v06_final_runner_binds_gate_source_product_and_candidate_evidence() -> None:
    runner = RUNNER.read_text(encoding="utf-8")

    assert 'schema_version = "spell.v06.final-qualification/1"' in runner
    ...
    assert "Assert-V05ArtifactsUnchanged $resolvedSource" in runner
    assert '"e2e/operator-workspace.spec.ts"' in runner
    assert '$realSpecs = @("e2e/integration.spec.ts")' in runner
    assert "driver-projection-real.spec.ts" not in runner


def test_v06_final_runner_serializes_only_the_stateful_live_browser_suite() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v06_final_runner_only_publishes_final_after_cleanup_and_validation() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ...
    assert 'Join-Path $root "artifacts/v0.6/final"' in runner
    assert "release-qualification.json" not in runner
    assert "validate_release_evidence_v06.py --root" not in runner
    assert '$artifactRoot = Join-Path $root "artifacts/v0.6"' in runner
...
def test_v06_final_runner_tears_down_only_owned_resources() -> None:
