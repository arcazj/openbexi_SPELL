...
from scripts import release_v10 as release
...
def test_repository_policy_and_pinned_evidence_validate() -> None:
    ...
    assert result["candidate_commit"] == "8377760be59033b3372512ad812c43cd6d2f7e29"
    assert len(result["reference_hashes"]) == 8
    assert len(result["evidence_hashes"]) == 7
...
def test_package_inventory_excludes_legacy_docs_tools_prior_artifacts_and_v11() -> None:
    ...
    assert "artifacts/v0.10/reference-examples.json" in names
    ...
    assert not any(name.startswith("artifacts/v0.9/") for name in names)
    assert not any("v11" in name.casefold() for name in names)
...
def test_gate_inventory_accepts_only_exact_resolved_results() -> None:
    ...
    with pytest.raises(release.ReleaseV10Error, match="gate failed"):
...
def test_gate_inventory_rejects_missing_and_extra_gate_ids() -> None:
    ...
    with pytest.raises(release.ReleaseV10Error, match="inventory differs"):
        release.validate_gate_results(gates)
...
def test_gate_inventory_rejects_changed_counts_or_unresolved_skips() -> None:
    ...
    with pytest.raises(release.ReleaseV10Error, match="test count differs"):
        release.validate_gate_results(gates)

    gates = _passing_gates()
    gates["backend_compose"]["skipped"] = 1
    gates["backend_compose"]["passed"] -= 1
    with pytest.raises(release.ReleaseV10Error, match="skip count differs"):
        release.validate_gate_results(gates)
...
def test_release_tag_message_is_exact_and_contains_required_nonclaims() -> None:
    ...
    assert message.startswith("SPELL v0.10.0\n\nDecision: ACCEPTED\n")
...
def test_release_policy_rejects_unknown_top_level_fields(tmp_path: Path) -> None:
    ...
    with pytest.raises(release.ReleaseV10Error, match="fields differ"):
...
def test_qualification_validation_is_positive_or_fails_closed_before_publication() -> None:
    if not (release.ROOT / release.QUALIFICATION_PATH).is_file():
        with pytest.raises(release.ReleaseV10Error, match="qualification"):
