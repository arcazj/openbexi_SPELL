...
RUNNER = ROOT / "scripts" / "qualify_release_v08.ps1"
QUALIFICATION_IGNORE = ROOT / "scripts" / "qualification-v08.Dockerfile.dockerignore"


def test_v08_final_runner_parses_as_powershell() -> None:
...
def test_v08_final_runner_freezes_source_and_never_qualifies_live_bytes() -> None:
    ...
    assert '"scripts/qualification-v08.Dockerfile"' in runner
    assert '"scripts/qualification-v08.Dockerfile.dockerignore"' in runner
    ...
    assert '$gate0aCommit = "451c065740e7b6501f86094d9be79578b30b1591"' in runner
    ...
    assert "openbexi-spell-v08-py313-site" not in runner


def test_v08_final_runner_normalizes_only_collected_node_paths() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v08_final_runner_flattens_every_collected_inventory_explicitly() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v08_final_runner_uses_native_argv_without_inline_subprocess_replay() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    snippets = re.findall(
        r"\$[A-Za-z]+(?:Code|Probe) = @'\n(.*?)\n'@",
        runner,
...
def test_v08_final_runner_accepts_leaf_playwright_suites() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v08_final_runner_accepts_intentional_empty_skip_inventories() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    boundaries = (
...
def test_v08_final_runner_stages_only_ledger_bound_external_manuals() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v08_final_runner_probes_exact_manual_and_schema_bytes_in_image() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ignore_lines = QUALIFICATION_IGNORE.read_text(encoding="utf-8").splitlines()

    assert '"artifacts/v0.8/work-package/schema.json"' in runner
...
def test_v08_final_runner_enforces_full_bijections_and_skip_contracts() -> None:
    from scripts import validate_release_evidence_v08 as release_validator
    ...
    assert "$hostToolingNodes.Count -ne 24" in runner
    assert (
        'scripts/tests/test_release_v08.py::'
        "test_v08_package_scans_candidate_manifest_and_tooling_structurally"
    ) in runner
    assert (
        'scripts/tests/test_release_v08.py::'
        "test_v08_package_rejects_duplicate_or_mislocated_evidence_canaries"
    ) in runner
    assert (
        'scripts/tests/test_release_v08.py::'
        "test_v08_package_publication_fault_rolls_back_executably"
    ) in runner
    assert (
    ...
    assert (
        ...
        "@($v05ExportToolingNodes) + @($v06ExportToolingNodes) + @($v07ExportToolingNodes))"
    ) in runner
    assert (
        "Merge-JUnit @($toolingBaseXml, $toolingHostXml, $toolingRootXml, "
        "$toolingV05Xml, $toolingV06Xml, $toolingV07Xml)"
        in runner
    )
    assert "$rootExternalToolingNodes = @(" in runner
    assert (
        "scripts/tests/test_validate_release_evidence_v08.py::"
        "test_v08_inherited_v07_binding_includes_external_archive_sidecar_and_tag"
    ) in runner
    assert "@rootExternalToolingNodes -q" in runner
    assert "locked-host accepted-v0.7 external release tests failed" in runner
    ...
    assert "$rootExternalToolingNodes.Count -ne 22" in runner
...
def test_v08_final_runner_uses_typed_fail_closed_cleanup_inspection() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    cleanup = runner[
...
def test_v08_final_runner_captures_exact_nine_suite_contract() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ...
    assert 'schema_version = "spell.v08.frontend-build/1"' in runner
...
def test_v08_final_runner_binds_gate_source_product_and_candidate_evidence() -> None:
    runner = RUNNER.read_text(encoding="utf-8")

    assert 'schema_version = "spell.v08.final-qualification/1"' in runner
    ...
    assert "Assert-V07ArtifactsUnchanged $resolvedSource" in runner
    ...
    assert '$env:SPELL_TELEMETRY_CONTEXT_ID = "v08-telemetry-synthetic-context"' in runner
...
def test_v08_final_runner_serializes_only_the_stateful_live_browser_suite() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
...
def test_v08_final_runner_only_publishes_final_after_cleanup_and_validation() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    ...
    assert 'Join-Path $root "artifacts/v0.8/final"' in runner
    assert "release-qualification.json" not in runner
    assert "validate_release_evidence_v08.py --root" not in runner
    assert '$artifactRoot = Join-Path $root "artifacts/v0.8"' in runner
...
def test_v08_final_runner_tears_down_only_owned_resources() -> None:
