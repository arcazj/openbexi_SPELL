...
from scripts import create_release_qualification_v05 as producer
from scripts import validate_release_evidence_v05 as release


def _source() -> producer.QualifiedSourceV05:
    return producer.QualifiedSourceV05(
        commit="a" * 40,
        tree="b" * 40,
...
def test_sbom_declaration_is_derived_from_checksum_subject_and_image(
    ...
    directory = tmp_path / "artifacts/v0.5/sbom"
...
def test_supply_chain_declaration_uses_canonical_capture_metrics(
    ...
    source = "d" * 64
    path = tmp_path / "artifacts/v0.5/supply-chain.json"
...
def test_manifest_builder_binds_all_release_primitives(
    ...
    candidate_toolchain = {
        ...
        "python_version": "3.13.14",
        "python_sha256": "2" * 64,
        "qualification_dockerfile_path": "qualification.Dockerfile",
        "qualification_dockerfile_sha256": "3" * 64,
        "qualification_dockerignore_path": "qualification.Dockerfile.dockerignore",
        "qualification_dockerignore_sha256": "4" * 64,
        "qualification_image_id": "sha256:" + "5" * 64,
        "external_manuals": {"files": {}},
    }
    final_toolchain = copy.deepcopy(candidate_toolchain)
    final_toolchain["qualification_image_id"] = "sha256:" + "6" * 64
    monkeypatch.setattr(
        producer,
        "_assert_generation_git_state",
        lambda root, *, manifest_present: source,
    )
    monkeypatch.setattr(producer, "source_fingerprint_v05", lambda root: fingerprint)
    monkeypatch.setattr(producer, "product_package_sha256_v05", lambda root: product)
    monkeypatch.setattr(
        producer,
    ...
    manifest = producer.build_release_qualification_v05(tmp_path)

    assert manifest["qualified_source"]["commit"] == source.commit
    assert manifest["qualified_source"]["tree"] == source.tree
    assert manifest["qualified_source"]["candidate_commit"] == release.CANDIDATE_COMMIT
...
def test_canonical_writer_is_atomic_idempotent_and_refuses_replacement(
    ...
    monkeypatch.setattr(
        producer, "build_release_qualification_v05", lambda root: manifest
    )
    monkeypatch.setattr(
        producer,
        "_assert_generation_git_state",
    ...
    first = producer.create_release_qualification_v05(tmp_path)
    second = producer.create_release_qualification_v05(tmp_path)
    ...
    with pytest.raises(
        ...
        producer.create_release_qualification_v05(tmp_path)
